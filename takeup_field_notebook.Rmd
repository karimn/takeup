...
```{r baseline-data}
baseline.data <- tu.data.reader("raw-data/Baseline Survey.csv") %>% 
...
all.sms.recruit.data <- read_csv("raw-data/Takeup Text Messages.csv", 
                             col_types = list(SubmissionDate = takeup.datetime.type,
...
```{r know-table-data}
endline.know.table.data <- bind_rows(table.A = read_csv("raw-data/Endline Survey-survey-sec_D-tableA.csv") %>% 
                                       transmute(recognized = recogniseA,
                                                 ...
                                                 visited = visitedA, 
                                                 PARENT_KEY, KEY),
                                     table.B = read_csv("raw-data/Endline Survey-survey-sec_D-tableB.csv") %>% 
...
wtp.data <- read_csv("raw-data/Willingness To Pay.csv") %>% 
  mutate_at(vars(SubmissionDate, starttime, endtime), 
            funs(parse_datetime(., datetime.format, locale = locale(tz = "America/New_York")) %>%
                   format(tz = "Africa/Nairobi") %>% 
                   parse_datetime(locale = locale(tz = "Africa/Nairobi")))) %>% 
