...
cluster.info <- read_csv("data/Cluster Survey V3 July 04.csv",
...
pot.verify.data <- read_csv("data/POT verification.csv", 
...
hh.census.data <- tu.data.reader("data/Census.csv") %>% 
...
census.data <- hh.census.data %>% {
  individ.data <- read_csv("data/Census-survey-individual.csv")
...
save(vill.pot.dist, verify.vill.pot.dist, file = "data/takeup_village_pot_dist.RData")
```
...
baseline.data <- tu.data.reader("data/Baseline Survey.csv") %>% 
...
all.sms.recruit.data <- read_csv("data/Takeup Text Messages.csv", 
                             col_types = list(SubmissionDate = takeup.datetime.type,
...
manual.sms.match.list <- read_csv("data/unmatched_sms_wave1.csv", 
                                  col_names = c("KEY.recruit", "name", "KEY.individ.manual"), skip = 1) %>% 
  bind_rows(read_csv("data/unmatched_sms_wave1_2.csv", 
                     col_names = c("KEY.recruit", "name", "KEY.individ.manual")))

sms.list <- bind_rows(latest = read_csv("data/sms_list_0929.csv"), 
                      original = read_csv("data/sms_list_original.csv"), 
                      .id = "data.source") %>% 
...
sense.mon.data <- read_csv("data/Sensitization Monitoring Form.csv") %>% 
  rename(cluster.id = cluster_id,
         hh.grp.key = KEY) %>% 
  right_join(read_csv("data/Sensitization Monitoring Form-household.csv"), c("hh.grp.key" = "PARENT_KEY")) %>% 
...
takeup.data.individuals <- map_df(c("data/Wave 1 Point of Treatment Form-ind.csv", "data/Kakamega Point of Treatment Form-ind.csv"), read.individ.takeup.data) 
...
all.takeup.survey.groups <- bind_rows(read.group.takeup.data("data/Wave 1 Point of Treatment Form.csv") %>% mutate(data.wave = 1), 
                                      read.group.takeup.data("data/Kakamega Point of Treatment Form.csv") %>% mutate(data.wave = 2))
...
census.data %<>% 
  mutate(dewormed = KEY.individ %in% takeup.data$KEY.individ,
         dewormed = ifelse(monitored & (wave == 2 | sms.ctrl.subpop == "phone.owner"), dewormed, NA))
```
...
```{r endline-data}
all.endline.data <- read_csv("data/Endline Survey.csv", col_types = list(#SubmissionDate = takeup.datetime.type,
                                                                    #starttime = takeup.datetime.type,
                                                                    #endtime = takeup.datetime.type,
                                                                    deviceid = col_character())) %>% 
  ...
  select(-county) %>% 
  left_join(cluster.wave.county.data, "cluster.id") %>% 
  filter(cluster.id != 1163 | SubmissionDate >= "2016-11-14") %>% # Data fabricated by enumerators 111
  # filter(wave == 1 | test == 1 | date(SubmissionDate) == "2016-11-7") %>% 
...
write_rds(wave2.endline.backchecks, "takeup_endline_backchecks_wave2_1.0.rds")

wave2.endline.backchecks %>%
  transmute(cluster.id, KEY.endline = KEY, enumerator) %>%
  write_csv("takeup_endline_backchecks_wave2_1.0.csv")
```
...
```{r know-table-data}
endline.know.table.data <- bind_rows(table.A = read_csv("data/Endline Survey-survey-sec_D-tableA.csv") %>% 
                                       transmute(recognized = recogniseA,
                                                 ...
                                                 visited = visitedA, 
                                                 PARENT_KEY, KEY),
                                     table.B = read_csv("data/Endline Survey-survey-sec_D-tableB.csv") %>% 
...
```{r reconsent-data}
reconsent.data <- read_csv("data/Reconsent.csv") %>% 
  mutate_at(vars(SubmissionDate, starttime, endtime), 
            funs(parse_datetime(., datetime.format, locale = locale(tz = "America/New_York")) %>%
                   format(tz = "Africa/Nairobi") %>% 
                   parse_datetime(locale = locale(tz = "Africa/Nairobi")))) %>% 
...
consent.dewormed.reports <- list(endline.survey = all.endline.data, 
  ...
  summarize(monitor.consent = any(monitor.consent), 
            dewormed.reported = ifelse(n_distinct(dewormed.reported) == 1, first(dewormed.reported), NA)) %>% 
  ungroup
```
...
name.match.monitored <- function(census.cluster.data, 
  ...
  census.cluster.data %>% 
    mutate(min.name.match.dist = aaply(dist.mat, 1, . %>% min(na.rm = TRUE)) %>% na_if(Inf),
           ...
           dewormed.matched.2 = !is.na(which.min.name.match.dist) & min.name.match.dist <= max.cost + 1)
            # add(matrix(.data$age, ncol = nrow(census.data), nrow = nrow(.data)) %>%
            #       subtract(matrix(census.data$age.census, ncol = nrow(census.data), nrow = nrow(.data), byrow = TRUE)) %>%  
            #       multiply_by(age.diff.cost) %>% 
            #       abs %>% 
            ...
            #       multiply_by(two.digits.cost) %>% 
            #       coalesce(0.0)) %>%
         #     alply(1, . %>% { ifelse(. <= max.cost, ., NA) %>% which.min %>% magrittr::extract(takeup.cluster.data$KEY.survey.individ, .) }) %>%
         #    map_if(is_empty, ~ NA) %>% 
         #    unlist,
         # dewormed.matched = !is.na(takeup.match))
}

matched.census.data <- census.data %>% 
  filter(monitored, is.na(dewormed) | !dewormed) %>% 
  group_by(cluster.id) %>% 
  do(name.match.monitored(., filter(takeup.data, 
                                    cluster.id %in% unique(.$cluster.id)))) %>% 
  ungroup %>% 
  select(KEY.individ, dewormed.matched, ends_with("min.name.match.dist"))

census.data %<>% 
  left_join(matched.census.data, "KEY.individ") %>% 
  mutate(monitored = !is.na(wave) & monitored,
         dewormed.any = (!is.na(dewormed) & dewormed) | (!is.na(dewormed.matched) & dewormed.matched))
```
