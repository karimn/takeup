...
cluster.wave.county.data <- read_rds("takeup_cluster_wave_county_5.0.rds")
...
cluster.info <- read_csv("Cluster Survey V3 July 04.csv",
                         col_types = list(SubmissionDate = takeup.date.type)) %>% 
  bind_rows(v3 = ., v1 = read_dta("Cluster Survey V1.dta") %>%
...
pot.verify.data <- read_csv("POT verification.csv", 
...
```{r villages-data}
rct.village.codes <- read_csv("village_codes_2.csv", skip = 1, col_names = c("village.cluster.id", "village_name", "village")) %>% 
...
cluster.survey.data <- read_rds("takeup_cluster_survey.rds") 

rct.villages <- read_rds("rct_target_villages_2.0.rds") %>% 
  mutate(new.village = FALSE) %>% 
  bind_rows(read_rds("rct_target_villages_2.0-4.rds") %>% 
...
pre.census.data <- read_rds("pre.census.processed.rds")
```
...
tu.data.reader <- function(file.name, submit.datetime.type = NULL) { # =  "%b %d, %Y %I:%M:%S %p") {
  read_csv(file.name, col_types = list(SubmissionDate = if (is.null(submit.datetime.type)) takeup.datetime.type else submit.datetime.type,
...
hh.census.data.pre24th <- tu.data.reader("census_gps_util23.csv", 
...
hh.census.data <- tu.data.reader("Census.csv") %>% 
...
missing.gps.data <- read_csv("missing_gps.csv") %>% 
...
missing.gps.149.data <- read_csv("missing_gps_149.csv") %>% 
  transmute(lon = gpslongitude,
            lat = gpslatitude, 
            KEY = `key of form with missing GPS data`,
            data.source = "missing_gps_149")

missing.gps.360.data <- read_csv("missing_gps_360.csv") %>% 
  transmute(lon = gpslongitude,
            lat = gpslatitude, 
            KEY = `key of form missing GPS data`,
            data.source = "missing_gps_149")

missing.gps.844.data <- read_csv("missing_gps_844.csv") %>%
...
hh.census.data %<>% 
  anti_join(ldply(c("takeup_returns_toremove.csv", 
                    "takeup_moved_notvalid_toremove.csv",
                    "takeup_noconsent_toremove.csv"), read_csv),
            by = c("KEY" = "key"))

hh.id.dict <- read_rds("takeup_hh_id_dict.rds")
...
person.id.dict <- read_rds("takeup_person_id_dict.rds")
...
census.data <- hh.census.data %>% {
  individ.data <- read_csv("Census-survey-individual.csv")
...
load("takeup_cluster_review.RData")
```
...
```{r corrections}
cluster.954.to.drop <- read_rds("cluster_954_to_drop.rds")
...
```{r}
cluster.strat.data <- read_rds("takeup_cluster_randomization_1.0.rds") %>% 
  select(-county) %>%
...
wave.1.baseline.individuals <- read_rds("takeup_baseline_1.3.rds") %>% 
  left_join(person.id.dict, "KEY.individ")
...
```{r}
wave.2.baseline.individuals <- read_rds("takeup_baseline_w2_1.0.rds") %>% 
  left_join(person.id.dict, "KEY.individ")
...
wave.1.sms.1.individuals <- read_rds("takeup_sms_1_w1_1.3.rds") %>% 
  mutate(sms.set = 1) %>% 
  left_join(person.id.dict, "KEY.individ")
...
```{r load-sms-1-sample}
wave.2.sms.1.individuals <- read_rds("takeup_sms_1_w2_1.1.rds") %>% 
  mutate(sms.set = 1) %>% 
  left_join(person.id.dict, "KEY.individ")
```
...
sms.treated <- read_rds("takeup_sms_sample_all_5.rds")

wave1.sms.ctrl.individuals <- bind_rows(phone.owner = read_rds('takeup_sms_ctrl_4.1_wave1.rds'),
                                        non.phone.owner = read_rds("takeup_sms_ctrl_nonphone_1.0_wave1.rds"),
                                        .id = "sms.ctrl.subpop")

wave2.sms.ctrl.individuals <- read_rds("takeup_sms_ctrl_1.0_wave2.rds")
...
```{r}
sms.airtime.sample <- read_rds("takeup_sms_airtime_sample_1.1.rds")
...
wave1.endline.individuals <- read_rds("takeup_endline_wave1_8.0.rds")
wave2.endline.individuals <- read_rds("takeup_endline_wave2_1.0.rds")
...
id.match.data <- read_rds("takeup_wave1_id_match.rds")
...
```{r, eval=FALSE}
xx <- foreach(1:5, .combine = rbind) %dopar% {
  census.data %>% 
    sample_frac(0.2) %>% {
    # sample_n(5) %>% {
      select(., name1st) %>% #, last_name) %>% 
        map(adist) %>% 
        map(~ magrittr::extract(., upper.tri(.))) %>% 
        reduce(`+`)
    } %>% 
    quantile(probs = c(0.01, 0.05))
}
```
...
baseline.data <- tu.data.reader("Baseline Survey.csv") %>% 
  filter(SubmissionDate >= "2016-09-05", !is.na(present), present == 1, !is.na(consent), consent == 1) %>% 
  select(cluster.id, person, KEY, lon, lat, invalid.coord, present, recruit, consent, starts_with("enumerator"), ends_with("_census")) %>% 
  left_join(filter(., !invalid.coord) %>% identify.closest.cluster, "KEY") %>% 
  # group_by(cluster.id) %>% 
  # do(is.out.of.boundary(., filter(boundaries, boundary.type == "census.convex.hull", cluster.id == .$cluster.id[1]))) %>% 
  # ungroup %>% 
  # left_join(select(cluster.strat.data, cluster.id, wave), "cluster.id") 
  left_join(cluster.wave.county.data, "cluster.id")
...
all.sms.recruit.data <- read_csv("Takeup Text Messages.csv", 
                             col_types = list(SubmissionDate = takeup.datetime.type,
...
manual.sms.match.list <- read_csv("unmatched_sms_wave1.csv", 
                                  col_names = c("KEY.recruit", "name", "KEY.individ.manual"), skip = 1) %>% 
  bind_rows(read_csv("unmatched_sms_wave1_2.csv", 
                     col_names = c("KEY.recruit", "name", "KEY.individ.manual")))

sms.list <- bind_rows(latest = read_csv("sms_list_0929.csv"), 
                      original = read_csv("sms_list_original.csv"), 
                      .id = "data.source") %>% 
...
plot.sms.recruit <- function(cluster.ids, .data, sample.data, ...) {
  ...
  if (any(.data$far.from.census, na.rm = TRUE)) {
    plot.obj <- plot.obj +
      geom_point(aes(lon, lat), shape = 5, size = 5, stroke = 1, color = "darkred", data = .data %>% filter(far.from.census))
  }
...
```{r sms-response-data}
sms.status.data <- c("takeup_bulk_sms_wave1.csv", "takeup_bulk_sms_wave2.csv") %>% 
  map_df(~ read_csv(.,
                    skip = 1,
...
sense.mon.data <- read_csv("Sensitization Monitoring Form.csv") %>% 
  rename(cluster.id = cluster_id,
         hh.grp.key = KEY) %>% 
  right_join(read_csv("Sensitization Monitoring Form-household.csv"), c("hh.grp.key" = "PARENT_KEY")) %>% 
...
```{r}
ea.pot.monitor.data <- read_csv("takeup_mlis_pot_monitoring.csv")
```
...
takeup.dict <- c("takeup_survey_dict_wave1.csv") %>%  # Only needed for wave 1
...
read.individ.takeup.data <- function(file.name) {
  read_csv(file.name,
    ...
    select(-c(person, person_key)) %>% 
    mutate(KEY.individ = na_if(KEY.individ, "0"),
           data.file.name = file.name)
}

takeup.data.individuals <- map_df(c("Wave 1 Point of Treatment Form-ind.csv", "Kakamega Point of Treatment Form-ind.csv"), read.individ.takeup.data) 
...
read.group.takeup.data <- function(file.name) {
  read_csv(file.name,
    ...
    # mutate_at(vars(SubmissionDate, starttime, endtime), funs(format(., tz = "Africa/Nairobi", usetz = TRUE))) %>% #, parse_datetime)) %>%
    rename(cluster.id = cluster_id,
...
all.takeup.survey.groups <- bind_rows(read.group.takeup.data("Wave 1 Point of Treatment Form.csv") %>% mutate(data.wave = 1), 
                                      read.group.takeup.data("Kakamega Point of Treatment Form.csv") %>% mutate(data.wave = 2))
...
find.name.matches <- function(.takeup.data, max.cost = 1) {
...
```{r pot-name-matched-monitored-plot, fig.width=9}
matched.tu.data.2 %>% 
...
```{r endline-data}
all.endline.data <- read_csv("Endline Survey.csv", col_types = list(#SubmissionDate = takeup.datetime.type,
                                                                    #starttime = takeup.datetime.type,
                                                                    #endtime = takeup.datetime.type,
                                                                    deviceid = col_character())) %>% 
...
# write_rds(wave1.endline.backchecks, "takeup_endline_backchecks_wave1_1.0.rds")
# 
# wave1.endline.backchecks %>% 
#   transmute(cluster.id, KEY.endline = KEY, enumerator) %>% 
#   write_csv("takeup_endline_backchecks_wave1_1.0.csv")
```

```{r}
plot.endline <- plot.sms.recruit
```
...
all.endline.data %>% 
  ...
  with(union(cluster.id, closest.cluster)) %>% 
  purrr::discard(is.na) %>% 
  walk(plot.endline, all.endline.data, hh.census.data, use.google.maps = FALSE)
```

## Knowledge Tables

```{r know-table-data}
endline.know.table.data <- bind_rows(table.A = read_csv("Endline Survey-survey-sec_D-tableA.csv") %>% 
                                       transmute(recognized = recogniseA,
                                                 ...
                                                 visited = visitedA, 
                                                 PARENT_KEY, KEY),
                                     table.B = read_csv("Endline Survey-survey-sec_D-tableB.csv") %>% 
