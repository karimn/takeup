...
```{r}
raw.data.path <- . %>% file.path("raw-data", .)
```
...
```{r know-table-data}
endline.know.table.data <- bind_rows(table.A = read_csv(raw.data.path("Endline Survey-survey-sec_D-tableA.csv")) %>% 
                                       transmute(recognized = recogniseA,
                                                 ...
                                                 PARENT_KEY, KEY),
                                     table.B = read_csv(raw.data.path("Endline Survey-survey-sec_D-tableB.csv")) %>% 
...
```{r reconsent-data}
reconsent.data <- read_csv(raw.data.path("Reconsent.csv")) %>% 
  mutate_at(vars(SubmissionDate, starttime, endtime), 
            funs(parse_datetime(., datetime.format, locale = locale(tz = "America/New_York")) %>%
                   format(tz = "Africa/Nairobi") %>% 
                   parse_datetime(locale = locale(tz = "Africa/Nairobi")))) %>% 
...
wtp.data <- read_csv(raw.data.path("Willingness To Pay.csv")) %>% 
  mutate_at(vars(SubmissionDate, starttime, endtime), 
            funs(parse_datetime(., datetime.format, locale = locale(tz = "America/New_York")) %>%
                   format(tz = "Africa/Nairobi") %>% 
                   parse_datetime(locale = locale(tz = "Africa/Nairobi")))) %>% 
...
```{r, include=FALSE}  
save(census.data, hh.census.data, cluster.strat.data, cluster.wave.county.data, pot.info, all.villages, file = file.path("data","takeup_census.RData"))
save(takeup.data, takeup.data.individuals, all.takeup.survey.groups, file = file.path("data", "takeup_at_pot.RData"))
