...
wtp.clusters <- cluster.strat.data %>% 
  filter(assigned.treatment == "control", !is.na(wave)) %>% 
  group_by(county) %>% 
...
wtp.sample <- hh.census.data %>% 
  ...
  filter(is.na(endline.type), assigned.treatment == "control") %>% 
  group_by(cluster.id) %>% 
  do(sample_n(., min(25, nrow(.))) %>% 
       mutate(wtp.offer = sample(c(rep(offer.range, n() %/% 10), sample(offer.range, n() %% 10))))) %>% 
  ungroup %>% 
  select(county, cluster.id, KEY, wtp.offer)

#write_rds(wtp.sample, "data/takeup_wtp_sample_1.0.rds")
```
...
census.data %<>% 
  mutate(dewormed = KEY.individ %in% takeup.data$KEY.individ,
         ...
         # dewormed = ifelse(monitored & !dewormed & (wave == 2 | sms.ctrl.subpop == "phone.owner"), dewormed, NA))
         # dewormed = ifelse(monitored & (!is.na(dewormed) | wave == 2 | sms.ctrl.subpop == "phone.owner"), dewormed, NA))
```
