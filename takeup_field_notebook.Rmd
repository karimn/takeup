...
takeup.data.individuals <- map_df(c("data/Wave 1 Point of Treatment Form-ind.csv", 
                                    "data/Kakamega Point of Treatment Form-ind.csv"), read.individ.takeup.data) 
...
```{r wtp-data}
wtp.sample <- read_rds("data/takeup_wtp_sample_2.0.rds")

wtp.data <- read_csv("data/Willingness To Pay.csv") %>% 
  mutate_at(vars(SubmissionDate, starttime, endtime), 
            funs(parse_datetime(., datetime.format, locale = locale(tz = "America/New_York")) %>%
                   format(tz = "Africa/Nairobi") %>% 
                   parse_datetime(locale = locale(tz = "Africa/Nairobi")))) %>% 
  ...
  mutate(duplicates = n()) %>% 
  ungroup %>% 
  left_join(select(wtp.sample, KEY, lon, lat), by = c("KEY.hh" = "KEY"), suffix = c(".wtp", ".census")) %>% 
