...
name.match.monitored <- function(census.data, 
  ...
  census.data %>% 
    mutate(min.name.match.dist = plyr::aaply(dist.mat, 1, . %>% min(na.rm = TRUE)) %>% na_if(Inf),
           which.min.name.match.dist = ifelse(!is.na(min.name.match.dist), 
                                              plyr::aaply(dist.mat, 1, . %>% 
                                                      which.min %>% 
...
prepare.cluster.takeup.data <- function(.data, 
 ...
 .data %>% 
   ...
   group_by(assigned.treatment, sms.treatment, county_dist_stratum, cluster.id, !!!syms(add_group_by)) %>% 
   ...
   group_by(assigned.treatment, sms.treatment, county_dist_stratum, !!!syms(add_group_by)) %>% 
...
calc.strata.stats <- function(.data, 
  ...
  strata.data %>% {
    distinct_by <- c(.strat.by, "treatment.group", .interact.with)
    # left_join(distinct_(., .dots = c(.strat.by, "treatment.group", .interact.with), .keep_all = TRUE) %>% 
    left_join(distinct(., !!!syms(distinct_by), .keep_all = TRUE) %>% 
...
plot_takeup <- function(takeup_summ_data, takeup_data = NULL, combiner = NULL, data_preparer = function(data) data,
                        ...
                        sms_treatment_col = "sms.treatment.2_static",
  ...
  takeup_summ_data %>%
    inner_data_preparer() %>%
    mutate(mean_est_label = sprintf("%.3f", mean_est)) %>% 
    { 
      dodge_width <- 0.5
      ...
      if (!is_null(lower_level_data)) {
        ...
        if (include_sms_treatment) {
          plot_obj <- plot_obj +
            geom_violin(aes_string(y = "wtd_iter_est", fill = sms_treatment_col),
                        alpha = 0.5, 
                        color = "black",
                        position = dodge,
      ...
      if (show_observed) {
        if (include_sms_treatment) {
          plot_obj <- plot_obj +
            geom_point(aes_string(y = "observed_takeup_prop", group = sms_treatment_col), 
      ...
      # if (include_sms_treatment) {
      #   plot_obj <- plot_obj +
      #     geom_text_repel(aes_string(label = "mean_est_label", group = sms_treatment_col), 
      #                     nudge_x = -0.5, segment.color = "black",
      #                # position = position_dodge(width = dodge_width + 0.25),
      #                     # nudge_x = if (include_sms_treatment) -0.5 else -0.25,
      #                     # label.padding = 0.1, segment.color = NA, 
      #                     # fill = alpha("white", 0.5), 
      #                     size = 3.5, color = alpha("black", 1))
      # } else {
        plot_obj <- plot_obj +
          geom_label_repel(aes(label = mean_est_label),
                          nudge_x = if (include_sms_treatment) -0.5 else -0.25,
                           label.padding = 0.1,
      ...
      if (!is.na(num_obs)) {
        caption_text %<>% str_c(str_interp("N = ${num_obs}."), sep = "\n")
      }
...
plot_dyn_takeup_daily <- function(daily_takeup_summ,
  ...
  color_col <- if (include_sms_treatment) "sms.treatment.2_static" else "incentive_treatment_static"
  
  if (control_observation) {
    daily_takeup_summ %<>% 
      mutate(treatment_fullobs = str_c(as.character(incentive_treatment_static), full_observation, sep = "-"))
    
    line_aes <- aes_string(group = "treatment_fullobs", color = color_col, linetype = "full_observation")
  } else {
    line_aes <- aes_string(color = color_col)
  }
  ...
  if (include_sms_treatment) {
    plot_obj <- plot_obj +
      scale_color_discrete("SMS Treatment") 
      # facet_wrap(~ incentive_treatment_static, ncol = 2, nrow = 2)
  } else {
    plot_obj <- plot_obj +
...
plot_ate <- function(ate_summ_data, ate_data = NULL, combiner = NULL, data_preparer = function(data) data, 
                     ...
                     incentive_treatment_right_col = NULL,
                     sms_treatment_col = "sms.treatment.2_static_left",
                     sms_treatment_col_right = str_replace(sms_treatment_col, "_left$", "_right"),
                     include_sms_treatment = FALSE,
  ...
  if (include_sms_treatment) {
    ate_summ_data %<>% 
      unite_("sms_left_right", c(sms_treatment_col, sms_treatment_col_right), remove = FALSE)
    
    if (!is_null(incentive_treatment_right_col)) {
      ate_summ_data %<>% 
        unite_("sms_left_incentive_right", c(sms_treatment_col, incentive_treatment_right_col), remove = FALSE)
    }
  }
      ...
      if (include_sms_treatment && !is_null(ate_data)) {
        ...
        if (include_sms_treatment) {
          if (sms_treatment_right) {
            plot_obj <- plot_obj +
              geom_linerange(aes_string(ymin = "lb_90", ymax = "ub_90", color = sms_treatment_col, group = "sms_left_right"), 
                             size = 1.5, position = dodge) +
              scale_color_discrete("SMS Treatment")
            plot_obj <- plot_obj +
              geom_pointrange(aes_string(ymin = "lb_95", ymax = "ub_95", color = sms_treatment_col, shape = sms_treatment_col_right),
            ...
            if (!is_null(incentive_treatment_right_col)) { 
              ...
              plot_obj <- plot_obj +
                geom_pointrange(aes_string(ymin = "lb_95", ymax = "ub_95", color = sms_treatment_col, shape = incentive_treatment_right_col), 
                                size = 1, position = dodge, show.legend = FALSE) 
            } else {
              plot_obj <- plot_obj +
...
get_unique_treatments <- function(ate_pairs, subgroups = NULL) {
  ate_pairs %>% {
    ...
    spread(treatment_key, id) %>% {
      distinct_by <- c(str_subset(names(.), "(all|dynamic)_treatment_id$"), subgroups)
  
      distinct(., !!!syms(distinct_by)) %>% 
        arrange(!!!syms(distinct_by))
    } %>% 
    # distinct_(.dots = c(str_subset(names(.), "(all|dynamic)_treatment_id$"), subgroups)) %>% 
    # arrange_(.dots = c(str_subset(names(.), "(all|dynamic)_treatment_id$"), subgroups)) %>% 
...
prepare_treatment_map <- function(static_treatment_map, dynamic_treatment_maps, 
  ...
  prepared <- static_treatment_map %>% 
    ...
    # left_join(distinct_(., .dots = cluster_level_treatment) %>% 
    left_join(distinct(., !!!syms(cluster_level_treatment)) %>% 
                mutate(cluster_treatment_id = seq_len(n())), 
              by = cluster_level_treatment) %>% 
...
prepare_bayesian_analysis_data <- function(origin_prepared_analysis_data, 
  ...
  prepared_analysis_data %<>% 
    ...
    left_join(
      # nest(know_table_data, -KEY.individ, -know.table.type, .key = "know_table"), 
      group_nest(know_table_data, KEY.individ, know.table.type, .key = "know_table"),
      by = "KEY.individ") %>% 
  ...
  if (!is.null(all_ate)) {
    ...
    if (param_dynamics) {
      ...
      if (!is_empty(subgroup_col)) {
        ate_treatments %<>% 
          left_join(select(treatment_map, all_treatment_id, all_of(subgroup_col)), c("static_all_treatment_id" = "all_treatment_id")) 
      }
      
      missing_treatment <- ate_treatments %>%
        plyr::alply(1, function(treat_row) {
          (if (!is_empty(subgroup_col)) {
            semi_join(prepared_analysis_data, select(treat_row, all_of(subgroup_col)), by = subgroup_col) 
      ...
      ate_treatments <- all_ate %>% 
        get_unique_treatments(subgroup_col)
      
      if (!is_empty(subgroup_col)) {
        ate_treatments %<>% 
          left_join(select(treatment_map, all_treatment_id, intersect(names(treatment_map), subgroup_col)), 
                    by = c("all_treatment_id", intersect(names(treatment_map), subgroup_col)))
      }
  ...
  days_poly_trend <- ((1:num_deworming_days) - 1) %>% 
    ...
    plyr::aaply(1, accumulate, multiply_by, .drop = FALSE) %>% 
  ...
  stan_data_list <- lst(
    ...
    all_treatment_dyn_id = if (is_empty(dynamic_treatment_map)) array(dim = c(0,0)) else treatment_map$dynamic_treatment_id,
...
fast_estimate_deworm_prob <- function(iter_cluster_parameters,
  ...
  fast_est_group_prob <- function(treatment_group) {
    treatment_group %>%
      distinct(all_treatment_id, dynamic_treatment_id, .keep_all = TRUE) %>% 
...
estimate_deworm_prob_ate <- function(iter_parameters,
  ...
  est_group_ate <- function(ate_group, iter_est, sim_stratum_hazards, treatment_map_dm, dyn_treatment_mask_map, full_dyn_treatment_map_dm,
    ...
    obs_est <- ate_group %>%
      distinct(!!!syms(group_id_colnames), .keep_all = TRUE) %>% 
      # distinct_(.dots = group_id_colnames, .keep_all = TRUE) %>% 
      rename_(.dots = group_id_colnames %>% 
                setNames(stringr::str_replace(., stringr::str_interp("_${group_suffix}$"), ""))) %>% 
      select(all_treatment_id, dynamic_treatment_id, phone_owner, name_matched) %>% 
      bind_cols(treatment_map_dm[.$all_treatment_id, ], slice(dyn_treatment_mask_map, .$dynamic_treatment_id)) %>% 
      select(-dynamic_treatment_id) %>% 
      plyr::ddply(.(all_treatment_id), estimate_treatment_deworm_prob, 
...
subgroup_combiner <- function(iter_data, outcome_var, subgroups = NULL, 
  ...
  iter_data %>% 
    group_by_at(c(vars(matches(group_by_regex), iter_id), all_of(subgroups))) %>% 
...
prepare_est_deworming <- function(est_data, outcome_var, daily = FALSE, subgroups = NULL, na_rm = FALSE) { 
  ...
  est_data %>%
    ...
    group_by_at(c(vars(matches("(incentive_treatment|(private|social)_value|sms.treatment.2)")), all_of(subgroups))) %>% 
