...
* $d \in \{0, 1\}$, is an indicator for $i$'s community being assigned to be _far_ from the treatment location.
...
\[ y_{im}(z,s,d) = \mathbbm{1}(E_i[U_{im}(z,s,d)] > 0) \lor y_{i,m-1}, \]
hence,
\begin{equation}
...
\end{equation}
and $E_i[\cdot]$ is $i$'s subjective expectation reflecting uncertainty about $\bar{y}_{-i,m}$ and functions $u^h, u^s$, and $u^r$ (the later with uncertainty about $\omega$ and $\Pr_{-i}[v|y,z]$). 
...
* $\widetilde{\mathcal{Z}}^p \equiv \mathcal{Z}^p \setminus \{control\}$ 
* $\widetilde{\mathcal{Z}}^r \equiv \mathcal{Z}^r \setminus \{control\}$ 
* $\widetilde{\mathcal{S}} \equiv \mathcal{S} \setminus \{control\}$
...
* Deworming outcome on day $m$: $Y_{im}(z, s, d) \in \{0, 1\}$ and $Y_i(z, s, d) = \prod_{h = 1}^{12} Y_{ih}(z, s, d)$.
...
We model the probability of take-up as
\[ \Pr[Y_i = 1 | T_i = t, H_i, X_i] = \Pr[Y_i(t) = 1 |H_i, X_i] = \Pr[Y^*_i(t) > 0 |H_i, X_i] \]

and the latent utility is modeled as
...
U(t, h; \beta) &= U(z^p, z^r, d, s, b, h; \beta) \\
               &= \sum_{w\in\widetilde{Z}^p} Z^p_w(z^p)\times\beta^p_{wh} \\
            {} &+ \sum_{w\in\widetilde{Z}^r} \left\{Z^r_w(z^r)\times\beta^r_{wh1} + Z^r_w(z^r)\times d \times \beta^r_{wh2} + Z^r_w(z^r)\times b \times \beta^r_{wh3}\right\} \\
            {} &+ \sum_{w\in\widetilde{S}} \left\{S_w(s)\times\beta^s_{wh1} + S_w(s)\times d \times \beta^s_{wh2} + S_w(S)\times b \times \beta^s_{wh3}\right\} \\
            {} &+ \sum_{u\in\widetilde{Z}^r}\sum_{w\in\widetilde{S}} Z^r_u(z^r) \times \left\{S_w(s)\times\beta^{rs}_{uwh1} + S_w(s)\times d \times \beta^{rs}_{uwh2} + S_w(s)\times b \times \beta^{rs}_{uwh3}\right\} \\
            {} &+ \beta_h + \beta^d_h + \beta^b_h 
\end{align*}
where for convenience we define the following indicator functions:

* $Z^p_w(z^p) \equiv \mathbbm{1}(z^p = w)$ 
* $Z^r_w(z^r) \equiv \mathbbm{1}(z^r = w)$ 
* $S_w(s) \equiv \mathbbm{1}(s = w)$

We carried out our estimation using a logit model using the above parameteric latent function.
...
<!-- &= \Pr[\varepsilon_{im} > -(\tilde{u}^c(z) + \tilde{L}_m(z,s,d) - \tilde{c}(z, d) + \tilde{\eta}_{m})] \\ -->
<!-- &= 1 - F\left(-(\tilde{u}^c(z) + \tilde{L}_m(z,s,d) - \tilde{c}(z, d) + \tilde{\eta}_{m})\right) \\ -->
\begin{equation}
\begin{split}
\Pr[Y_{im}(z,s,d) = 1 | Y_{i,m-1} = 0, z,s,d] &= \Pr[\tilde{u}^c(z) + \tilde{L}_m(z,s,d) - \tilde{c}(z, d) + \tilde{\eta}_{m} + \varepsilon_{im} > 0] \\
                                              &= F\left(\tilde{u}^c(z) + \tilde{L}_m(z,s,d) - \tilde{c}(z, d) + \tilde{\eta}_{m}\right), \label{eqn:latent-var-model} 
\end{split}
\end{equation}
...
\begin{align}
\left[\tilde{u}^c(z) - \tilde{c}(d)\right] - \left[\tilde{u}^c(\mathrm{control}) - \tilde{c}(0)\right] = \sum_{w\in\widetilde{\mathcal{Z}}^p} Z_w(z) \times \beta^p_w + \beta^d \times d \label{eqn:private-benefits-costs-param}
\end{align}
and
\begin{equation}
\begin{split}
...
```{r basic-model-1-prep-data, ref.label="basic-model-1-prep-data", include=FALSE}
```

```{r static-model-ate-plot, ref.label="static-model-ate-plot", fig.width=10, fig.height=7, fig.cap="Posterior distribution of signal/incentive treatment effects on take-up compared with the control arm take-up (with no SMS treatment)"}
```

```{r static-model-phone-ate-plot, ref.label="static-model-phone-ate-plot", fig.width=10, fig.height=10, fig.cap="Posterior distribution of signal/incentive treatment effects on take-up compared with the control arm take-up (with no SMS treatment), split by phone-ownership type."}
```

```{r static-model-phone-dist-ate-plot, ref.label="static-model-phone-dist-ate-plot", fig.width=10, fig.height=10, fig.cap="Posterior distribution of signal/incentive treatment effects on take-up compared with the control arm take-up (with no SMS treatment), for phone-owners only, split by distance from treatment location."}
```

```{r static-model-non-phone-dist-ate-plot, ref.label="static-model-non-phone-dist-ate-plot", fig.width=10, fig.height=5, fig.cap="Posterior distribution of signal/incentive treatment effects on take-up compared with the control arm take-up (with no SMS treatment), for non-phone-owners only, split by distance from treatment location."}
```

```{r static-est-takeup-plotter, ref.label="static-est-takeup-plotter"}
```

```{r static-model-phone-dist-takeup-plot, ref.label="static-model-phone-dist-takeup-plot", fig.width=10, fig.height=11, fig.cap="Posterior distribution of the probability of deworming take-up, split by phone ownership and distance from treatment location."}
```

```{r static-model-ate-percent-plot, ref.label="static-model-ate-percent-plot", fig.width=10, fig.height=7, fig.cap="Posterior distribution of percentage change in take-up compared with the control arm take-up (with no SMS treatment)"}
```

```{r static-model-phone-ate-percent-plot, ref.label="static-model-phone-ate-percent-plot", fig.width=10, fig.height=10, fig.cap="Posterior distribution of percentage change in take-up compared with the control arm take-up (with no SMS treatment), split by phone-ownership type."}
```
...
In the below analysis, we isolate the social effect of bracelets by controling for its private value. We do this by assuming the private value of bracelets is equal to calendars (a conservative assumption given the greater preference for calendars shown in our willingness-to-pay analysis above): $\beta^p_{calendar,h} = \beta^p_{bracelet,h}$for all $h$ and all strata. We present both the combined social and private (consumption) effect as the treatment arm "Bracelet" and the social value only imputed treatment arm as "Bracelet Social".
...
```{r sim-fast-take-up, ref.label="sim-fast-take-up", fig.width=10, fig.height=7, fig.cap="Posterior distribution of the finite sample average probability of deworming take-up across all treatments and subgroups."}
```
...
```{r fast-sim-ate-plot, ref.label="fast-sim-ate-plot", fig.width=10, fig.height=8, fig.cap="Posterior distribution of the finite sample average treatment effect across all treatments and subgroups."}
```
