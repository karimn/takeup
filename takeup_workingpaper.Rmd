...
We model the individuals' utility function as
\begin{equation}
\begin{split}
U_{im}(z,s,d) &= u_i^c(z) + u_{im}^n(\bar{y}_{-i,m}, s) + u^h + u^s v_i + \rho_i\omega u^r - c_i(z, d) \label{eqn:structural-utility}
\end{split} 
\end{equation}
where
...
* $\bar{y}_{-i,m} = \bar{y}_{-i,m}(z, s, d) \in [0, 1]$, is a measure of observed/inferred peer deworming take-up.
* $u_i^c(z)$ is the private consumption value of $z$ and is known to $i$.
* $u_{im}^n(\bar{y}_{-i}, s, m)$ captures the effect of deworming salience from observation and information provided by SMS, as well as the reminder/nudge effects of SMS treatment, on day $m$. This is also known to $i$.
...
* $u^s$ is the social health benefit of deworming, assumed to be uniform for all communities but is unknown. This term combines intrinsic motivation and externality/coordination/conditional cooperation effects.
...
\[ y_{im}(z,s,d) = \mathbbm{1}(E_i[U_{im}(z,s,d)|\theta] > 0 \lor y_{i,m-1} = 1), \]
where $\theta_i$ is a vector of parameters relevant to $i$'s beliefs about $u^h$, $u^s$, and $u^r$,
\begin{equation}
E_i[U_{im}(z,s,d)|\theta] = u_i^c(z) + \overbrace{u_{im}^n(\bar{y}_{-i,m}, s) + E_i[u^h + u^s v_i + \rho_i u^r|\bar{y}_{-i,m}, z, s, d, \theta_i]}^{L_{im}(z,s,d)} - c_i(z, d),  \label{eqn:structural-expected-utility} 
\end{equation}
and $E_i[\cdot]$ is $i$'s subjective expectation. For convenience, we have defined $L_{im}(z, s, d) \equiv u_{im}^n(\bar{y}_{-i,m}, s) + E_i[u^h + u^s v_i + \rho_i u^r|\bar{y}_{-i,m}, z, s, d, \theta_i]$, a function of $i$'s evolving utility over the intervention's twelve deworming days, driven by observed changes in others' take-up, $\bar{y}_{-i,m}$.  
...
<!-- Below assumption assumes fixed conformity effect. Is this reasonable given that individuals might have different priors on others' take-up based on their signal/incentive treatment? -->
...
\Pr[Y_{im}(z,s,d) = 1 | Y_{i,m-1} = 0, z,s,d] &= 1 - \exp\left[-\exp[\tilde{u}^c(z) + \tilde{L}_m(z,s,d) - \tilde{c}(z, d) + \log \lambda_m]\right] \\
                                              &= 1 - \exp\left[-\exp[\tilde{u}^c(z) + \tilde{L}_m(z,s,d) - \tilde{c}(z, d)]\lambda_m\right],
\end{align*}
...
\begin{align}
\left[\tilde{u}^c(z) - \tilde{c}(z, d)\right] - \left[\tilde{u}^c(\mathrm{control}) - \tilde{c}(\mathrm{control}, 0)\right] = \sum_{w\in\widetilde{\mathcal{Z}}^p} Z_w(z) \times \beta^p_w + \beta^d \label{eqn:private-benefits-costs-param}
\end{align}
and
\begin{equation}
\begin{split}
\tilde{L}_m(z, s, d) - \tilde{L}_m(\mathrm{control}, \mathrm{control}, 0) &= \tilde{L}_1(z, s, d) - \tilde{L}_1(\mathrm{control, control}, 0) + \gamma_m(z, s) \\
\tilde{L}_1(z, s, d) - \tilde{L}_1(\mathrm{control, control}, 0) &= \sum_{w\in\widetilde{Z}^r} \left\{Z_w(z)\times\beta^r_{w1} + Z_w(z)\times d \times \beta^r_{w2} \right\} \\
...
\begin{equation} 
\gamma_m(z, s) = \sum_{q = 1}^2\left\{\sum_{w\in\widetilde{Z}^r} (m - 1)^q\times Z_w(z)\times\gamma^r_{qw} + \sum_{w\in\widetilde{S}} \lfloor (m - 1)/2 \rfloor^q\times S_w(s)\times\gamma^s_{qw}\right\}.
\end{equation}
...
\Pr[Y_{im}(z,s,d) = 0 | Y_{i,m-1} = 1, z,s,d] &= 0, 
\end{align*}
where we sum the right-hand terms of \eqref{eqn:private-benefits-costs-param} and \eqref{eqn:learned-params} and use as the exponent of exponential function $\kappa(z,s,d)$.
...
* the finite sample average probability of take-up
  \begin{align*}
    \frac{1}{N}\sum_{i=1}^N& \Pr[Y_i(t) = 1|\hat{\theta}_{j[i]}] \\
    \frac{1}{N}\sum_{i=1}^N& \Pr[Y_{im}(t, m') = 1|\hat{\theta}_{j[i]}] 
  \end{align*}
* the finite sample average treatment effect on take-up
  \begin{align*}
    \frac{1}{N}\sum_{i=1}^N& \Pr[Y_i(t') = 1|\hat{\theta}_{j[i]}] - \Pr[Y_i(t) = 1|\hat{\theta}_{j[i]}] \\
    \frac{1}{N}\sum_{i=1}^N& \Pr[Y_{im}(t', m'') = 1|\hat{\theta}_{j[i]}] - \Pr[Y_{im}(t, m') = 1|\hat{\theta}_{j[i]}] 
  \end{align*}
* the finite sample average dynamic effect of signals and SMS treatment
  \begin{equation*}
    \exp\left\{\frac{1}{N}\sum_{i=1}^N \log[\kappa^{dyn}(t,m,H_i;\hat{\gamma}_{j[i]})]\right\}
  \end{equation*}
...
<!-- TODO Turn this plot (and the one below for SMS) on after fixing the name-matched/monitored issue for take-up level estimates -->

```{r, ref.label="sim-fast-take-up", fig.width=10, fig.height=7, fig.cap="Posterior distribution of the finite sample average probability of deworming take-up across all treatments and subgroups."}
```

```{r, ref.label="fast-sim-ate-plot", fig.width=10, fig.height=7, fig.cap="Posterior distribution of the finite sample average treatment effect across all treatments and subgroups."}
```

```{r, ref.label="fast-sim-daily-obs-plot", fig.width=10, fig.height=5, fig.cap="Posterior distribution of the daily finite sample average probability of deworming take-up (with no SMS treatment)."}
```

```{r, ref.label="fast-sim-daily-dyn-kappa", fig.width=8, fig.cap="Posterior distribution of the finite sample dynamic effect of signal observation."}
```

```{r, ref.label="fast-sim-daily-ate-obs-plot", fig.width=10, fig.height=5, fig.cap="Posterior distribution of the daily finite sample average treatment effect (with no SMS treatment)."}
```

```{r, ref.label="fast-sim-daily-sms-plot", fig.width=9, fig.cap="Posterior distribution of the daily finite sample average probability of deworming take-up (with SMS treatment).", eval=FALSE}
```
...
```{r, ref.label="static-model-ate"}
```

```{r, ref.label="static-model-stan-data"}
```

```{r, ref.label="load-static-model-fit", include=FALSE}
```

```{r, ref.label="basic-model-1-prep-data", include=FALSE}
```

```{r, ref.label="static-model-takeup", fig.width=10, fig.height=7, fig.cap="Posterior distribution of the probability of deworming take-up."}
```

```{r, ref.label="static-model-ate-plot", fig.width=10, fig.height=7, fig.cap="Posterior distribution of signal/incentive treatment effects on take-up compared with the control arm take-up (with no SMS treatment)"}
```
