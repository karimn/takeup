...
<!-- This research has broader relevance than just deworming. We should move reference to deworming a few paragraphs down. -->
...
```{r, fig.width=8, fig.cap="Distribution of actual distance from targeted villages and their assigned treatment location."}
analysis.data %>%  
...
\begin{equation*}
\mathcal{L}_i = \left\{\widetilde{G}\cdot [F_V(\widetilde{G}_i \cdot M_i|\mu, \mu_{j[i]}, \sigma) - F_V(0|\mu, \mu_{j[i]}, \sigma)]\right\}^{W_i}\cdot\left\{ F_V(\widetilde{G}_i\cdot M_i|\mu, \mu_{j[i]}, \sigma)\right\}^{(1 - W_i)} 
\end{equation*}
