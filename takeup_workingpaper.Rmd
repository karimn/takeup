...
```{r know-about-mda, warning=FALSE, message=FALSE, fig.width=8, fig.height=6.5, fig.cap="Knowledge of RCT intervention."}
...
\begin{align}
\prob{D(g) = d} = \prob{D = g | G = g, \boldsymbol{\rho}, \boldsymbol{\pi}, \boldsymbol{\eta}} = \sum_m^M \rho_m \frac{\phi(d; \pi_m(g), \eta_m(g))}{1 - \Phi(d; \pi_m(g), \eta_m(g))}. \label{eq:P-D-struct}
\end{align}
...
\end{align*}
and $\pi_m(\textrm{close}) \leq \pi_m(\textrm{far})$, for all $1 \leq m \leq M$. This corresponds to the causal function
\begin{equation} 
...
```{r wtp-prefer-calendar-plot, fig.height=4, fig.width=8, fig.cap = "Willingness-to-pay experiment results."}
wtp_model_0_fit %>% 
