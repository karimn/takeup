...
\begin{equation}
\begin{split}
U_{im}(z,s,d) &= u_i^c(z) + u_{im}^n(\bar{y}_{-i,m}; s) + u^h(\bar{y}_{-i,m}; v_i) + u^s(\bar{y}_{-i,m};v_i) + u^r(\bar{y}_{-i,m};\rho_i, z) - c_i(d) \label{eqn:structural-utility}
\end{split} 
\end{equation}
where
...
* $u^h(\bar{y}_{-i,m};v_i)$ is the private health benefit of deworming, assumed to be uniform for all $i$ but is unknown, conditional on prosocial type type $v_i$. Health returns can differ in response to inferred peer take-up due to positive externalities, a response that potentially differs between prosocial and non-prosocial individuals.
* $u^s(\bar{y}_{-i,m};v_i)$ is the social health benefit of deworming, assumed to be the same for all communities but is unknown. This term combines intrinsic motivation and externality, coordination, or conditional cooperation effects, and hence potentially responds directly to the level of community takeup, $\bar{y}_{-i,m}$.
* $\rho_i$ is a measure of how image motivated $i$ is.
* $u^r(\bar{y}_{-i,m};\rho_i, z) \equiv \rho_i\omega\cdot\left(\Pr_{-i}[v_i = 1|y_{im} = 1, \bar{y}_{im}, z] - \Pr_{-i}[v_i = 1|y_{im} = 0, \bar{y}_{im}, z]\right)$ is $i$'s belief about how others will infer $v_i$ based on $i$'s deworming, given signal/incentive treatment $z$. This is a random variable due to $i$'s uncertainty about others' inference.
...
hence,
\begin{equation}
E_i[U_{im}(z,s,d)] = u_i^c(z) + \overbrace{u_{im}^n(\bar{y}_{-i,m}; s) + E_i[u^h(\bar{y}_{-i,m};v_i) + u^s(\bar{y}_{-i,m};v_i) + u^r(\bar{y}_{-i,m};\rho_i, z)]}^{L_{im}(z,s,d)} - c_i(d), \label{eqn:structural-expected-utility} 
\end{equation}
...
For convenience, we have defined \[L_{im}(z, s, d) \equiv u_{im}^n(\bar{y}_{-i,m}; s) + E_i[u^h(\bar{y}_{-i,m};v_i) + u^s(\bar{y}_{-i,m};v_i) + u^r(\bar{y}_{-i,m};\rho_i,z)],\] a function of $i$'s evolving utility over the intervention's twelve deworming days, driven by observed changes in others' take-up, $\bar{y}_{-i,m}$.  
...
For all $i$,
\begin{enumerate}
\item $E_i[u^h(\bar{y}_{-i,m};v_i)|z,d] = E_i[u^h(\bar{y}_{-i,m};v_i)| z',d'], \forall i \text{ and }\forall z,z',d,d'$: on the first day of deworming prior beliefs about private health benefits are constant and not influenced by beliefs about others' take-up.
...
# Appendix: Static Bayesian Model {-}
...
```{r static-model-ate, ref.label="static-model-ate"}
```

```{r static-model-stan-data, ref.label="static-model-stan-data"}
```
...
```{r static-model-takeup, ref.label="static-model-takeup", fig.width=10, fig.height=7, fig.cap="Posterior distribution of the probability of deworming take-up."}
```
