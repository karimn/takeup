...
For our analysis we build four models to investigate different aspects of the data generating process. To estimate these models we use hierarchical Bayesian statistical. Two of the models are reduced-form models data that use the experimental intervention to identify treatment effects and the two other models are parameteric models that rely on structural assumptions to identify unobservable counterfactuals. Our Bayesian statistical analysis allows for the flexible construction of statistical models accounting for the complexity of the experiment in terms of the population studied (stratification) and experimental treatment; it allows us to conduct a large number of comparisons over the four experiments, using regularization; it allows us to more efficiently learn from the data using partial pooling; and finally it allows us to efficiently average treatment effects over the four statistical models used [@Imbens2015;@Gelman2013a;@stan;@Yao2018].^[Reduced form OLS regression analysis plots are included in Appendix A for comparison.]
...
where $r^{'}(v^*)$ is the change in reputational returns, caused by a shift in the cut-off type $v^*$ through the change in cost. Reputational returns change as signals become more or less informative, that is, the difference between $\expect{v|v \geq v^*}$ and $\expect{v|v < v^*}$ increases or decreases:
...
## Take-Up Deworming Models

In order to answer our research questions we will take a multipronged approach. We will build and use different causal models to query different aspects of the mechanism of social signaling. First, we will use reduced form models that rely on the experiment's randomized intervention to identify the causal quantities of interest. We will use these models to estimate straight forward estimands such as the average treatment effect of signals/incentives on deworming take-up, and the effect a change in cost (distance) has on such estimands. We will compare bracelets and ink incentives with calendar incentives to separate the social effect (combiningg social signaling with salience and social learning) from the the private consumption effect. We will test the theoretical prediction of how an increase in cost can be mitigated by an increase in reputational returns by comparing the difference in average treatment effects between _close_ and _far_ distance assignment.

Second, we will construct and fit two parameteric models mirroring the theoretical framework laid out in Section \ref{theoretical-framework}. Such models permit us to explicitly model such latent variables as the private benefit of deworming and incentives, the reputational returns to signaling, and the salience/social learning effect of signals. We can repeat any estimation done with the reduced form models, but we can also estimate counterfactuals in response to manipulations difficult or impossible to conduct in an experimental setting, such as holding private benefits fixed at the control level while allowing signaling at the level of bracelets or ink. To effectively communicate our identification strategy and the assumptions it relies on, we will use graphical models to elucidate the causal relationships and the independencies between the models' variables [@Pearl2009]. 
...
$\tilde{\bar{Y}}(z,g;\theta_j)$ is the parameteric version of equation \eqref{eq:finite-sample-mean}. Since we will be relying on Markov Chain Monte Carlo (MCMC) simulations [@stan] for out statistical inference we actually compute this posterior mean^[To calculate credibility intervals, we need to determine the quantiles needed from the $S$ simulations.] as
...
For the structural models we will exploit our fine grained control over the mechanisms governing take-up to compute the following estimands:

* The reputational treatment effect of bracelets, ink, and calendar. 
    - Leaving the distance treatment as assigned.
      \begin{equation}
      \begin{aligned}
        \fsexpect{Y(\textrm{control},\overline{B}(\textrm{bracelet}))} - \fsexpect{Y(\textrm{control})} \\
        \fsexpect{Y(\textrm{control},\overline{B}(\textrm{ink}))} - \fsexpect{Y(\textrm{control})} \\
        \fsexpect{Y(\textrm{control},\overline{B}(\textrm{calendar}))} - \fsexpect{Y(\textrm{control})} 
      \end{aligned} \label{eq:struct-estimand-1}
      \end{equation}
    - Holding fixed the distance at _close_ and _far_. 
      \begin{equation}
      \begin{aligned}
        \fsexpect{Y(\textrm{control},\overline{B}(\textrm{bracelet,close}))} &- \fsexpect{Y(\textrm{control, close})} \\
        \fsexpect{Y(\textrm{control},\overline{B}(\textrm{bracelet,far}))} &- \fsexpect{Y(\textrm{control, far})} \\
        \fsexpect{Y(\textrm{control},\overline{B}(\textrm{ink, close}))} &- \fsexpect{Y(\textrm{control, close})} \\
        \fsexpect{Y(\textrm{control},\overline{B}(\textrm{ink, far}))} &- \fsexpect{Y(\textrm{control, far})} \\
        \fsexpect{Y(\textrm{control},\overline{B}(\textrm{calendar, close}))} &- \fsexpect{Y(\textrm{control, close})} \\
        \fsexpect{Y(\textrm{control},\overline{B}(\textrm{calendar, far}))} &- \fsexpect{Y(\textrm{control, far})} 
...
\end{align}
where $\mathbbm{1}(\cdot)$ is the indicator function, $\Phi(\cdot)$ is the standard normal cumulative density function, and the vector $\boldsymbol{\beta_j}$ has the hierarchical distribution[^discrete-param] 
\begin{equation}
\begin{aligned}
...
\end{equation}
We restrict $\beta(\textrm{calendar})$ and $\beta(\textrm{bracelet})$ to be positive; these incentives are voluntary to use/wear and therefore they are not likely to have a negative effect on utility.

From equation \eqref{eq:P-Y-reduced} we can interpret the causal function of $Y$ to be
\[y \leftarrow f_y(z,g;\boldsymbol{\beta}_j,u_y) = \mathbbm{1}(\beta_{0j} + \sum_{\zeta \in \mathcal{Z}}\sum_{\gamma\in\mathcal{G}} \beta_{1j}(\zeta)\cdot\mathbbm{1}(z = \zeta) +  \beta_{2j}(\gamma)\cdot\mathbbm{1}(g = \gamma) + \beta_{3j}(\zeta,\gamma) \cdot\mathbbm{1}(z = \zeta \wedge g = \gamma) + u_y > 0),\]
...
We use the notation $k[j]$ to denote the county that village $j$ belongs to. The function $\mathsf{diag}(v)$ constructs a diagonal matrix using its vector argument $v$. We use $\mathtt{HalfNormal}$ to reprsent a normal distribution restricted to be positive. 
...
[^discrete-param]: For all the incentive/signal treatments we have the parameters $\beta_{1j}(\textrm{control}) = 0$ (omitted), $\beta_{1j}(\textrm{calendar}), \beta_{1j}(\textrm{ink})$, and $\beta_{1j}(\textrm{bracelet})$; for the distance treatment we have $\beta_{2j}(\textrm{close}) = 0$ (omitted) and $\beta_{2j}(\textrm{far})$; and for the interaction of both treatments we have $\beta_{3j}(\textrm{control,close}) = 0$ (omitted), $\beta_{3j}(\textrm{control,far}), \beta_{3j}(\textrm{calendar,close}), \beta_{3j}(\textrm{calendar,far}), \beta_{3j}(\textrm{ink,close}), \beta_{3j}(\textrm{ink,far}), \beta_{3j}(\textrm{bracelet,close})$, and $\beta_{3j}(\textrm{bracelet,far})$. 
...
For private consumption utility we will introduce the variable $\overline{B}$ to represent the village-level private utility, such that 
\begin{equation} 
  \bar{b} \leftarrow f_{\overline{B}}(z, d; \boldsymbol{\beta}_j, \boldsymbol{\delta}) = \beta_{0j} + \sum_{\zeta \in \mathcal{Z}} \beta_{1j}(\zeta)\cdot\mathbbm{1}(z = \zeta) + c(d;\delta), \label{eq:Bbar-causal}
\end{equation}
where $c(\cdot;\delta)$ is a convex cost (distance) function parameterized by $\delta$. We will consider linear and quadratic cost functions. We separately model the household private consumption utility as
\begin{equation}
...
Figure \ref{dag:struct} presents the causal structure corresponding to equations \eqref{eq:D-causal}, \eqref{eq:Bbar-causal}, \eqref{eq:B-causal}, \eqref{eq:R-causal}, and \eqref{eq:Y-causal}. We represent latent variables using hollow nodes and observed variables using solid nodes. We combine the $U_b, U_r, U_y$, and $V$ latent background variables using the variable $U$ since we make no claims about their indepdence from each other.
...
So far we have restricted ourselves to the causal structural model. Now we turn to the statistical model used to estimate our estimands. We have already defined the statistical model for distance  determined by $G$. Now, we defined the probability
\begin{align*}
...
For $\boldsymbol{\beta}$ we use the same hierarchical model as in equations \eqref{eq:hier-beta-model}. For the cost function we use a simple linear cost function
\[ c(d;\delta) = \delta\cdot d, \]
with
\[ \delta \sim \mathtt{HalfNormal}(0, 1), \]
restricted to be positive. We place no restrictions on the reputational returns coefficient, $\mu$, other than being positive:
\begin{align*}
\mu(z) \sim \mathtt{HalfNormal}(0, 1),~\forall z \in \mathcal{Z}.
\end{align*}

Statistical inference is carried out in two stages. First, given a draw of $\boldsymbol{\beta}$ and $\delta$, $\overline{B}$ is evaluated conditional on observed treatment assignment and distance to deworming treatment. Second, we plug this into the algebraic equation solver available in the statistical modeling software, Stan [@stan], to solve equation \refeq{eq:V-equil}, resulting in $V^*$. Since we model $V$ as normally distributed we make use of the close form solutions for the expectations:
\begin{align*}
...
Figure \ref{fig:fp-2ord-prop-knows-mean-all} shows the proportion of recognized peers that respondents believe have knowledge of their own deworming status. In the control _and_ calendar group, adults believe that 66 percent of peers have information about their deworming status, illustrating i) that there is significant scope for reducing information asymmetries and ii) that a material incentive, in spite of its perceived visibility, does not necessarily increase the (perceived) visibility of actions. Bracelets significantly shifted beliefs: adults reported that 74 percent (a 7.9 percentage point increase) of peers have information about their deworming status. We find a smaller (2.7  percentage points) and insignificant shift of second-order beliefs in the ink treatment. Figure \ref{fig:fp-1ord-prop-knows-mean-all} shows a similar movement for first-order beliefs. Adults in the bracelet treatment are 10.5 (over the control) and 8.8 percentage points (over the calendar group) respectively are more likely to say that they have information about someone's deworming status. For the ink, we find smaller (5.8 over the control group) shifts in first-order beliefs. When asking respondents "Why do you think they would think [Yes/No/Don't know]?" 35 percent of respondent mention that others had "seen the bracelet" in their answer, while only 6 percent and 7 percent of respondents respectively mentioned ink or calendar as a reason. 
...
```{r static_ate_smsctrl-2, fig.width=8, fig.height=3, fig.cap="Posterior distribution of signal/incentive treatment effects on take-up compared with the control arm take-up (with no SMS treatment). Black lines show average treatment effects compared to control group; blue line shows the bracelet treatment effect compared to the calendar treatment."}
