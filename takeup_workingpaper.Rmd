...
\begin{equation}
\begin{split}
\widetilde{U}_{im} &= u_i^c(z) + u_{im}^n(\bar{a}_{im}, s) - c_i(z, d) + u^h + u^s v_i + \rho_i\omega u^r \label{eqn:structural-utility}
\end{split} 
\end{equation}
where
...
* $\bar{a}_{im} = \bar{a}_{im}(z, s, d) \in [0, 1]$, is a measure of observed/inferred peer deworming take-up.
* $u_i^c(z)$ is the private consumption value of $z$ and is known to $i$.
* $u_{im}^n(\bar{a}_i, s, m)$ captures the effect of deworming salience from observation and information provided by SMS, as well as the reminder/nudge effects of SMS treatment, on day $m$. This is also known to $i$.
...
* $u^r \equiv \omega\cdot\left(\Pr_{-i}[v_i = 1|y_{im} = 1] - \Pr_i[v_i = 1|y_{im} = 0]\right)$ is $i$'s belief about how others will infer $v_i$ based on $i$'s deworming. This is a random variable due to $i$'s uncertainty about others' inference.
* $\omega$ is a measure of the social desireability of deworming.
* $\theta$ is a vector of parameters relevant to $i$'s updating of beliefs about $u^h$, $u^s$, and $u^r$.

We model the binary decision to take-up deworming, conditional on not having already done so, as
\[ y_{im} = \mathbbm{1}(E_i[\widetilde{U}_{im}|z,s,d,\theta] > 0), \]
where
\begin{equation}
E_i[\widetilde{U}_{im}|z,s,d,\theta] = u_i^c(z) + u_{im}^n(\bar{a}_{im}, s) - c_i(z, d) + E_i[u^h + u^s v_i + \rho_i u^r|\bar{a}_{im}, z, s, d, \theta] \label{eqn:structural-expected-utility} 
\end{equation}

We can view $i$'s type to be $(\bar{a}_{im}, u^c_i, u_{im}^n, c_i, v_i, \rho_i, \theta)$. So far, we have kept our model agnostic about the distribution of types in the population and individuals' priors and learning models (how their beliefs are updated).

\begin{assumption}[Private Consumption Value of Signals/Incentives]
For all $i$, 
\begin{enumerate}
\item $u^c_i(\text{control}) = u^c_i(\text{ink}) = 0$: ink has no consumption value.
\item $u^c_i(\text{bracelet}) \geq 0$ and $u^c_i(\text{calendar}) \geq 0$: these are voluntary signals/incentive so they cannot cause disutility.
\end{enumerate}
\end{assumption}
...
\item $c_i(z, d) = c_i(z', d), \forall z, z' \in \{\text{control, calendar, braclet}\}$: only ink can introduce disutility since it is mandatory.
\end{enumerate}
\end{assumption}
...
\begin{assumption}[Constant Day One Salience]
For all $i$, $u^n_{i0}(\bar{a}, \text{control}) = u^n_{i0}(\bar{a}', \text{control}), \forall \bar{a}, \bar{a}'$: on the first day salience/nudge effects are constant across signal/incentive arms. 
\end{assumption}

\begin{assumption}[Constant Prior Beliefs About Health Benefits]
For all $i$,
\begin{enumerate}
\item $E_i[u^h|\bar{a}_{i0}(z, s, d), z, d] = E_i[u^h|\bar{a}_{i0}(z', s, d), z', d], \forall z,z' \in \mathcal{Z}$: on the first day of deworming prior beliefs about private health benefits are constant and not influenced by beliefs about others' take-up.
\item $E_i[u^s|\bar{a}_{i0}(z, s, d), z, d] = E_i[u^s|\bar{a}_{i0}(z', s, d), z', d], \forall z,z' \in \mathcal{Z}$: on the first day of deworming prior beliefs about social health benefits and externalities are constant and not influenced by beliefs about others' take-up. 
\end{enumerate}
\end{assumption}
...
* $T_i \equiv (Z^p_{j[i]}, Z^r_{j[i]}, D_{j[i]}, S_i, B_i)',$ a vector of all randomly assigned treatments. 
...
\begin{align*}
  ...
  \sigma &\sim \mathtt{StudentT}[0, \infty)(\nu_\sigma, 0, \tau_\sigma)
\end{align*}
...
\begin{align*}
\alpha_0 &\sim \mathtt{Uniform}(0, 1) \\
\alpha^{clust}_j &\sim \mathtt{StudentT}(\nu^{coef}, 0, \tau^{clust}) \\
\alpha^{strat}_k &\sim \mathtt{StudentT}(\nu^{coef}, 0, \tau^{strat}) \\
\delta &\sim \mathtt{StudentT}\left(\nu^{coef}, 0, \begin{pmatrix} \sigma^{coef} & 0             & \cdots & 0  \\
...
\beta_k &\sim \mathtt{StudentT}\left(\nu^{coef}, \beta, \begin{pmatrix} \tau_1 & 0             & \cdots & 0  \\
                                                     ...
                                                     \vdots        &               & \ddots &   \\
                                                     0             &               &        &  \end{pmatrix}\right) \\\\
\tau^{clust} &\sim \mathtt{StudentT}[0,\infty](\nu^{scale}, 0, \sigma^{scale}) \\ 
\tau^{strat} &\sim \mathtt{StudentT}[0,\infty](\nu^{scale}, 0, \sigma^{scale}) \\
\tau &\sim \mathtt{StudentT}[0,\infty](\nu^{scale}, 0, \sigma^{scale}) 
\end{align*}
...
We parameterize the structural model described in section \ref{structural-model}, by modeling the private consumption benefits and costs as
\begin{equation*}
E\left[\left(u_i^c(z) - c_i(z, d)\right) - \left(u_i^c(\mathrm{control}) - c_i(\mathrm{control}, 0)\right)\right] = \sum_{w\in\widetilde{\mathcal{Z}}^p} Z^p_w(z) \times \beta^p_w + \beta^d 
\end{equation*}

To simplify our notation, we define $L_m(z, s, d) \equiv E[u^n_{im}(\bar{a}_{im}(z, s, d), s) + E_i[u^h + u^s v_i + \rho_i u^r|\bar{a}_{im}(z, s, d), z, s, d, \theta]]$, the function of the learned component of individuals' expected utility function.

\begin{align*}
L_0(z, s, d) - L_0(\mathrm{control, control}, 0) &= \sum_{w\in\widetilde{Z}^r} \left\{Z^r_w(z)\times\beta^r_{w1} + Z^r_w(z)\times d \times \beta^r_{w2} \right\} \\
            {} &+ \sum_{w\in\widetilde{S}} \left\{S_w(s)\times\beta^s_{w1} + S_w(s)\times d \times \beta^s_{w2} \right\} \\
            {} &+ \sum_{u\in\widetilde{Z}^r}\sum_{w\in\widetilde{S}} Z^r_u(z) \times \left\{S_w(s)\times\beta^{rs}_{uw1} + S_w(s)\times d \times \beta^{rs}_{uw2}\right\} \\\\
L_m(z, s, d) - L_m(\mathrm{control}, \mathrm{control}, 0) &= L_0(z, s, d) - L_0(\mathrm{control, control}, 0) \\
                                {}&+ \sum_{q = 1}^2\left\{\sum_{w\in\widetilde{Z}^r} (m - 1)^q\times Z^r_w(z)\times\gamma^r_{qw} + \sum_{w\in\widetilde{S}} \lfloor (m - 1)/2 \rfloor^q\times S_w(s)\times\gamma^s_{qw}\right\}
\end{align*}

In this model we introduce time-invariant and time varying treatments. The former is the same as in the static model, while the later allows us to model changes in take-up over time, compared to the baseline probability of take-up (no variation). We thus capture any accumulated effect of the deworming intervention in the presence of observable social signals and/or a flow of SMS reminders. This allows us to model the effect of social learning and salience on take-up. 

The time-varying treatments we introduced are modeled as a quadratic function of 

* the number of days observing an ink or bracelet signal
* the number of reminders
* the number of social information messages

and we add two more possible random treatments
...
In this model, the outcome variables studies are

* $Y_{im}(t, m) \in \{0, 1\}$, an indicator of deworming on day $m \in \{0, 1, \ldots, 13\}$, in response to time-invariant and time-varying treatments. We define $Y_{i0}(t, m) = 0, \forall i, t, m$, and we use $Y_{i,13} = 1$ to indicate failure to deworm during the 12 day deworming intervention.
* $M_i \in \{1, \ldots, 13\}$ is the day an individual got dewormed. As above, we reserved day 13 for those who did not get dewormed at all ($Y_i = 0$).

We conducted our duration analysis using a piecewise-constant proportional hazard model [@Wooldridge2010].
...
\alpha_m(t, m, h;\beta_{j[i]}, \gamma_{j[i]}, \lambda_{j[i],m}) &= \exp[-\kappa^{static}(t, h;\beta_{j[i]})\cdot\kappa^{dyn}(t, m, h;\gamma_{j[i]})\cdot\lambda_{j[i],m}] \\
\kappa^{static}(t,h;\beta) &= \exp\left[U(t,h;\beta)\right] \\
\kappa^{dyn}(t,m,h;\gamma) &= \exp\left[M(t,m,h;\gamma)\right] \\
\lambda_{jm} &= v_j v_{k[j]} \lambda_m
\end{align*}

where $U(t,h;\beta)$ is the same as in the basic model. The dynamic component of the model is defined as
\begin{align*}
M(t, m, h; \gamma) &= M(z^{dyn}, s^{dyn}, d, s, b, m, h; \gamma) \\
                   &= \sum_{q = 1}^2\left\{\sum_{w\in\widetilde{Z}^r} (m - 1)^q\times Z^r_w(z^{dyn})\times\gamma^r_{qwh} + \sum_{w\in\widetilde{S}} \lfloor (m - 1)/2 \rfloor^q\times S_w(s^{dyn})\times\gamma^s_{qwh}\right\} 
\end{align*}

In addition to the priors defined in the static model above, we define the following priors on the dynamic model's parameters and hyperparameters
\begin{align*}
...
\gamma_k &\sim \mathtt{StudentT}\left(\nu^{coef}, \gamma, \begin{pmatrix} \tau^{dyn}_{1} & 0             & \cdots & 0  \\
                                                     ...
                                                     \vdots        &               & \ddots &   \\
                                                     0            &               &        &  \end{pmatrix}\right) \\
\delta^{clust} &\sim \mathtt{StudentT}[0, \infty)(\nu^{scale}, 0, \tau^{scale}) \\
\delta^{strat} &\sim \mathtt{StudentT}[0, \infty)(\nu^{scale}, 0, \tau^{scale}) 
\end{align*}
