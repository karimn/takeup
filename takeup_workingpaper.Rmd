...
takeup.data %>% 
  mutate_at(vars(dist.pot.group, assigned.treatment), ~ factor(.) %>% fct_relabel(str_to_title)) %>% 
  plot.aggreg.takeup 
```
...
\tilde{\bar{Y}}_j(z,g) =
  \begin{cases} 
    ...
    n_j \expect{Y_j(z,g)} & \text{otherwise.}
  \end{cases} \label{eq:finite-sample-mean} 
\end{align}

If we use $\theta$ to generically represent the statistical parameters of our models, $\bf{X}$ as the matrix of all observed data $(z,g,y)$, and $\prob{\theta|\bf{X}}$ as the posterior distribution of $\theta$, we can write the posterior mean of any estimand as
...
In addition, we will estimate equation \eqref{eq:social-mult-theory}, the _social multiplier_, except in response to a change in utility cost, 
<!-- \[ -\frac{\partial v^*}{\partial d} = \frac{-C'(d)}{1 + r'(v^*)}, \] -->
\[ -\frac{\partial v^*}{\partial c} = \frac{-1}{1 + r'(v^*)}, \]
as well as the multiplier effect on take-up in terms of meter distance, $d$, to treatment location,
<!-- \[ \frac{\expect{Y}}{\partial \overline{C}} = \frac{\partial \expect{Y}}{\partial V^*} \frac{\partial V^*}{\partial \overline{C}} = - \frac{g(V^*)}{1 + r'(V^*)}, \] -->
\[ \frac{\expect{Y}}{\partial d} = \frac{\partial \expect{Y}}{\partial v^*} \frac{\partial v^*}{\partial d} = - f(V^*)\frac{C'(d)}{1 + r'(v^*)}, \]
...
where $f$ is the probability density function of the distribution of prosocial types, $V$. In order for us to be able to compare the change in $v^*$ or $\expect{Y}$ we need to hold fixed the value $v^*$: we want to compare the effect of social signaling in response to change in net benefit from the same initial takeup-level or $v^*$.  
<!-- Thus, -->
<!-- If we define $V^{*-1}(z,v)$ as function returning the level of $\overline{C}$ that would produce the cutoff $v$ when incentive/signal treatment is $z$. -->
<!-- \begin{equation}  -->
...
To understand the causal effect of signals we will estimate the differences
\begin{equation}
\begin{aligned} 
-\frac{\partial v^*}{\partial d}\Biggr|_{\substack{v^*=v}} &+ \frac{\partial v^*}{\partial d}\Biggr|_{\substack{v^*=v}} \\
\frac{\expect{Y}}{\partial d}\Biggr|_{\substack{v^*=v}} &- \frac{\expect{Y}}{\partial d}\Biggr|_{\substack{v^*=v}},
...
\beta(\textrm{calendar}) &\sim \mathtt{Normal}^{+}\left(0, \tau(\textrm{calendar})\right) \\
\beta(\textrm{bracelet}) &\sim \mathtt{Normal}^{+}\left(0, \tau(\textrm{bracelet})\right) \\
\boldsymbol{\sigma}^\textrm{village} &\sim \mathtt{Normal}^{+}\left(\bf{0}, \mathsf{diag}(\boldsymbol{\tau^\textrm{village}})\right) \\
\boldsymbol{\sigma}^\textrm{county} &\sim \mathtt{Normal}^{+}\left(\bf{0}, \mathsf{diag}(\boldsymbol{\tau^\textrm{county}})\right).
...
We use the notation $k[j]$ to denote the county that village $j$ belongs to. The function $\mathsf{diag}(v)$ constructs a diagonal matrix using its vector argument $v$. We use $\mathtt{Normal}^{+}$ to represent a normal distribution restricted to be positive. 
...
<!-- \sigma^\textrm{spline} &\sim \mathtt{Normal}^{+}(0, 1). -->
<!-- \end{align*} -->
...
\begin{align}
\prob{D(g) = d} = \prob{D = g | G = g, \boldsymbol{\rho}, \boldsymbol{\pi}, \boldsymbol{\eta}} = \sum_m^M \rho_m \frac{\phi(d; \pi_{mg}, \eta_{mg}}{1 - \Phi(d; \pi_{mg}, \eta_{mg})}. \label{eq:P-D-struct}
\end{align}
...
\begin{align*}
  ...
  \pi_{mg} &\sim \mathtt{Normal}^{+}(0, 1), \forall m \\
  \eta_{mg} &\sim \mathtt{Normal}^{+}(0, 1), \forall m,  
\end{align*}
and $\pi_{m,\textrm{close}} \leq \pi_{m,\textrm{far}}$, for all $1 < m < M$. This corresponds to the causal function
\begin{equation} 
  d \leftarrow f_D(g; \boldsymbol{\pi}_m, u_D) = \pi_{mg} + u_D, \label{eq:D-causal}
\end{equation}
where $u_D \sim \mathtt{Normal}^{+}(0, \eta_{mg})$.
...
\end{align}
where $c(d;\boldsymbol{\delta})$ and $l(z,d;\boldsymbol{\psi},\boldsymbol{\mu})$ are linear cost functions. We separately model the household private consumption utility as
...
\begin{align}
 r \leftarrow f_R(z,\bar{b}; \boldsymbol{\mu},u_r) = \mu_z\cdot\Delta[v^*(z,\bar{b})] + u_r, \label{eq:R-causal}
\end{align}
where $u_r$ is latent reputational shock and
\begin{equation}
\begin{aligned}
v^*(z,\bar{b}) &= - \bar{b} - \mu_z\Delta[v^*(z,\bar{b})] \\ 
\Delta[v^*(z,\bar{b})] &= \expect{V | V > v^*(z,\bar{b})} - \expect{V | V < v^*(z,\bar{b})}. 
\end{aligned} \label{eq:V-equil}
\end{equation}
Equation \eqref{eq:R-causal} corresponds to the the reputational returns term in \eqref{equ:signal2} where we use $\mu_z = x(z)\lambda$ since we cannot separate the observability effect of the signal, $x(z)$, from the value placed on reputation, $\lambda$.
...
\begin{align*}
  ...
  \sigma_{B,R,Y} &\sim \mathtt{Normal}^{+}(0, 1).
\end{align*}
...
For $\boldsymbol{\beta}$ we use the same hierarchical model as in equations \eqref{eq:hier-beta-model}. For the cost function we will use a linear cost function
\begin{align*}
  c(d;\boldsymbol{\delta}) &= \delta\cdot d, 
\end{align*}
  <!-- c(d;\boldsymbol{\delta}) &= \delta_1\cdot d + \delta_2\cdot d^2,  -->
with
\[ \delta \sim \mathtt{Normal}^{+}(0, 1), \]
restricted to be positive, and for the salience and social learning cost function we will also use a linear function
\begin{align*}
  l(z,d;\boldsymbol{\mu},\boldsymbol{\psi}) &= \mu_z\cdot(\psi\cdot d), 
\end{align*} 
with
\[ \psi \sim \mathtt{Normal}^{+}(0, 1). \]
As for the reputational returns coefficient, $\mu$, we restrict it to be positive
\[\mu_z \sim \mathtt{Normal}^{+}(0, 1),~\forall z \in \mathcal{Z}\]
and
\[\mu_{\textrm{bracelet}},\mu_{\textrm{ink}},\mu_{\textrm{calendar}} > \mu_{\textrm{control}},\]
since all incentives might have some signaling power which cannot be less than the control arm.

Statistical inference is carried out in two stages. First, given a draw of $\boldsymbol{\beta}$ and $\delta$, $\overline{B}$ is evaluated conditional on observed treatment assignment and distance to deworming treatment. Second, we plug this into the algebraic equation solver available in the statistical modeling software, Stan [@stan], to solve equation \refeq{eq:V-equil}, resulting in $V^*$. Since we model $V$ as normally distributed we make use of the closed form solutions, from the Mills ratio, for the expectations,
\begin{align*}
...
* $G_i \in \{-1, 1\}$ indicates initial gift choice of bracelet or calendar, respectively. 
...
* $W_i(m) \in \{0, 1\}$ indicates acceptance of $m$ Ksh to switch gift choice, respectively.

where,
\begin{align*}
  \text{Difference in valuation,}~V_i\equiv V^c_i - V^b_i &\sim \mathtt{Normal}(\mu_{k[i]}, \sigma) \\
...
```{r beliefs-results, fig.width=8, fig.height=6, fig.cap="First and second order beliefs."}
cowplot::plot_grid(
...
```{r fp-2ord-prop-knows-mean-all, fig.width=8, fig.height=3, fig.cap="Second order beliefs (levels).",eval=FALSE}
# fp_2ord_prop_knows_mean %>% 
#   filter(!fct_match(incentive_treatment, "bracelet social")) %>% 
#   group_by(incentive_treatment) %>% 
#   mutate(iter_value = fp_2ord_prop_knows_mean) %>% 
#   summarize(
...
#   ggplot(aes(y = incentive_treatment)) +
...
```{r fp-1ord-prop-knows-mean-all, fig.width=8, fig.height=3, fig.cap="First order beliefs (levels).",eval=FALSE}
# plot_takeup(
...
wtp_stan_data <- analysis.data %>% 
  ...
  prepare_bayes_wtp_data(
    ...
    fit_wtp_model_to_data = TRUE, 
    use_strata_levels = TRUE,
...
```{r, eval=FALSE}
wtp_model_0 <- cmdstanr::cmdstan_model(file.path("stan_models", "wtp_model.stan"), , include_paths = "~/Code/takeup/stan_models")

wtp_model_0_fit <- wtp_model_0$sample(
  data = wtp_stan_data, 
  chains = 8, 
  parallel_chains = 8,
  iter_warmup = 2000,
...
wtp_model_0_fit$save_object(file.path("data", "stanfit", "wtp_model_fit.rds"))
```
...
```{r prepare-wtp-sim-data, eval=FALSE}
wtp_sim_data <- wtp_model_0_fit$draws(c("sim_gift_choice", "sim_wtp_response")) %>% 
...
```{r wtp-table, results="asis", eval=FALSE}
wtp_model_0_fit %>% 
...
```{r wtp-prefer-calendar-plot, fig.height=6, fig.width=8, fig.cap="Willingness-to-pay experiment results."}
wtp_model_0_fit$draws(c("prob_prefer_calendar", "strata_wtp_mu")) %>% 
  ...
  pivot_wider(names_from = per, values_from = quant, names_prefix = "per_") %>% { 
    cowplot::plot_grid(
      ...
      filter(., fct_match(parameter, "strata_wtp_mu")) %>% 
...
To test the assumption that the consumption value of the bracelet is equal to that of the calendar, we conducted a separate willingness-to-pay experiment with a random subsample of adults in the control group as part of the endline survey. Adults had no prior exposure to the bracelet or calendar incentives. The experiment was conducted in two stages. First, as a gift for completing the survey, adults were offered a bracelet or calendar. The majority of people (75 percent) chose the calendar, 23 percent chose the bracelet and 2 percent wanted neither of the items. After choosing their preferred item, respondents were offered to exchange it for the not chosen item, plus a randomly assigned Kenyan Shilling (KSh) value between 0 and 100 (0 and \$1). We estimate the average utility difference between the two items. Figure \ref{fig:wtp-prefer-calendar-plot} shows the posterior distribution of the probability of preference for calendars to bracelets and the difference in valuation between these two incentives. Individuals value calendars around 50 KSh more than the bracelets. The calendar is therefore a valid control for the consumption value of the bracelet. In the following estimation we assume that the private value of bracelets is equal to calendars, which is a conservative assumption given the greater preference for calendars. 
...
```{r cv-table, dependson="load-processed-dist_fit"}
dist_fit_data %>% 
...
```{r take-up-levels, fig.width=8,fig.height=6, fig.cap="Take-up levels in response to an incentive treatment intervention.", dependson="load-processed-dist_fit"}
dist_fit_data %>%
...
```{r take-up-ate, fig.width=8,fig.height=8.5, fig.cap="Average treatment effect in response to an incentive treatment intervention.", dependson="load-processed-dist_fit"}
dist_fit_data %>% 
  filter(!fct_match(model, "STRUCTURAL_STACKED")) %>% 
...
```{r take-up-level-struct, fig.width=8, fig.height=5.15,fig.cap="Take-up levels in response to a signaling intervention, holding the private consumption utility at the control level.", dependson="load-processed-dist_fit"}
dist_fit_data %>%
...
```{r take-up-ate-struct, fig.width=8, fig.height=4.5, fig.cap="Average treatment effect of signaling holding consumption utility at the control level, compared to control.", dependson="load-processed-dist_fit"}
dist_fit_data %>% 
  filter(fct_match(model_type, "structural")) %>% 
  select(model, model_name, est_takeup_te, fit_type) %>% 
  unnest(est_takeup_te) %>%
...
```{r take-up-ate-dist-effect, fig.width=8,fig.height=6, fig.cap="Average treatment effect in response to a change in distance.", dependson="load-processed-dist_fit"}
dist_fit_data %>% 
  filter(!fct_match(model, "STRUCTURAL_STACKED")) %>% 
  mutate(est_takeup_te =
    map_if(est_takeup_te, 
...
```{r take-up-ate-dist-split, fig.width=8,fig.height=8, fig.cap="Average treatment effect in response to an incentive treatment intervention, split by distance.", dependson="load-processed-dist_fit"}
dist_fit_data %>% 
  filter(!fct_match(model, "STRUCTURAL_STACKED")) %>% 
  mutate(est_takeup_te =
    map_if(est_takeup_te, 
...
```{r take-up-level-struct-dist-split, fig.width=8, fig.height=8.5,fig.cap="Take-up levels in response to a signaling intervention, holding the private consumption utility at the control level.", dependson="load-processed-dist_fit"}
dist_fit_data %>%
  filter(fct_match(model_type, "structural")) %>% 
  select(est_takeup_level, model, model_name, fit_type) %>% 
  unnest(est_takeup_level) %>% 
...
```{r take-up-ate-struct-dist-split, fig.width=8, fig.height=7.25, fig.cap="Average treatment effect of signaling holding consumption utility at the control level, compared to control.", dependson="load-processed-dist_fit"}
dist_fit_data %>% 
  filter(fct_match(model_type, "structural")) %>% 
  select(model, model_name, est_takeup_te, fit_type) %>% 
  unnest(est_takeup_te) %>%
...
As mentioned in section \ref{theoretical-framework}, the signaling effect can be amplified when take-up is high and cost could be mitigated when take-up is low. The signaling effect appears to be slightly stronger when the cost is higher (the _far_ treatments), as shown the intention-to-treat analysis, while on the other hand it appears to change very little when using the structural models. However, to properly investigate this we need to estimate the social multiplier effect as defined in equations \eqref{eq:partials-diff}; in order to compare between arms we need to hold fixed the initial level of $v^*$, otherwise we would not be evaluating how the same marginal person is affected by a change in net benefit. 

```{r v-post-histogram, fig.width=8, fig.height=5.4, message=FALSE, warning=FALSE, fig.cap="Posterior histogram of equilibrium $v^*$.", dependson="load-processed-dist-fit"}
...
v_cluster_data %>% 
  ...
  labs(
    title = latex2exp::TeX(r"{Posterior Histogram of Equilibrium $v^*$}"),
...
In Figure \ref{fig:cost-mitig-level}, subfigure (a) shows the expected level of take-up over a range of simulated cutoff values, to clarify how the range of simulated $v$ alters take-up. As expected the lower the $v$ the higher the level of take-up. In subfigure (b) we show the rate of change of the equilibrium $v^*$ cutoff in response to cost at different initial cutoffs, $v^* = v$. We do observe both the amplification effect for $v<0$ and the cost mitigation effect for $v > 0$, for all treatments, but slightly stronger for the bracelet, ink, and calendar. Subfigure (c) presents the same result but in terms of the probability of deworming take-up and how it changes in response to distance to treatment location. However, neither looking at the rate of change in utility nor the probability of take-up is very informative; it is difficult to have a clear interpretation for utility outcomes and we cannot consider the incentive/signal treatments without comparing them to their control counterfactual. 

In Figure \ref{fig:cost-mitig-diff} we compare the estimated rate-of-change of $v*$ and $\expect{Y}$ in response to utility cost and distance to deworming treatment, respectively, between the three incentive arms and the control arm, as in equations \eqref{eq:partials-diff}. The main pattern to notice is that the change in the take-up probability in response to changes to distance is quite high when $v^*$ is high (few takers) for all signals. The highest posterior median difference in the rate-of-change for bracelets, shown in the plotted range, is about 0.5 percentage points per meter. Therefore, we conclude that cost mitigation could have played an small role in this experiment; the width of the credible intervals makes cautious about the magnitude of this difference. On the other hand, the magnitude of the amplification effect as $v^*$ drops and take-up becomes more of a norm appears to be relatively higher for the signal treatments than for the control. However, since in this experiment take-up is not observed at such high levels we do not consider the model's predictions at this range of $v^*$ to be reliable. 

```{r cost-mitig-level, fig.width=8,fig.height=6,fig.cap="Changes in $V^*$ and $E[Y]$ in response utility cost and distance.", dependson="load-processed-dist-fit"}
treatment_colors <- set_names(RColorBrewer::brewer.pal(4, "Set1"), c("control", "ink", "calendar", "bracelet"))
v_lim <- c(-1, 1)

level_rate_of_change_data <- dist_fit_data %>% 
...
takeup_v_plot <- level_rate_of_change_data %>%  
  unpack(prob_takeup) %>% 
  ...
  labs(subtitle = "Take-up", y = TeX("E\\[Y\\]")) +
  NULL

social_mult_v_plot <- level_rate_of_change_data %>%  
  ...
  coord_cartesian(xlim = v_lim) +
  scale_y_continuous(breaks = seq(-5, 1, 0.1)) +
  scale_color_manual("Assigned Treatment (z)", values = treatment_colors, labels = str_to_title) +
  labs(subtitle = "Social Multiplier Compared to Control Level", y = TeX(r"{$-\frac{\partial v^*}{\partial c}$}")) + 
  theme(legend.position = "right") +
  NULL

takeup_partial_v_plot <- level_rate_of_change_data %>%  
  unpack(partial_d) %>% 
  ggplot(aes(v, per_0.5, color = assigned_treatment)) +
  ...
  labs(subtitle = "Rate-of-change of Take-up", y = TeX(r"{$\frac{\partial E\[Y(z,v)\]}{\partial d}}")) +
  theme(legend.position = "right") +
  NULL
...
```{r cost-mitig-diff, fig.width=8,fig.height=6,fig.cap="Differences in changes in $V^*$ and $E[Y]$ in response to utility cost and distance."}
...
diff_social_mult_v_plot <- diff_rate_of_change_data %>%  
  ...
  geom_line(show.legend = FALSE) + 
  geom_ribbon(aes(ymin = per_0.25, ymax = per_0.75, fill = assigned_treatment_left), color = NA, alpha = 0.25, show.legend = FALSE) +
  geom_ribbon(aes(ymin = per_0.1, ymax = per_0.9, fill = assigned_treatment_left), color = NA, alpha = 0.25, show.legend = FALSE) +
  geom_hline(yintercept = 0, linetype = "dotted") +
  geom_vline(xintercept = 0, linetype = "dotted") +
  coord_cartesian(xlim = v_lim, ylim = c(-0.5, 0.2)) +
  scale_color_manual("Assigned Treatment (z)", values = treatment_colors) +
  scale_fill_manual("Assigned Treatment (z)", values = treatment_colors) +
  labs(subtitle = "Social Multiplier Compared to Control Level", y = TeX(r"{$-\frac{\partial v^*}{\partial c}$ Difference}")) +
  theme(legend.position = "bottom", axis.title.y = element_text(size = 8)) +
...
diff_takeup_partial_v_plot <- diff_rate_of_change_data %>%  
  ...
  labs(subtitle = "Rate-of-change of Take-up Compared to Control Level", 
       y = TeX(r"{$\frac{\partial E\[Y(z,v)\]}{\partial d} - \frac{\partial E\[Y(control,v)\]}{\partial d}$}")) + 
  facet_wrap(vars(assigned_treatment_left), labeller = as_labeller(str_to_title)) +
...
est_deworming_day_dist %>% 
  ...
  filter(full_observation,
         ...
         sms.treatment.2_static == sms.treatment.2_dynamic,
         dist.pot.group_static == dist.pot.group_dynamic) %>% 
  mutate_at(vars(starts_with("dist.pot.group_static")), fct_relabel, str_to_title) %>% 
  plot_dyn_takeup_daily(control_observation = FALSE, include_sms_treatment = TRUE) +
