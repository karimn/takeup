...
[^why-bayes]: Our Bayesian analysis allowed for the flexible construction of statistical models accounting for the complexity of the experiment in terms of 
    ...
    This allowed us to manage the large number of treatment comparisons, using regularization, while allowing us to efficiently learn from the data using partial pooling and by modeling treatment effect correlations at different levels (e.g., county) of the study [@Carpenter2017;@Gelman2013a].

<!-- Contribution -->
Our work contributes to the literature of social signaling in three ways. First, we provide direct evidence of the effectiveness of making actions observable through a low-cost signal. Recent empirical studies have highlighted the potential negative effects of visibility like in the case of student effort [@Bursztyn2015] and career ambitions among single women [@Bursztyn2017a]. This study shows how social image concerns can be leveraged to increase public goods. Even in the absence of an understanding of externalities, social signals can increase the direct benefits of deworming - as individuals care to be perceived as doing the ``right thing'' - and as a result by acting in their own best interest, internalize that of others (namely reduce the reinfection risk of children whose health is most affected by worms). 

Second, this paper is one of two papers (in addition to @Karing2018) that provides the first evidence on social signaling in health, and therefore contributes to a large literature on incentives to increase the use of health services and public goods in low-income settings [@Thornton2008; @Banerjee2010;@Ashraf2012] @sato2017psychic. Complementing Karing's [-@Karing2018] findings, our paper demonstrates the potential effectiveness of social signals in an environment with low take-up levels (36 percent take-up of deworming treatment in control group, compared to 73 percent take-up of vaccine four in control group) and where a new technology, like an adult deworming program, is introduced. 

Third, we show that they type of signal offered can drastically influence its effectiveness. To our knowledge, this is the first study that simultaneously tests two different signaling. The (perceived) visibility of signals and their acceptability are important factors to consider. While ink is a known, well-established signal - used for voting - in Kenya, this study shows that signals may not easily be useable across domains. 

Fourth, this study makes an important contribution by exogenously varying distance - which in most settings is impossible - and thereby can cleanly identify its effects on the impact of signals. By generating large exogenous differences in take-up levels, we show that signals have much larger effects at far distances and can undo the negative demand response observed in the control group. We observe no such effect for the material incentive. This result has the potential to inform policy decisions about optimal treatment location choice: as signaling returns are higher at far compared to close distances, increasing individuals' willingness to walk, optimal treatment locations can be set up further apart and with the same number of locations larger geographic areas can be covered.  
...
\noindent The framework further provides insights into how changes in cost affect equilibrium take-up. Following B\'enabou \& Tirole (@Benabou2012a), we assume that the type distribution $G(v)$ has finite support $V \equiv [v_{\text{max}},v_{\text{min}} ]$ and a continuously differentiable density $g(v) > 0$. For simplicity, define the equilibrium take-up level of deworming treatment as a function of the cost $c$ as $a(c) = 1-G(\hat{v}(c))$, with its derivative $\frac{d y(c)}{d c} = -g(\hat{v}) \frac{\partial \hat{v}}{\partial c}$. Further, we assume that cost enter individuals' utility function linearly and that the type distribution is unimodal^[If the type distribution is uniform, reputational returns will be constant, such that a change in $c$ has the same effect on take-up as in the no visibility case.]. In the absence of visibility (x=0), a one unit increase in $c$ will reduce take-up by $g(\hat{v})$. As actions become visible (asssume x=1), there is an indirect effect ($\sim$ \it{social multiplier}\rm) of cost on take-up through reputational returns: 
