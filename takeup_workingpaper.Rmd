...
abstract: "We present findings for a randomized control trial (RCT) assessing the effectiveness of social incentives on adult deworming. Working with the Kenyan Government, we implemented a new community deworming program that offered free deworming treatment to adults and explicitly emphasized the public good aspect of deworming. We distributed two types of social incentives in the form of colorful bracelets and ink. The bracelets and ink made the decision to deworm or abstain from treatment observable and allowed adults to signal to others that they contributed to protecting their community from worms. We further introduced a calendar as private incentive that was comparable in its consumption value to the bracelet but could not easily be observed by others. Communities were randomized into being eligible for the ink, calendar or bracelet incentive or a control arm where no incentive was offered. In a second stage, we offered free text messages to a random subset of adults living in these communities, reminding them of the availability of free deworming treatment and providing information about deworming treatment take-up in their community. We found that bracelets strongly incentivized deworming take-up, and our results point to this being a social rather than a private motivation. In addition, we found that informational text messages amplified this effect." 
...
<!-- TODO This research has broader relevance than just deworming. We should move reference to deworming a few paragraphs down. -->
...
This study explores cost-effective ways to achieve high deworming take-up among adults by offering free deworming treatment at central locations and incentives. The evaluation of the incentivized community deworming program relates four separate strands of the literature: compliance with mass drug administration (MDA), incentives, voluntary good contributions, and pro-social signaling. 
...
<!-- TODO update results in intro with new Bayesian model results -->
...
The remainder of the report is organized as follows: In section \ref{the-intervention} we describe the design of the intervention and the motivation behind it; in section \ref{experiment-design} we describe the design of the experiment; in section \ref{descriptive-statistics} we provide descriptive statistics from the experiment; in section \ref{analysis} we conduct our analysis; in section \ref{cost-effectiveness} we compare the cost effectiveness of the incentives/signals tested; and finally, in section \ref{conclusion} we present our conclusions.
...
\[ \Delta\tau[(z,s,d),(z', s', d')] \equiv \frac{1}{N}\sum_i \Delta\tau_i[(z,s,d),(z', s', d')] = \frac{1}{N}\sum_i Y_i(z,s,d) - Y_i(z',s',d') \]
and
\[ \Delta\tau[(z,s),(z', s')] \equiv \frac{1}{N}\sum_i \Delta\tau_i[(z,s,D_{j[i]}),(z', s', D_{j[i]})]  \]
\[ \tau[(z,s),(z', s')] \equiv \frac{1}{N}\sum_i Y_i(z,s,D_{j[i]}) - Y_i(z',s',D_{j[i]}), \]
...
Table \ref{tab:wtp} shows the summary of the posterior distribution of the willingness-to-pay model. As shown, the posterior probability of preferring calendar to bracelets quite high with an average of 42-53 Ksh difference in valuation. 
...
We assume the private value of bracelets is equal to calendars : $\beta^p_{calendar} = \beta^p_{bracelet}$ for all strata. 
\end{assumption}
 
 (a conservative assumption given the greater preference for calendars shown in our willingness-to-pay analysis above) We present both the combined social and private (consumption) effect as the treatment arm "Bracelet" and the social value only imputed treatment arm as "Bracelet Social".
...
```{r static-model-phone-ate-plot, ref.label="static-model-phone-ate-plot", fig.width=10, fig.height=10, fig.cap="Posterior distribution of signal/incentive treatment effects on take-up compared with the control arm take-up (with no SMS treatment), split by phone-ownership type.", dependson="basic-model-1-prep-data"}
```

```{r static-model-phone-dist-ate-plot, ref.label="static-model-phone-dist-ate-plot", fig.width=10, fig.height=10, fig.cap="Posterior distribution of signal/incentive treatment effects on take-up compared with the control arm take-up (with no SMS treatment), for phone-owners only, split by distance from treatment location.", dependson="basic-model-1-prep-data"}
```

```{r static-model-non-phone-dist-ate-plot, ref.label="static-model-non-phone-dist-ate-plot", fig.width=10, fig.height=5, fig.cap="Posterior distribution of signal/incentive treatment effects on take-up compared with the control arm take-up (with no SMS treatment), for non-phone-owners only, split by distance from treatment location.", dependson="basic-model-1-prep-data"}
```
...
```{r static-model-ate-percent-plot, ref.label="static-model-ate-percent-plot", fig.width=10, fig.height=7, fig.cap="Posterior distribution of percentage change in take-up compared with the control arm take-up (with no SMS treatment)", dependson="basic-model-1-prep-data"}
```

```{r static-model-phone-ate-percent-plot, ref.label="static-model-phone-ate-percent-plot", fig.width=10, fig.height=10, fig.cap="Posterior distribution of percentage change in take-up compared with the control arm take-up (with no SMS treatment), split by phone-ownership type.", dependson="basic-model-1-prep-data"}
```
...
In Figure \ref{fig:sim-fast-take-up}, we are reporting the posterior distribution of deworming take-up levels as defined in \eqref{eqn:any-day-takeup-level}, separately by distance from treatment location and SMS cross treatment.
...
In Figure \ref{fig:fast-sim-ate-plot}, we report the posterior distribution of treatment effects as defined in \eqref{eqn:any-day-ate}, separately by distance from treatment location and SMS cross treatment. We report treatment effect in comparison with
...
```{r fast-sim-ate-phone-plot, ref.label="fast-sim-ate-phone-plot", fig.width=10, fig.height=10, fig.cap="Posterior distribution of the finite sample average treatment effect across all treatments and subgroups."}
```
...
<!-- TODO Clean up this plot and make it useful  -->

```{r, ref.label="wtp-model-check-plot", fig.cap="Comparing observed choices with those predicted by the model."}
```

# Conclusion

While the estimated absolute take-up rates in response to incentives are potentially lower than the very high take-up proportions that might be needed to achieve STH infection transmission breakpoint^[This is still an open research question.], the estimated average treatment effects are high. Thus, as an incentive for the take-up of a health-related treatment with low private valuation (such as deworming or vaccination), bracelets are remarkably effective. They were also found to be the most cost-effective approach even if it was not the lowest in cost. 

It is worth noting that the deworming take-up data collected is based on a monitoring activity more likely to suffer from false negatives than false positives. In other words, we are more likely to miss the treatment of compliers than to incorrectly record the treatment of non-compliers, and therefore, the absolute take-up rates estimates are likely to be a lower bound.

Finally, our analysis of SMS treatment combined with incentives does show high levels of absolute take-up. Since social incentives potentially activate different mechanisms such as social signaling and social learning, it is important for us to conduct more in-depth model-based analysis (which the goal of the academic paper we plan to produce). Even if SMS social information treatment is not scalable it would be very useful to know why it was so effective, in order to find more practical approaches to replicate its effect. For example, if social information caused a upward updating of beliefs about others take-up and thus increased the returns from signaling, we should consider other approaches to improve social knowledge of community take-up.

# References

<div id="refs"></div>

# Appendix: Bayesian Analysis Stratified by Phone Ownership { - }


```{r, ref.label="sim-fast-take-up-phone-owner", fig.width=10, fig.height=7, fig.cap="Posterior distribution of the finite sample average probability of deworming take-up across all treatments and subgroups."}
```

```{r, ref.label="fast-sim-ate-plot-phone-owner", fig.width=10, fig.height=7, fig.cap="Posterior distribution of the finite sample average treatment effect across all treatments and subgroups."}
```

```{r, ref.label="fast-sim-daily-obs-plot-phone-owner", fig.width=9, fig.height=7, fig.cap="Posterior distribution of the daily finite sample average probability of deworming take-up (with no SMS treatment)."}
```

```{r, ref.label="fast-sim-daily-dyn-kappa-phone-owner", fig.width=8, fig.cap="Posterior distribution of the finite sample dynamic effect of signal observation."}
```

```{r, ref.label="fast-sim-daily-ate-obs-plot-phone-owner", fig.width=8, fig.height=7, fig.cap="Posterior distribution of the daily finite sample average treatment effect (with no SMS treatment)."}
```

```{r, ref.label="fast-sim-no-obs-plot-phone-owner", fig.width=10, fig.height=7, , fig.cap="Posterior distribution of the finite sample average treatment effect across all treatments and subgroups (comparing treatment with signal and SMS observation with imputed outcomes with no observation)."}
```

# Appendix: Regression Analysis { - }

In this appendix we conduct a reduced form regression analysis in order identify the causal impact of cluster assigned incentive treatments; cluster assigned distance to treatment location i.e. exogenous variation in travel cost; and individually assigned text message reminder and social information treatment. We use the regression approach proposed by @Imbens2015 to analyze stratified experiments:
...
```{r dist-reg-covar, ref.label="dist-reg-covar", warning=FALSE}
```
...
```{r sms-dist-reg-covar, ref.label="sms-dist-reg-covar", warning=FALSE}
```

```{r reg-tab-data, include=FALSE}
reg.table.data <- list(#"Monitored, No SMS" = reg.output.sms.ctrl, 
                       "All, No SMS" = restrict.unmon.reg.output.sms.ctrl, 
...
dist.reg.table.data <- list("All, No SMS" = restrict.unmon.reg.output.sms.ctrl.dist,
...
sms.reg.table.data <- list("All, SMS" = restrict.unmon.reg.output.sms.treat,
...
dist.sms.reg.table.data <- list("All, SMS" = restrict.unmon.reg.output.sms.treat.dist,
...
3. The average treatment effects of incentives and SMS treatment on deworming take-up, excluding individuals who do not own a mobile phone.
4. The average treatment effects of incentives, distance to treatment location and SMS treatment on deworming take-up, excluding individuals who do not own a mobile phone.
...
```{r reg-basic-plot, fig.width=8, fig.cap="Plot of regression results comparing incentive treatments (without controls)."}
prep.sms.ctrl.plot.data(restrict.unmon.reg.output.sms.ctrl) %>% 
  plot.sms.ctrl.takeup() 
```

```{r reg-basic-covar-plot, fig.width=8, fig.cap="Plot of regression results comparing incentive treatments (with control)."}
...
```{r dist-reg-plot, fig.width=8, fig.height=7, fig.cap="Plot of regression results comparing incentive treatments with distance from treatment location interactions (without controls)."}
...
```{r dist-reg-covar-plot, fig.width=8, fig.height=7, fig.cap="Plot of regression results comparing incentive treatments with distance from treatment location interactions (with controls)."}
...
```{r sms-reg-plot, fig.width=8, fig.height=6, fig.cap="Plot of regreession results comparing incentive treatment with SMS treatment interactions (without controls)."}
prep.sms.treat.plot.data(restrict.unmon.reg.output.sms.treat) %>% 
  plot.sms.treat.takeup() 
```

```{r sms-reg-covar-plot, fig.width=8, fig.height=6, fig.cap="Plot of regreession results comparing incentive treatment with SMS treatment interactions (with controls)."}
...
sms.interact.table.data <- list("All, SMS" = restrict.unmon.reg.output.sms.treat,
                                "Endline, SMS" = reg.output.sms.treat.covar) %>% 
  map(linear_tester, sms.interact.diff.lh %>% magrittr::extract(!str_detect(., "far")))

sms.dist.interact.table.data <- list("All, SMS" = restrict.unmon.reg.output.sms.treat.dist,
                                     "Endline, SMS" = reg.output.sms.treat.dist.covar) %>% 
  map(linear_tester, sms.interact.diff.lh)
```

```{r sms-interact-tab, results="asis", warning=FALSE}
...
```{r sms-dist-plot, fig.width=8, fig.height=8, fig.cap="Plot of regression results comparing incentive/signal treatment with SMS and distance from treatment location interactions (without controls)."}
prep.sms.treat.dist.plot.data(restrict.unmon.reg.output.sms.treat.dist) %>% 
  ...
  labs(subtitle = "Split by distance to treatment location")
```

```{r sms-dist-covar-plot, fig.width=8, fig.height=8, fig.cap="Plot of regression results comparing incentive/signal treatment with SMS and distance from treatment location interactions (with controls)."}
