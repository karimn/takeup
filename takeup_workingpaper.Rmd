...
\begin{equation}
\begin{split}
U_{im}(z,s,d) &= u_i^c(z) + u_{im}^n(\bar{y}_{-i,m}, s) + u^h + u^s(\bar{y}_{-i,m}) v_i + \rho_i\omega u^r - c_i(z, d) \label{eqn:structural-utility}
\end{split} 
\end{equation}
where
...
* $u_i^c(z)$ is the private consumption value of $z$ which is known to $i$.
* $u_{im}^n(\bar{y}_{-i}, s, m)$ captures the effect of deworming salience^[We use this term broadly to refer to any "nudge" or reminder effects.] from observation and information provided by SMS on day $m$. This is also known to $i$.
* $c_i(z, d)$ is the cost of deworming at distance $d$ and with assigned signal/incentive $z$. This is also known to $i$.
<!-- TODO we should explore heterogeneity here with mixtures models -->
* $u^h$ is the private health benefit of deworming, assumed to be uniform for all $i$ but is unknown.
* $u^s(\bar{y}_{-i,m})$ is the social health benefit of deworming, assumed to be the same for all communities but is unknown. This term combines intrinsic motivation and externality, coordination, or conditional cooperation effects, and hence potentially responds directly to the level of community takeup, $\bar{y}_{-i,m}$.
...
* $u^r \equiv \omega\cdot\left(\Pr_{-i}[v_i = 1|y_{im} = 1] - \Pr_{-i}[v_i = 1|y_{im} = 0]\right)$ is $i$'s belief about how others will infer $v_i$ based on $i$'s deworming. This is a random variable due to $i$'s uncertainty about others' inference.
...
\[ y_{im}(z,s,d) = \mathbbm{1}(E_i[U_{im}(z,s,d)|\theta] > 0) \lor y_{i,m-1}, \]
where $\theta_i$ is a vector of parameters relevant to $i$'s beliefs about $u^h$, $u^s$, and $u^r$,
\begin{equation}
E_i[U_{im}(z,s,d)|\theta] = u_i^c(z) + \overbrace{u_{im}^n(\bar{y}_{-i,m}, s) + E_i[u^h + u^s(\bar{y}_{-i,m}) v_i + \rho_i u^r|\bar{y}_{-i,m}, z, s, d, \theta_i]}^{L_{im}(z,s,d)} - c_i(z, d),  \label{eqn:structural-expected-utility} 
\end{equation}
and $E_i[\cdot]$ is $i$'s subjective expectation. For convenience, we have defined \[L_{im}(z, s, d) \equiv u_{im}^n(\bar{y}_{-i,m}, s) + E_i[u^h + u^s(\bar{y}_{-i,m}) v_i + \rho_i u^r|\bar{y}_{-i,m}, z, s, d, \theta_i],\] a function of $i$'s evolving utility over the intervention's twelve deworming days, driven by observed changes in others' take-up, $\bar{y}_{-i,m}$.  

We can view $i$'s type to be $(\bar{y}_{-i,m}, u^c_i, u_{im}^n, c_i, v_i, \rho_i, \theta_i)$. So far, we have kept our model agnostic about the distribution of types in the population and individuals' priors and learning models (how their beliefs are updated).

\begin{assumption}[Private Consumption Value of Signals/Incentives]
For all $i$, 
\begin{enumerate}
\item $u^c_i(\mathrm{control}) = u^c_i(\mathrm{ink}) = 0$: ink has no consumption value.
...
For all $i$,
\begin{enumerate}
\item $c_i(z, d) \geq c_i(z, d') \geq 0, \forall d > d'$
\item $c_i(z, d) = c_i(z', d), \forall z, z' \in \{\mathrm{control, calendar, braclet}\}$: only ink can introduce disutility since it is mandatory.
\end{enumerate}
\end{assumption}

<!-- Below assumption assumes fixed conformity effect. Is this reasonable given that individuals might have different priors on others' take-up based on their signal/incentive treatment? Perhaps we can weaken this by using strong priors on this equality and make use of reported beliefs on community take-up (in the baseline survey). This allows the data to overwhelm this assumption if needed. -->
...
For all $i$,
\begin{enumerate}
\item $E_i[u^h|\bar{y}_{-i,1}(z, s, d), z, d] = E_i[u^h|\bar{y}_{-i,1}(z', s, d), z', d], \forall z,z' \in \mathcal{Z}$: on the first day of deworming prior beliefs about private health benefits are constant and not influenced by beliefs about others' take-up.
\item $E_i[u^s|\bar{y}_{-i,1}(z, s, d), z, d] = E_i[u^s|\bar{y}_{-i,1}(z', s, d), z', d], \forall z,z' \in \mathcal{Z}$: on the first day of deworming prior beliefs about social health benefits and externalities are constant and not influenced by beliefs about others' take-up. 
\end{enumerate}
\end{assumption}
...
\begin{equation*}
E_i[U_{im}(z,s,d)|\theta] = \tilde{u}^c(z) + \tilde{L}_m(z,s,d) - \tilde{c}(z, d) + \tilde{\eta}_m + \varepsilon_{im}. 
\end{equation*}
...
\begin{align*}
\Pr[Y_{im}(z,s,d) = 1 | Y_{i,m-1} = 0, z,s,d] &= 1 - \exp\left\{-\exp[\tilde{u}^c(z) + \tilde{L}_m(z,s,d) - \tilde{c}(z, d) + \log \lambda_m]\right\} \\
                                              &= 1 - \exp\left\{-\exp[\tilde{u}^c(z) + \tilde{L}_m(z,s,d) - \tilde{c}(z, d)]\cdot\lambda_m\right\},
\end{align*}
...
\begin{align}
\left[\tilde{u}^c(z) - \tilde{c}(z, d)\right] - \left[\tilde{u}^c(\mathrm{control}) - \tilde{c}(\mathrm{control}, 0)\right] = \sum_{w\in\widetilde{\mathcal{Z}}^p} Z_w(z) \times \beta^p_w + \beta^d \times d \label{eqn:private-benefits-costs-param}
\end{align}
and
\begin{equation}
\begin{split}
\tilde{L}_m(z, s, d) - \tilde{L}_m(\mathrm{control}, \mathrm{control}, 0) &= \Delta \tilde{L}_m(z, s, d) \\
