...
plot_estimands <- function(.data, y, include_prior_predict = FALSE) {
  ...
  ggplot_obj +
    ...
    geom_point(aes(x = mean_est, color = model), fatten = 3, position = plot_pos, data = . %>% filter(fct_match(fit_type, "fit"))) +
...
```{r take-up-levels, fig.width=8,fig.height=7, fig.cap="Take-up levels in response to an incentive treatment intervention.",cache=FALSE}
dist_fit_data %>%
...
```{r take-up-ate, fig.width=8,fig.height=11.5, fig.cap="Average treatment effect in response to an incentive treatment intervention."}
dist_fit_data %>% 
  filter(!fct_match(model, "STRUCTURAL_STACKED")) %>% 
...
```{r take-up-level-struct, fig.width=8, fig.height=7,fig.cap="Take-up levels in response to a signaling intervention, holding the private consumption utility at the control level."}
dist_fit_data %>%
...
```{r take-up-ate-struct, fig.width=8, fig.height=6, fig.cap="Average treatment effect of signaling holding consumption utility at the control level, compared to control."}
dist_fit_data %>% 
  filter(fct_match(model_type, "structural")) %>% 
  select(model, model_name, est_takeup_te, fit_type) %>% 
  unnest(est_takeup_te) %>%
...
```{r take-up-levels-dist-split, fig.width=8,fig.height=12, fig.cap="Take-up levels in response to an incentive treatment intervention, split by assigned\ndistance group."}
dist_fit_data %>%
  filter(!fct_match(model, "STRUCTURAL_STACKED")) %>% 
  mutate(est_takeup_level = map(est_takeup_level, filter, is.na(mu_assigned_treatment) | mu_assigned_treatment == assigned_treatment)) %>%  
  select(est_takeup_level, model, model_name, fit_type) %>% 
...
```{r take-up-ate-dist-effect, fig.width=8,fig.height=8, fig.cap="Average treatment effect in response to a change in distance."}
dist_fit_data %>% 
  filter(!fct_match(model, "STRUCTURAL_STACKED")) %>% 
  mutate(est_takeup_te =
    map_if(est_takeup_te, 
...
```{r take-up-ate-dist-split, fig.width=8,fig.height=10, fig.cap="Average treatment effect in response to an incentive treatment intervention, split by distance."}
dist_fit_data %>% 
  filter(!fct_match(model, "STRUCTURAL_STACKED")) %>% 
  mutate(est_takeup_te =
    map_if(est_takeup_te, 
...
```{r take-up-ate-diff-dist-effect, fig.width=8,fig.height=5, fig.cap="Average difference in the average treatment effect in response to a change in assigned distance to treatment, compared against the control average treatment effect.", eval=FALSE}
dist_fit_data %>% 
  filter(!fct_match(model, "STRUCTURAL_STACKED")) %>% 
...
```{r take-up-level-struct-dist-split, fig.width=8, fig.height=9.5,fig.cap="Take-up levels in response to a signaling intervention, holding the private consumption utility at the control level."}
dist_fit_data %>%
  filter(fct_match(model_type, "structural")) %>% 
  select(est_takeup_level, model, model_name, fit_type) %>% 
  unnest(est_takeup_level) %>% 
...
```{r take-up-ate-struct-dist-split, fig.width=8, fig.height=9, fig.cap="Average treatment effect of signaling holding consumption utility at the control level, compared to control."}
dist_fit_data %>% 
  filter(fct_match(model_type, "structural")) %>% 
  select(model, model_name, est_takeup_te, fit_type) %>% 
  unnest(est_takeup_te) %>%
