...
\[ y_{im} = \mathbbm{1}(\widetilde{U}_{im} > 0) \]
where

\begin{equation}
\begin{split}
\widetilde{U}_{im} &= u_i^c(z) + u_i^n(\bar{a}_i(z, s, m), s, m) - c_i(z, d) \\
                   &{} + \int \int \left(u^h + u^s v_i + \lambda_i \omega u^r\right)\,\mathrm{d}F(u^h, u^s, \omega, u^r|\bar{a}_i(z, s, m), z, d, \theta)\,\mathrm{d}F_i(\theta) \label{eqn:structural-model}
\end{split} 
\end{equation}
...
* $\bar{a}_i(z, s, m) \in [0, 1]$, is a measure of observed peer deworming take-up.
* $u_i^c(z)$ is the private consumption value of $z$ and is known to $i$.
* $u_i^n(\bar{a}_i, s, m)$ captures the effect of deworming salience from observation and information provided by SMS, as well as the reminder/nudge effects of SMS treatment. This is also known to $i$.
...
* $\lambda_i$ is a measure of how image motivated $i$ is.
* $omega$ is a measure of the social desireability of deworming.
* $u^r \equiv \Pr_{-i}[v_i = 1|y_{im} = 1] - \Pr_i[v_i = 1|y_{im} = 0]$ is $i$'s belief about how others will infer $v_i$ based on $i$'s deworming. This is a random variable due to $i$'s uncertainty about others' inference.
* $\theta$ is a vector of parameters relevant to $i$'s updating of beliefs about $u^h$, $u^s$, $\omega$, and $u^r$.

We can view $i$'s type to be \[(\bar{a}_i(z, s, m), u^c_i(z), u_i^n(\bar{a}_i, s, m), c_i(z, d), v_i, \lambda_i, p_i(\theta)),\] where $p_i(\theta)$ is $i$'s prior on parameters $\theta$. 
