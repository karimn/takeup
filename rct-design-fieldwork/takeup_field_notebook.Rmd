...
```{r}
raw.data.path <- . %>% file.path("..", "data", "raw-data", .)
```
...
ke.lvl2.adm.data <- read_rds(file.path("..", "data", "adm", "KEN_adm2.rds"))
...
hh.census.data.pre24th <- tu.data.reader(file.path("..", "data", "census_gps_util23.csv"), 
...
hh.census.data <- tu.data.reader(raw.data.path("Census.csv"), 
  ...
  bind_rows(select(hh.census.data.pre24th, !c(hhh_name1st_check, directions))) %>% 
...
grouping.data <- plyr::adply(c(25, 50), 1, . %>% generate.grouping.data(hh.locations, .))
```
...
plyr::dlply(counted.grouping.data, .(prox.cluster.id), 
      . %>% { 
        ggplot(.) + 
...
```{r, echo=FALSE, eval=FALSE}
plyr::l_ply(grouping.plots, plot)
```
...
census.data <- list(wave.1.baseline.individuals,
  ...
  mutate(individ.baseline.sample.pool = !is.na(individual.sample.order),
         hh.baseline.sample.pool = !is.na(baseline.sample.wave))
```
...
write_rds(wave1.sms.ctrl.individuals, file.path("..", "data", "takeup_sms_ctrl_4.1_wave1.rds"))
```

```{r update-main-data-with-sms, eval=TRUE}
...
sms.treated <- read_rds(file.path("..", "data", "takeup_sms_sample_all_6.rds"))

wave1.sms.ctrl.individuals <- bind_rows(phone.owner = read_rds(file.path("..", "data", "takeup_sms_ctrl_4.1_wave1.rds")),
                                        non.phone.owner = read_rds(file.path("..", "data", "takeup_sms_ctrl_nonphone_1.0_wave1.rds")),
                                        .id = "sms.ctrl.subpop")

wave2.sms.ctrl.individuals <- read_rds(file.path("..", "data", "takeup_sms_ctrl_1.0_wave2.rds"))

combined.sms.sample <- bind_rows(sms.treated,
                                 wave1.sms.ctrl.individuals, 
                                 wave2.sms.ctrl.individuals) 

hh.census.data <- combined.sms.sample %>% 
  left_join(select(census.data, KEY.individ, KEY), "KEY.individ") %>% 
  distinct(KEY, sms.treatment, sms.ctrl.backup, sms.ctrl.subpop) %>% 
  right_join(hh.census.data, "KEY")

census.data %<>%
  right_join(combined.sms.sample, ., "KEY.individ", suffix = c(".sms.recruit", ".census")) %>% 
  mutate(phone = ifelse(nchar(phone) == 10 & str_sub(phone, 1, 1) == "0", str_sub(phone, 2), phone),
         phone = ifelse(!is.na(phone), paste0("+254", phone), NA),
         monitored = (!is.na(sms.treatment) & (sms.treatment == "sms.control" | (!is.na(sms.consent) & sms.consent))) | individ.baseline.sample.pool,
         true.monitored = (wave == 2 & monitored) | KEY.individ %in% takeup.dict$KEY.individ)
```
...
There are 29 individuals with `is.na(endline.type)` and `sms.consent == TRUE`. These are people in excluded clusters (`is.na(wave)`).
...
census.data %<>%
  ...
  mutate_at(vars(endline, endline.backup), funs(coalesce(., FALSE))) %>% 
  mutate(endline.type = case_when(is.na(.$sms.treatment) | (.$sms.treatment != "sms.control" & (is.na(.$sms.consent) | !.$sms.consent)) ~ "", # Converted to NA below
...
id.match.data <- tibble(KEY.individ = wave.1.baseline.individuals$KEY.individ,
                        min.dist = plyr::aaply(min.id.match, 1, . %>% min(na.rm = TRUE)),
                        which.min.dist = plyr::aaply(min.id.match, 1, which.min),
                        type = "names.phone") %>%
  bind_rows(tibble(KEY.individ = wave.1.baseline.individuals$KEY.individ,
                   min.dist = plyr::aaply(min.id.match[,] + min.age.dist[,], 1, . %>% min(na.rm = TRUE)), 
                   which.min.dist = plyr::aaply(min.id.match[,] + min.age.dist[,], 1, which.min), 
                   type = "age")) %>% 
...
```{r, eval=FALSE}
wtp.clusters.include <- read_lines(file.path("..", "data", "wtp_clusters_include.csv")) %>% 
  as.integer
...
wtp.sample <- hh.census.data %>% 
  ...
  filter(is.na(endline.type) | endline.type %in% c("reconsent", "reconsent.backup"), 
         is.na(assigned.treatment) | assigned.treatment == "control") %>% 
  group_by(cluster.id) %>% 
  do(sample_n(., min(25, nrow(.))) %>% 
       mutate(wtp.offer = sample(c(rep(offer.range, n() %/% 10), sample(offer.range, n() %% 10))))) %>% 
  ungroup 

write_rds(wtp.sample, file.path("..", "data", "takeup_wtp_sample_2.0.rds"))
```

```{r}
write_rds(census.data, file.path("..", "data", "takeup_census.rds"))
```
...
baseline_sample_list_0 <- read_csv(file.path("..", "data", "baseline_sample_list.csv")) 

baseline_sample_list <- baseline_sample_list_0 %>% 
  mutate(name = str_replace_all(name, "\\s+", " "),
         hhh_last_name = hhh_name %>% str_trim() %>% str_extract("\\S+$") %>% str_to_upper()) %>% 
  left_join(select(census_dict, cluster.id, village, name_census, hhh_name2nd, KEY.individ, KEY), c("name" = "name_census", "cluster_key" = "cluster.id", "village_code" = "village", "hhh_last_name" = "hhh_name2nd"))

# Up to this point, 15 with duplicate households (count(person_key) %>% filter(n > 1)) and 191 unidentified

baseline_sample_list %<>% {
  bind_rows(filter(., !is.na(KEY.individ)),
            filter(., is.na(KEY.individ)) %>%
              select(-KEY) %>% 
              left_join(distinct(census_dict, cluster.id, hhh_name2nd, KEY), c("cluster_key" = "cluster.id", "hhh_last_name" = "hhh_name2nd")))
}

# Up to this point, 31 with duplicate households (count(person_key) %>% filter(n > 1)) and 160 unidentified
...
baseline.data.w2 <- baseline.data %>% 
  filter(wave == 2) %>% 
  left_join(select(baseline_sample_list, person_key, person_id, KEY, KEY.individ), c("person" = "person_key"))

# Wave 2: 5 duplicates (KEY.survey) and 23 unidentified

baseline.data.w1 <- baseline.data %>% 
  filter(wave == 1) %>% 
  left_join(distinct(baseline_sample_list, cluster_key, village_code, KEY, hhh_last_name), 
            c("cluster.id" = "cluster_key", "village" = "village_code", "hhh_last_name" = "hhh_last_name"))

# Wave 1: 94 duplicates and 139 unidentified

baseline.data.w1.dup <- baseline.data.w1 %>% 
  filter(!is.na(KEY)) %>% 
  add_count(KEY.survey) %>% 
  filter(n > 1) %>%
  select(-c(KEY, n)) %>% 
  left_join(select(baseline_sample_list, cluster_key, village_code, name, KEY, hhh_last_name), 
            c("cluster.id" = "cluster_key", "village" = "village_code", "hhh_last_name" = "hhh_last_name")) %>% 
  mutate(namesearch_match = str_detect(name, regex(sprintf("^%s", namesearch), ignore_case = TRUE))) %>% 
  group_by(KEY.survey) %>% 
  filter(!any(namesearch_match) | namesearch_match) %>% 
  ungroup() %>%
  add_count(KEY.survey) %>% {
    bind_rows(filter(., n == 1),
              filter(., n > 1) %>% 
                select(-c(name)) %>% 
                distinct(KEY.survey, KEY, .keep_all = TRUE))
  }

# Down to 16 duplicates

baseline.data.w1.unmatched <- baseline.data.w1 %>% 
  filter(is.na(KEY))
```
...
all.sms.recruit.data <- read_csv(raw.data.path("Takeup Text Messages.csv"), 
                             col_types = list(SubmissionDate = takeup.datetime.type,
...
all.sms.recruit.data %>% 
  ...
  transmute(KEY.individ, phone, age,
            sms.consent = if_else(is.na(consent), NA, consent == 1),
            sms.treatment = sms.treatment.recruit) %>% 
  distinct(KEY.individ, sms.treatment, .keep_all = TRUE) %>%
  write_rds(file.path("..", "data", "takeup_sms_sample_all_6.rds"))
```
...
sense.mon.data <- read_csv(raw.data.path("Sensitization Monitoring Form.csv")) %>% 
  rename(cluster.id = cluster_id,
         hh.grp.key = KEY) %>% 
  right_join(read_csv(raw.data.path("Sensitization Monitoring Form-household.csv")), c("hh.grp.key" = "PARENT_KEY")) %>% 
...
takeup.data.individuals <- map_df(c(raw.data.path("Wave 1 Point of Treatment Form-ind.csv"), 
                                    raw.data.path("Kakamega Point of Treatment Form-ind.csv")), read.individ.takeup.data) 
...
all.takeup.survey.groups <- bind_rows(read.group.takeup.data(raw.data.path("Wave 1 Point of Treatment Form.csv")) %>% 
                                        mutate(data.wave = 1), 
                                      read.group.takeup.data(raw.data.path("Kakamega Point of Treatment Form.csv")) %>% 
                                        mutate(data.wave = 2))
...
all.takeup.survey.groups %<>%
  filter(is.valid.wave.datetime(SubmissionDate), 
         ...
         !is.na(cluster.id)) %>% 
  mutate(dewormed.date = date(starttime),
         deworming.day = as.integer(dewormed.date - ifelse(wave == 1, day1.wave1, day1.wave2) + 1),
...
all.takeup.survey.groups %<>%
  ...
  plyr::ddply(.(cluster.id, missing.coords),
        function(.data) {
...
takeup.data <- all.takeup.data %>% 
  ...
  plyr::ddply(.(KEY.survey.group), calc.grp.individ.deviation, .parallel = TRUE) %>% 
  plyr::ddply(.(cluster.id, missing.coords.individ), function(.data) {
...
find.name.matches <- function(.takeup.data, max.cost = 1, monitored.only = TRUE) {
    ...
    mutate(.data,
           matched.KEY.individ = adist(.data$name1st, census.data$name1st, ignore.case = TRUE) %>% 
              ...
              add(adist(.data$two_digits, census.data$two_digits) %>% 
                    ...
                    coalesce(0.0)) %>%
              plyr::alply(1, . %>% { ifelse(. <= max.cost, ., NA) %>% which.min %>% magrittr::extract(census.data$KEY.individ, .) }) %>%
              map_if(is_empty, ~ NA) %>% 
              unlist)
  }
  ...
  .takeup.data %>% 
    ...
    plyr::ddply(.(timestamp_gps.day, cluster.id), find.in.census, .parallel = TRUE) %>% 
...
```{r endline-data}
all.endline.data <- read_csv(raw.data.path("Endline Survey.csv"), col_types = list(deviceid = col_character())) %>% 
...
endline.know.table.data <- bind_rows(table.A = read_csv(file.path("..", "data", "raw-data", "Endline Survey-survey-sec_D-tableA.csv")) %>% 
                                     ...
                                     table.B = read_csv(file.path("..", "data", "raw-data", "Endline Survey-survey-sec_D-tableB.csv")) %>% 
                                       transmute(recognized = recogniseB,
                                                 dewormed = dewormedB,
...
```{r, include=FALSE}  
save(census.data, hh.census.data, cluster.strat.data, cluster.wave.county.data, pot.info, all.villages, file = "takeup_census.RData")
save(takeup.data, takeup.data.individuals, all.takeup.survey.groups, file = "takeup_at_pot.RData")
save(matched.tu.data.2, file = "takeup_matched_population.RData")
```