...
2. _Social vs. no social incentives treatment for text message treatment individuals_ 
    ...
    (i) Comparing 3 vs. 5 we identify the effect of signaling using ink 
    (ii) Comparing 9 vs. 7 the effect of social signaling through bracelets 
    
    A comparison only among text message recipients allows us to control for the reminder and social learning effects of ink or bracelets and separately identify signaling. 

3. _Text message treatment with social information vs. no text messages for social incentive individuals_ 

    Comparing 4 vs. 5 and 6 vs. 7 we identify the additional reminder and social learning effects of text messages in the presence of social incentives. The cross-treatments identify how "satiated" individuals are in terms of being reminded or informed about other people's decision through signals. If we find no significant differences, reminders or information about others' choices play either no role for individual decision-making or the ink and bracelet incentives already acted as strong reminders and ways to observe others' actions. To distinguish the two we look at the effects of text messages among no incentive individuals. 

4. _Text message reminders and social information treatments vs. no text messages for no incentive individuals_

    Comparing 1 vs. 2 and 2 vs. 3 allows us to identify the effect of reminders and information about others' treatment decisions on deworming take-up. The text message treatments within no incentive clusters allow us to identify whether reminders or social learning are relevant in the context of deworming take-up decisions. 
...
* $\mathbf{X}_{ij}$ individual and cluster level pre-treatment characteristics. While we will omit this vector from most of our notation, we will sometimes need to refer to one variable in that vector
  - $P_{ij} \in \{0, 1\}$, indicating phone ownership.
  
We describe the set of sampled and monitored individuals as $\mathcal{W}$ and the set of phone owners as $\mathcal{Q}(q) = \{i: P_{ij} = q\}$.
...
Whenever we reference any of this potential outcomes using one input, this refers to having $m = none$ (e.g., $S^*_{ij}(z) \equiv S^*_{ij}(z, none)$).
...
\[ H_0: \tau[ink, control] = 0 \land \tau[bracelet, control] = 0 \] 
and the private value only incentive by testing
\[ H_0: \tau[calendar, control] = 0 \] 
...
\[ H_0: \tau[(ink, socialinfo), (control, socialinfo)] = 0 \]
(ii) Comparing 9 vs. 7 the effect of social signaling through bracelets 
\[ H_0: \tau[(bracelets, socialinfo), (calendar, socialinfo)] = 0 \]
...
\[ H_0: \tau[(ink, socialinfo), (ink, none)] = 0 \land \tau[(bracelet, socialinfo), (bracelet, none)] = 0 \]
as well as separately
\[ H_0: \tau[(ink, socialinfo), (ink, none)] = 0 \]
and
\[ H_0: \tau[(bracelet, socialinfo), (bracelet, none)] = 0 \]
...
\[ H_0: \tau[(control, reminders), (control, none)] = 0 \]
to see whether reminders have an effect on deworming take-up, and test
\[ H_0: \tau[(control, socialinfo), (control, reminders)] = 0 \]
