...
```{r info-change-ate}
info_inc_ate_iter_data <- est_deworming_takeup_ate %>% 
...
info_inc_ate_iter_data_phone <- est_deworming_takeup_ate %>% 
  filter(dist.pot.group_static_left != dist.pot.group_dynamic_left | dist.pot.group_static_right != dist.pot.group_dynamic_right,
         dist.pot.group_static_right == dist.pot.group_dynamic_right, 
         dist.pot.group_dynamic_right == "far", 
         dist.pot.group_static_left == dist.pot.group_static_right) %>% 
  ...
  takeup_summ_data %>%
    inner_data_preparer() %>% { 
      ...
      if (!is_null(takeup_data)) {
        plot_obj <- plot_obj + geom_violin(aes(y = wtd_iter_est), draw_quantiles = c(0.25, 0.5, 0.75), color = "white", fill = "darkgrey", data = takeup_data) 
      }
...
plot_dyn_ate <- function(ate_summ_data, ate_data = NULL, combiner = NULL, data_preparer = function(data) data) {
  ...
  ate_summ_data %>% 
    inner_data_preparer() %>% { 
      ...
      if (!is_null(ate_data)) {
        plot_obj <- plot_obj + geom_violin(aes(y = wtd_iter_est), draw_quantiles = c(0.25, 0.5, 0.75), color = "white", fill = "darkgrey", data = ate_data)
      }
