...
```{r}
save(
  dyn_treatment_cell_info,
  dyn_ate_pair_info,
  est_deworming_takeup_dyn,
  est_deworming_days,
  est_deworming_takeup_dyn_ate,
  est_deworming_takeup_dyn_phone,
  ...
  file = file.path("stan_analysis_data", "dynamic_processed.RData")
)
```

```{r}
model_3_fit %>% 
  tidy(pars = c(str_c(c("stratum", "cluster"), "_student_df"), "stratum_baseline_dyn_student_df", "stratum_dyn_student_df"))
```
...
```{r dyn_level_smsctrl, fig.height=3, fig.width=8}
plot_takeup(
...
```{r param-dyn-ate-all, fig.width=10, fig.height=8}
est_deworming_takeup_dyn_ate_all %>% 
  plot_ate(
    ate_summ_data = .,
    ...
    data_preparer = . %>% 
      filter(incentive_treatment_static_left == incentive_treatment_dynamic_left,
         incentive_treatment_static_right == incentive_treatment_dynamic_right,
         sms.treatment.2_static_left == sms.treatment.2_dynamic_left,
...
# est_deworming_takeup_dyn_ate %>% 
  filter(incentive_treatment_static_left != "bracelet social",
         !(incentive_treatment_static_left == "bracelet" & incentive_treatment_static_right == "calendar")) %>% 
      # filter(incentive_treatment_static_right %in% c("control", "calendar"),
      #        incentive_treatment_static_left != incentive_treatment_static_right,
      #        incentive_treatment_dynamic_left != incentive_treatment_dynamic_right,
      #        sms.treatment.2_static_left == "control",
      #        sms.treatment.2_static_left == sms.treatment.2_static_right)
  plot_ate(
    ate_summ_data = .,
      data_preparer = . %>% 
      filter(incentive_treatment_static_right %in% c("Control", "Calendar"),
...
```{r param-dyn-daily-takeup-levels-with-sms, fig.width=12, fig.height=4}
est_deworming_day_all %>% 
  mutate(full_observation = incentive_treatment_static == incentive_treatment_dynamic) %>% 
  filter(full_observation,
         incentive_treatment_static != "bracelet social",
...
ggsave(filename = "dyn_daily_all_withsms.png", path = "march_2018_plots", 
       width = 9, height = 4)
```
...
ggsave(filename = "dyn_daily_all_dist.png", path = plot_save_path,
       width = 8.5, height = 6)
```

```{r param-dyn-daily-takeup-levels-dist-with-sms, fig.width=12, fig.height=8}
est_deworming_day_dist %>% 
  mutate(full_observation = incentive_treatment_static == incentive_treatment_dynamic) %>% 
  filter(full_observation,
         incentive_treatment_static != "bracelet social",
...
ggsave(filename = "dyn_daily_sms_dist.png", path = plot_save_path,
       width = 8.5, height = 6)
```
