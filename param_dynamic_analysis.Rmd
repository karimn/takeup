...
```{r param-dyn-load-bayes-data, include=FALSE}
...
tidy(model_3_fit, pars = "hyper_beta_day1") %>%
  pull(estimate) %>%
  multiply_by_matrix(as.matrix(param_dyn_stan_data$treatment_map_design_matrix), .) %>% 
  mutate(param_dyn_stan_data$treatment_map, latent_var = .) %>% 
