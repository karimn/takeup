...
# Within SLURM tasks
srun --export=all --exclusive --ntasks=1 bash -c "postprocess_model quick_ate_postprocess.R ${MODEL} ${VERSION} ${POSTPROCESS_INOUT_ARGS}" &
srun --export=all --exclusive --ntasks=1 bash -c "postprocess_model quick_roc_postprocess.R ${MODEL} ${VERSION} ${POSTPROCESS_INOUT_ARGS}" &
srun --export=all --exclusive --ntasks=1 bash -c "postprocess_model quick_submodel_postprocess.R ${MODEL} ${VERSION} ${POSTPROCESS_INOUT_ARGS}" &
wait
