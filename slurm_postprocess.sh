...
# Within SLURM tasks
srun --export=all --exclusive --ntasks=1 bash -c "postprocess_model quick_ate_postprocess.R ${VERSION} ${POSTPROCESS_INOUT_ARGS} ${MODEL}" &
srun --export=all --exclusive --ntasks=1 bash -c "postprocess_model quick_roc_postprocess.R ${VERSION} ${POSTPROCESS_INOUT_ARGS} ${MODEL}" &
srun --export=all --exclusive --ntasks=1 bash -c "postprocess_model quick_submodel_postprocess.R ${VERSION} ${POSTPROCESS_INOUT_ARGS} ${MODEL}" &
wait
