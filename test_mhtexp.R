...
test_data <- read_csv("~/Code/dewormtheworld-private/takeup/data.csv") %>%
  transmute(amount, treatment = factor(ratio, levels = 0:3), dummy_stratum = factor(1), cluster = 1:n())

test_hypo_mat <- diag(3) %>%
  cbind(1:3, 0) %>%
  set_colnames(c("treatment[T.1]", "treatment[T.2]", "treatment[T.3]", "left", "right"))

test_est <- test_data %>%
  run_strat_reg(amount ~ treatment, .cluster = "cluster", .strat.by = NULL) %>%
  # predict(test_hypo_mat)
  linear_tester(test_hypo_mat[, 1:3])
...
test_data %>%
  strat_mht(amount ~ treatment, cluster = "cluster", strat_by = NULL, covar = NULL,
            hypotheses = test_hypo_mat, exploit_transitivity = TRUE, ate_pair_cols = c("left", "right"),
            num_resample = 10)
