...
discrete_dist_balance_pval = map(
  disc_dist_balance, 
  ~car::lht(.x, construct_joint_test_m(.x))
  )  %>%
  map("Pr(>Chisq)") %>%
  map_dbl(2)
...
fully_cts_dist_balance %>%
  map_dfr(tidy, conf.int = TRUE, .id = "lhs") %>%
  filter(term == "dist_measure")  %>%
  ggplot(aes(
    x = estimate,
    xmin = conf.low,
    xmax = conf.high, 
...
stan_data$num_recognized %>% length()
