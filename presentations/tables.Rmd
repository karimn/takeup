...
endline_data %>%
  ...
  lm(
    data = .,
    value ~ assigned_treatment + variable
  ) %>%
  tidy(conf.int = TRUE)
...
endline_data %>%
  filter(assigned_treatment == 3) %>%
  mutate(
    means_dewormed = str_detect(str_to_lower(cal_meaning), "dewom|derwom|pill|medicine|drug|deworm|worm|treat"), 
  ) %>%
  filter(means_dewormed == FALSE) %>%
  select(cal_meaning) %>%
  group_by(cal_meaning) %>%
  count() %>%
  arrange(-n) %>%
  print(n = 80)
...
endline_data %>%
  filter(assigned_treatment == 4) %>%
  mutate(
    means_dewormed = str_detect(bracelet_meaning, "deworm|Deworm|worm|treat"), 
  ) %>%
  filter(means_dewormed == FALSE) %>%
  select(bracelet_meaning) %>%
  group_by(bracelet_meaning) %>%
  count() %>%
  arrange(-n) %>%
  print(n = 60)
