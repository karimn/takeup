...
baseline.data %>%
  select(why_yes_few_deworm, why_yes_many_deworm) %>%
  filter(why_yes_few_deworm != why_yes_many_deworm)

baseline.data %>%
  mutate(
    few_yes = few_deworm == "yes", 
    many_yes = many_deworm == "yes"
  ) %>%
  feols(
    data = .,
    .[c("few_yes", "many_yes")] ~ dist.pot.group
  ) %>%
  etable()

baseline.data %>%
  select(contains("dworm"), assigned.treatment, dist.pot.group)  %>%
  feols(
    data = .,
    ink_dworm_rate ~ dist.pot.group
  ) %>%
  etable()
...
dworm_endline_fit = endline_data %>%
  ...
  feols(
    dworm_rate ~ 0 + assigned_treatment:dist_pot_group + dist_to_pot
  ) 
...
bind_rows(
  ...
  bind_rows(
    ...
    tidy() %>%
    mutate(term = "Combined", treatment = "control")
  ) %>%
  select(term, treatment, estimate, std.error, p.value) 





dworm_endline_comp_tbl = dworm_endline_comp_df   %>%
  mutate(
  ...
  mutate(treatment = factor(treatment, levels = c("Control mean", str_to_title(treat_levels_c))) %>% fct_rev) %>%
  spread(
    term, 
    estim_std
  )  %>%
...
# tidy_dworm_endline_fit = dworm_endline_fit %>%
#   tidy(conf.int = TRUE)  %>%
#   filter(term != "dist_to_pot") %>%
#   mutate(
#     dist_pot_group = if_else(
#       str_detect(
#         term, 
#         "far"
#         ), 
...
#     treatment = str_extract(
#       term, 
#       "(?<=assigned_treatment)\\w+"
#     )
#   )  %>%
#   mutate(
#     treatment = factor(
#       treatment, 
#       levels = treat_levels_c
#     ), 
...
optim_summ_robust_tbl = optim_summ_input_df %>%
  ...
  pack_rows(
    index = c(
      "Panel A: Random allocation" = 1, 
      "Panel B: Optimal allocation, 2.5km" = 4,
      "Panel B: Optimal allocation, 3.5km" = 4,
      "Panel B: Optimal allocation, 4.5km" = 4,
      "Panel B: Optimal allocation, 5.5km" = 4,
      "Panel B: Optimal allocation, 10km" = 4
    ), 
    italic = TRUE,
    escape = FALSE,
    # latex_gap_space = latex_group_gap_space, 
