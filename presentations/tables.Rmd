...
te_df = bind_rows(
  ...
  mutate(
    assigned_treatment = if_else(
      assigned_treatment == "Pr(Bracelet > Calendar)", 
      "$Pr(\\textit{Bracelet} > \\textit{Calendar})$", 
      assigned_treatment
      ),
    assigned_treatment = factor(
      assigned_treatment, 
      levels = c("Bracelet", "Calendar", "Ink", "Control", "$Pr(\\textit{Bracelet} > \\textit{Calendar})$"))
  )
...
te_kbl_df = te_input_df %>%
  ...
  kbl(
    col.names = c(
      # "Estimand", 
      # "Treatment", 
      "\\textit{Dependent Variable: takeup}",
      paste0("(", 1:6, ")")
    ), 
    format = "latex", 
  ...
  pack_rows(
    index = c(
      "Panel A: \\textit{Overall}" = 4, 
      "Panel B: \\textit{Signalling}" = 4, 
      "Panel C: \\textit{Private}" = 3, 
      "Control Mean" = 1
    ), 
    escape = FALSE,
    # latex_gap_space = latex_group_gap_space, 
...
overall_te_kbl_df = te_input_df %>%
  filter(Estimand == "incentive" | Treatment == "Control") %>%
  arrange(Estimand, Treatment) %>%
  mutate(
    Estimand = if_else(
      Treatment == "Ink", 
...
decomposed_te_kbl_df = te_input_df %>%
  select(-contains("rf_")) %>%
  filter(Estimand == "private" | Estimand == "signal" | Treatment == "Control") %>%
  arrange(Estimand, Treatment) %>%
  mutate(
    Estimand = if_else(
      Treatment == "Ink", 
  ...
  pack_rows(
    index = c(
      "Panel A: \\textit{Signal}" = 4, 
      "Panel B: \\textit{Private}" = 3, 
      "Control Mean" = 1
    ), 
    escape = FALSE,
    # latex_gap_space = latex_group_gap_space, 
