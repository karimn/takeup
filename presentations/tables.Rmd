...
comparison_te_df = full_dist_fit_data %>%
  filter(fit_type == "fit") %>%
  select(model_type, fit_type, est_takeup_te) %>%
  unnest(est_takeup_te)  %>%
  group_by(
  ...
  select(mean_est, per_0.05, per_0.95) %>%
  ungroup()
...
full_dist_fit_data %>%
  ...
  select(model_type, assigned_treatment_left, iter_id, far_close_diff) %>%
  spread(assigned_treatment_left, far_close_diff)  %>%
  mutate(
...
incentive_comp_far_bigger_te = full_dist_fit_data %>%
  filter(fct_match(fit_type, "fit")) %>%
  select(
    model_type,
    est_takeup_te
...
incentive_bracelet_calendar_diff_df = pr_comparison_te_df %>%
    ...
    mutate(
      assigned_treatment = "$\\hat{\\pi}$(Bracelet > Calendar)"
    )

incentive_bracelet_calendar_comp_diff_df = full_dist_fit_data %>%
  filter(fit_type == "fit") %>%
  mutate(
    est_takeup_te = map2(
      est_takeup_te,
      ...
      ~filter(.x,
        (assigned_dist_group_left == assigned_dist_group_right) | (is.na(assigned_dist_group_left) & is.na(assigned_dist_group_right)), # same dist group
        .y == "reduced form" | (.y == "structural" & (mu_assigned_treatment_left == assigned_treatment_left)), # same mu and B
        .y == "reduced form" | (.y == "structural" & (mu_assigned_treatment_right == assigned_treatment_right)), # same mu and B
        fct_match(assigned_treatment_right, "calendar"), 
        fct_match(assigned_treatment_left, "bracelet")
      ) %>%
      mutate(
  ...
  select(
    ...
    model_type,
    assigned_treatment = assigned_treatment_left,
    assigned_dist_group = assigned_dist_group_left,
    mean_est, 
    per_0.05, 
    per_0.95
  ) 

signal_bracelet_calendar_comp_diff_df = comparison_te_df %>%
  filter(
    (assigned_dist_group_left == assigned_dist_group_right) | (is.na(assigned_dist_group_left) & is.na(assigned_dist_group_right)), # same dist group
...
te_df = bind_rows(
  ...
  mutate(
    ...
    assigned_treatment = factor(
      assigned_treatment, 
      levels = c("Bracelet", "Calendar", "Ink", "Control mean", "$\\hat{\\pi}$(Bracelet > Calendar)"))
  )
...
```{r}
#| overall-table-only
te_far_bigger_input_df = incentive_far_bigger_te_df %>%
  mutate(
  ...
  spread(
    model_type, pr_far_bigger
  ) %>%
  rename(
    rf_pr_bigger = `reduced form`,
    pr_bigger = structural
  )
...
overall_te_kbl_df = 
  ...
  left_join(
    te_far_bigger_input_df %>% 
      mutate(Treatment = factor(Treatment), Estimand = factor(Estimand)) %>%
      mutate(across(where(is.numeric), round, 3)),
