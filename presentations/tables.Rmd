...
if (interactive()) {
    params = lst(
        ...
        optim_input_path =  "optim/data/STRUCTURAL_LINEAR_U_SHOCKS_PHAT_MU_REP/agg-full-many-pots",
        table_output_path = "presentations/tables",
        struct_models = "STRUCTURAL_LINEAR_U_SHOCKS_PHAT_MU_REP",
        rf_models = "REDUCED_FORM_NO_RESTRICT",
        show_probs = FALSE,
        cache = TRUE
    )
...
if (params$fit_version != params$rf_fit_version) {
  ...
  dist_fit_data = dist_fit_data %>%
    filter(model %in% c(params$struct_models))
  ...
  rf_fit_data = with_env(load_rf_function, rf_env)() %>%
    filter(model %in% c(params$rf_models))
...
dist_fit_data = dist_fit_data %>%
  filter(model %in% models_we_want)
...
load_full_data_function = function(){
    load(file.path("temp-data", str_interp("processed_dist_fit${params$rf_fit_version}.RData"))) 
    rf_dist_fit_data = dist_fit_data %>%
      filter(model %in% params$rf_models)
    load(file.path("temp-data", str_interp("processed_dist_fit${params$fit_version}.RData"))) 
    struct_dist_fit_data = dist_fit_data %>%  
      filter(model %in% params$struct_models)
    dist_fit_data = bind_rows(
      rf_dist_fit_data,
      struct_dist_fit_data
    )
    return(dist_fit_data)
}
...
#' Prepare Incentive TE Data
#' 
#' Filtering so that we have:
#' visibility matching private benefit (if structural)
#' comparison within same distance group
#' control on RHS
#' Bracelet/calendar/ink on lhs
prep_incentive_te = function(takeup_te_data, model_type) {
  if (fct_match(model_type, "structural")) {
    takeup_te_data = takeup_te_data %>%
      filter(
        # make sure visibility and private benefit match
        mu_assigned_treatment_left == assigned_treatment_left,
        mu_assigned_treatment_right == assigned_treatment_right
      )
  }
  output_te = takeup_te_data %>%
    filter(
      # comparing within same distance group (NA corresponds to reduced form model)
      (is.na(assigned_dist_group_left) & is.na(assigned_dist_group_right)) | (assigned_dist_group_left == assigned_dist_group_right),
      # rhs is control, lhs isn't same as rhs 
      ...
      mutate(
        ...
        assigned_dist_group_right = fct_explicit_na(assigned_dist_group_right, "combined")  %>%
          fct_relabel(str_to_title) %>%
          fct_relevel("Combined")
        )
  return(output_te)
...
#' Prepare Signal TE Data
#' 
#' Filtering so that we have:
#' comparison within same distance group
#' control private benefit on RHS & LHS
#' Bracelet/calendar/ink visibility on lhs
#' Control visibility on RHS
prep_signal_te = function(takeup_te_data, model_type, fit_type) {
  if (fct_match(fit_type, "fit") & fct_match(model_type, "structural")) {
    output_te = takeup_te_data %>%
      filter(
      # comparing within same distance group (NA corresponds to reduced form model)
      (is.na(assigned_dist_group_left) & is.na(assigned_dist_group_right)) | (assigned_dist_group_left == assigned_dist_group_right),
      # ensure private benefit set to control
      across(c(assigned_treatment_left, assigned_treatment_right), fct_match, "control"),
      # visibility LHS has to exist
...
#' Prepare Private Incentive TE Data
#' 
#' Filtering so that we have:
#' comparison within same distance group
#' control visibility on RHS & LHS
#' Bracelet/calendar/ink private benefit on lhs
#' Control private benefit on RHS
prep_private_te = function(takeup_te_data, model_type, fit_type) {
  if (fct_match(fit_type, "fit") & fct_match(model_type, "structural")) {
    output_te = takeup_te_data %>%
      filter(
      # same distance comparison
      (is.na(assigned_dist_group_left) & is.na(assigned_dist_group_right)) | (assigned_dist_group_left == assigned_dist_group_right),
      # visibility not NA (probably don't need if matching on struct)
      !is.na(mu_assigned_treatment_left),
      !is.na(mu_assigned_treatment_right),
      # visibility control on LHS & RHS
      across(c(mu_assigned_treatment_left, mu_assigned_treatment_right), fct_match, "control"),
      # control private benefit RHS
      fct_match(assigned_treatment_right, "control"),
      # LHS != RHS private benefit
      assigned_treatment_right != assigned_treatment_left
      )   %>%
      mutate(
        assigned_dist_group_left = fct_explicit_na(assigned_dist_group_left, "combined") %>%
          fct_relabel(str_to_title) %>%
...
calculate_bra_cal_fc_diff = function(data, lhs_treat) {
data %>%
  unnest(est_takeup_te) %>%
  ...
  mutate(
    far_close_diff = Far - Close
  ) %>%
  select(model_type, {{ lhs_treat }}, iter_id, far_close_diff) %>%
  spread({{ lhs_treat }}, far_close_diff)  %>%
  mutate(
    bra_fc_minus_cal_fc = Bracelet - Calendar
  ) %>%
  group_by(
    model_type
  ) %>%
  summarise(
    pr_positive = mean(bra_fc_minus_cal_fc > 0),
    mean_diff = mean(bra_fc_minus_cal_fc),
    median_diff = median(bra_fc_minus_cal_fc),
    per_0.05 = quantile(bra_fc_minus_cal_fc, 0.05),
    per_0.95 = quantile(bra_fc_minus_cal_fc, 0.95)
  ) 
}



incentive_bra_cal_fc_df = full_dist_fit_data %>%
  filter(fct_match(fit_type, "fit")) %>%
  select(
    model_type,
    est_takeup_te
  ...
  mutate(
    est_takeup_te = map2(est_takeup_te, model_type, prep_incentive_te)
  ) %>%
  calculate_bra_cal_fc_diff(assigned_treatment_left)

  
signal_bra_cal_fc_df = full_dist_fit_data %>%
  filter(fct_match(fit_type, "fit")) %>%
  select(
    model_type,
    fit_type,
  ...
  mutate(
    est_takeup_te = pmap(
      list(est_takeup_te, model_type, fit_type),
      ~prep_signal_te(..1, ..2, ..3)
      )
    )  %>%
  calculate_bra_cal_fc_diff(mu_assigned_treatment_left)

private_bra_cal_fc_df = full_dist_fit_data %>%
  filter(fct_match(fit_type, "fit")) %>%
  select(
    model_type,
    fit_type,
...
bra_cal_fc_df = bind_rows(
  incentive_bra_cal_fc_df %>%
    mutate(Estimand = "incentive"),
  signal_bra_cal_fc_df %>%
    mutate(Estimand = "signal")
  # private_bra_cal_fc_df %>%
  ...
  mutate(
    across(c(mean_diff, per_0.05, per_0.95), round, digits),
    estim_std = if_else(
      rep(!params$show_probs, n()),
      linebreak(paste0(mean_diff,"\n", str_glue("({per_0.05}, {per_0.95})")), align = "c"),
      as.character(round(pr_positive, digits))
    )
  ) %>%
  select(
    model_type,
  ...
  spread(
    model_type, estim_std
  ) %>%
  rename(
    rf_estim = `reduced form`,
    estim = structural
  )



calculate_far_bigger_te = function(data, lhs_treat) {
  data %>%
    unnest(est_takeup_te) %>%
    filter(assigned_dist_group_left != "Combined")  %>%
    unnest(iter_data) %>%
    ...
    summarise(
      mean_est = mean(Far - Close),
      per_0.05 = quantile(Far - Close, 0.05),
      per_0.95 = quantile(Far - Close, 0.95),
      pr_far_bigger = mean(Far > Close)
    ) 
}

incentive_far_bigger_te_df = full_dist_fit_data %>%
    filter(fct_match(fit_type, "fit")) %>%
    select(
      model_type,
      est_takeup_te
    ...
    calculate_far_bigger_te(assigned_treatment_left) %>%
    mutate(Estimand = "incentive")


signal_far_bigger_te_df = full_dist_fit_data %>%
  ...
  mutate(
    ...
    calculate_far_bigger_te(mu_assigned_treatment_left) %>%
    mutate(Estimand = "signal")


private_far_bigger_te_df = full_dist_fit_data %>%
  filter(fit_type == "fit" & model_type == "structural") %>%
  select(model, model_type, model_name, est_takeup_te, fit_type) %>% 
  mutate(
    est_takeup_te = pmap(
...
signalling_te = dist_fit_data %>% 
  ...
  mutate(
    ...
    filter(map_lgl(est_takeup_te, ~!is.null(.x))) %>%
    unnest(est_takeup_te) %>%
    select(
...
incentive_bracelet_calendar_diff_df = pr_comparison_te_df %>%
    ...
    mutate(
      assigned_treatment_pr = "P(Bracelet > Calendar)",
      assigned_treatment = "Bracelet - Calendar"
    )


private_bracelet_calendar_diff_df = pr_comparison_te_df %>%
...
fp = str_glue("temp-data/temp-res-{params$rf_fit_version}")
dir.create(fp)
...
incentive_bracelet_calendar_diff_df %>%
  mutate(across(where(is.numeric), round, 3)) %>%
  write_csv(
    file.path(
      fp,
...
te_df = bind_rows(
  ...
  private_bracelet_calendar_diff_df %>% mutate(estimand = "private", is_diff = TRUE),
  incentive_control_mean %>% mutate(estimand = "control mean")
) %>%
  mutate(
    assigned_treatment = if_else(
      assigned_treatment == "control" | assigned_treatment == "Control", 
      "Control mean", 
      assigned_treatment
    ),
...
rf_te_input_df = te_df %>%
  ...
  mutate(
    estim_value = if_else(
      (is_diff == FALSE | is.na(is_diff)) | !params$show_probs,
      linebreak(
        paste0(mean_est, "\n", "(", per_0.05, ", ", per_0.95, ")"), 
        align = "c"
        ), 
    ...
    assigned_treatment = if_else(
      params$show_probs == TRUE & !is.na(assigned_treatment_pr),
      fct_recode(assigned_treatment, "Bracelet - Calendar" = "P(Bracelet > Calendar)"),
      assigned_treatment
    )
  ) %>%
  select(
...
te_far_bigger_input_df = 
  bind_rows(
    incentive_far_bigger_te_df,
    signal_far_bigger_te_df
  ) %>%
  mutate(
    across(c(mean_est, per_0.05, per_0.95), round, digits),
    Treatment = case_when(
      Estimand == "incentive" ~ assigned_treatment_left,
      Estimand == "private" ~ assigned_treatment_left,
      Estimand == "signal" ~ mu_assigned_treatment_left
    ),
    estim_std = if_else(
      rep(!params$show_probs, n()),
      linebreak(paste0(mean_est,"\n", str_glue("({per_0.05}, {per_0.95})")), align = "c"),
      as.character(round(pr_far_bigger, digits))
    )
  ) %>%
  select(
    model_type,
...
overall_te_kbl_df = 
  ...
  add_header_above(
    c(" ", 
      ...
      if_else(
        params$show_probs,
        "$\\\\hat{\\\\pi}$($\\\\textrm{TE}_{\\\\text{Far}} > \\\\textrm{TE}_{\\\\text{Close}}$)",
        "Far - Close"
      ),
      "Combined", 
      "Close", 
