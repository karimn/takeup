...
car::linearHypothesis(indiv_balance_fit[[1]], hyp_matrix)$"Pr(>Chisq)"
...
knitr::kable(
    format = "latex",
    escape = FALSE, 
    col.names = c("", rep(str_to_title(treat_levels_c), 2), "F-test $p$-value"), 
    booktabs = TRUE, 
...
sample_meta_df = tibble(
  sample = c("Takeup", "WTP", "Beliefs"),
  n_indiv = c(stan_data$num_obs, stan_data$num_wtp_obs, stan_data$num_beliefs_obs)
)
...
sample_meta_tbl = sample_meta_df %>%
    kbl(
        ...
        align = "lcc",
        col.names = c("Experiment", "N Individuals")
    )  %>%
    kable_styling()
