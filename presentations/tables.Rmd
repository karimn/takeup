...
endline_and_baseline_data = bind_rows(
  endline_balance_data %>%
    select(treat_dist, neighbours_worm_knowledge = endline_neighbours_worm_knowledge, constituency, county) %>%
    mutate(
      type = "endline"
    ),
  baseline_balance_data %>%
    select(treat_dist, neighbours_worm_knowledge = baseline_neighbours_worm_knowledge, constituency, county) %>%
    mutate(
      type = "baseline"
    )
) %>%
...
balance_fits = c(
  ...
  list("lhs: endline_neighbours_worm_knowledge" = endline_balance_fit)
)
...
baseline_endline_externality_fit = feols(
      ...
      neighbours_worm_knowledge ~ 0 + treat_dist:type, 
      ~county
      ) 
perform_externality_cluster_boot = function(data, R) {
  subset_data = data %>%
    select(neighbours_worm_knowledge, treat_dist, type, constituency, county ) %>%
    na.omit()

  fit = lm(
    formula = neighbours_worm_knowledge ~ 0 + treat_dist:type, 
    data = subset_data
  )
...
indiv_externality_comp_test = comparisons(
  baseline_endline_externality_fit, 
...
indiv_externality_comp_df = tibble(
  treat_dist = indiv_externality_comp_test$treat_dist, 
  p.value = indiv_externality_comp_test$p.value
)

indiv_externality_comp_df
...
externality_joint_hyp_matrix = map2(
  ...
  ~generate_joint_externality_hyp_m(fit = baseline_endline_externality_fit, .x, .y )
) %>%
  do.call(rbind, .)


joint_externality_p_value = perform_externality_cluster_boot(
  endline_and_baseline_data,
  externality_joint_hyp_matrix
)

indiv_close_externality_p_value = map(
    c(split(externality_joint_hyp_matrix[1:4, ], 1:4), list(externality_joint_hyp_matrix[1:4, ])),
    ~perform_externality_cluster_boot(endline_and_baseline_data, .x)
)

indiv_far_externality_p_value = map(
...
externality_p = c(
  indiv_close_externality_p_value %>% unlist(), 
  indiv_far_externality_p_value %>% unlist(), 
  joint_externality_p_value
)

externality_hyp_row = dist_treat_grid %>%
  add_row(treat = NA, dist = "close_joint_p", .after = 4) %>%
  add_row(treat = NA, dist = "far_joint_p", .after = 9) %>%
  add_row(treat = NA, dist = "joint_p", .after = 10) %>%
  mutate(p.value = externality_p)  %>%
  mutate(
    p.value = round(p.value, 3),
  ) %>%
  mutate(
  p.value = p.value %>% as.character()
  ) %>%
  pivot_wider(
    names_from = c(treat, dist), 
    values_from = p.value
  ) %>%
  mutate(
    lhs = "No change in externality knowledge, $p$-value"
  )  %>%
  mutate(across(contains("joint_p"), as.numeric))

colnames(externality_hyp_row) = str_remove(colnames(externality_hyp_row), "NA_")
...
footnote(general = "Insert footnote here.") %>%
  pack_rows(
    index = c(
      "Individual-level Covariates" = 13, 
...
summ_optim_df = all_optim_df %>%
        ...
        filter(
            model %in% 'STRUCTURAL_LINEAR_U_SHOCKS'
        ) %>%
        filter(
            cutoff_type  == "cutoff"
        ) %>%
