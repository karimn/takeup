...
dist_fit_data %>%
  ...
  first() %>%
  plot_beliefs_est(width = 0.7, crossbar_width = 0.4, include = "1ord")
...
dist_fit_data %>% 
  ...
  transmute(across(c(cluster_takeup_prop_control, cluster_takeup_prop_bracelet), map, ~ mutate(.x, roc_distance = roc_distance / 1000))) %>% 
...
```{r rf-load-processed-dist-fit, dependson="rf-version", eval=FALSE}
load(file.path("..", "temp-data", str_interp("processed_dist_fit${rf_fit_version}.RData")))
...
```{r rf-incentive-te, fig.width=7.5, fig.height=4, warning=FALSE, dependson="rf-version", eval=FALSE}
dist_fit_data %>%
  filter(fct_match(fit_type, "fit")) %>% 
  mutate(est_takeup_te =
    map(est_takeup_te, 
  ...
  mutate(
    ...
    cowplot::plot_grid(
      plot_estimands(., assigned_treatment_left, results_group = fit_type, group_labels = c(fit = "Posterior", "prior-predict" = "Prior")) +
        ...
        scale_y_discrete("") +
        labs(
          title = "Reduced-Form Average Treatment Effect"
        ) +
        ggforce::facet_col(vars(assigned_dist_group_left), 
                   space = "free",
                   scales = "free_y") +
...
<!-- ## Reduced-Form Take-up Level -->

```{r rf-levels, fig.width=7.5, fig.height=4.25, warning=FALSE, dependson="rf-version", eval=FALSE}
dist_fit_data %>%
  filter(fct_match(fit_type, "fit")) %>% 
  select(model, model_name, est_takeup_level, fit_type) %>% 
  unnest(est_takeup_level) %>%
  mutate(
    ...
    cowplot::plot_grid(
      plot_estimands(., assigned_treatment, results_group = fit_type, group_labels = c(fit = "Posterior", "prior-predict" = "Prior")) +
        ...
        scale_y_discrete("") +
        labs(
          title = "Reduced-Form Take-up Level"
        ) +
        ggforce::facet_col(vars(assigned_dist_group), 
                   space = "free",
                   scales = "free_y") +
