...
knitr::opts_chunk$set(
  ...
  warning = FALSE, 
  cache = TRUE, 
  cache.path = str_glue("takeup-{params$models}{params$fit_version}-figure-cache/"), 
  fig.path = str_glue("takeup-{params$models}{params$fit_version}-fig/"), 
  fig.align = "center"
  )
...
if (params$component_checks) {
  component_files = map(
      ...
      ~file.path(
        ...
        "data",
        str_glue(
          "pred-social-multiplier-fit{params$fit_version}-cutoff-b-{.x}-mu-{.x}-{params$models}.csv"
        )
        )
      ) %>%
      unlist()
...
2+2
  raw_u_sd_alpha = 4
  raw_u_sd_beta = 1
  inv_raw_u_sd = rgamma(100000, raw_u_sd_alpha, raw_u_sd_beta)
  raw_u_sd = 1 / inv_raw_u_sd
  library(tidyverse)
  tibble(
    x = raw_u_sd
  )  %>%
...
if (params$fit_version != params$rf_fit_version) {
  ...
  rf_fit_data = with_env(load_rf_function, rf_env)() %>%
    filter(model %in% c("REDUCED_FORM_NO_RESTRICT_DIST_CTS"))
...
dist_fit_data = dist_fit_data %>%
  filter(model %in% c("REDUCED_FORM_NO_RESTRICT_DIST_CTS", models_we_want))
...
structural_prior_present = dist_fit_data %>%
  filter(
...
# mu = mu_0 exp(linear predictor)
# mu = \hat{p}_{z,d} x lambda 

# mu = (\hat{p}_{z,d})/(1 - \hat{p}_{z,d}) x lambda 
...
overall_pref_gift_df %>%                                            
  filter(gift_choice != "Neither") %>%
  filter(dewormed == FALSE) %>%
  filter(str_detect(assigned.treatment, "Calendar|Bracelet")) %>%
  mutate(
    assigned_treatment = str_remove(assigned.treatment, " Arm"),
  )  %>%
  filter(assigned_treatment == gift_choice)
  ...
  data = data %>%  
    ...
    mutate(
      est_takeup_te = map(
        ...
        # is.na(assigned_dist_group_left) 
      ) %>% 
        map(
          mutate,
          assigned_dist_group_left = fct_explicit_na(assigned_dist_group_left, "Combined") %>% 
...
structural_private_ate_plot = dist_fit_data %>%
  grab_private_ate(., "structural", models_we_want) %>%
    # mutate(est_takeup_te = map(est_takeup_te, 
    # ~mutate(.x, assigned_dist_group_left = fct_rev(assigned_dist_group_left)))) %>%
      plot_estimands(., 
                     est_takeup_te, 
                     assigned_treatment_left, 
                     results_group = fit_type,
...
single_structural_private_ate_plot = dist_fit_data %>%
  grab_private_ate(., model_type_to_plot = "structural", models_we_want) %>%
  mutate(est_takeup_te = map(est_takeup_te, ~filter(.x, assigned_dist_group_left == "Combined"))) %>%
      plot_single_estimands(., 
                     est_takeup_te, 
                     assigned_treatment_left, 
                     results_group = fit_type, 
                     single_prior_predict = TRUE, 
...
single_structural_private_ate_plot +
  default_plot_caption
...
joint_wtp_results <- dist_fit_data %>%
  filter(fct_match(fit_type, "fit") & fct_match(model_type, "structural") & fct_match(model, models_we_want)) %>%
  pull(wtp_results) %>%
  first()
...
dist_prob_plot <- dist_fit_data %>% 
  filter(fct_match(fit_type, "fit"), fct_match(model, models_we_want)) %>% #, "STRUCTURAL_LINEAR_U_SHOCKS_NO_SUBMODELS"))) %>%
  transmute(
    model_name,
...
#| rep-return-te-df
if (params$component_checks) {
rep_te_plot_df = read_csv(
  file.path(
    params$output_path, 
    str_glue("
    rep_return_te_diff_draws_dist_fit{params$fit_version}-{params$models}.csv")
  )
)
...
rep_te_plot_fun = function(data, treat) {
  data %>%
    ...
    filter(assigned_treatment == treat) %>%
    ggplot(aes(distance)) +
...
if (params$component_checks) {
rep_te_plot_fun(rep_te_plot_df, "bracelet") + 
  dist_caption
}
```
...
if (params$component_checks) {
rep_te_plot_fun(rep_te_plot_df, "calendar") +
  dist_caption
}
```
...
if (params$component_checks) {
rep_te_plot_fun(rep_te_plot_df, "ink") +
  dist_caption
}
```
...
ggsave("temp-plots/anne-check/control-rr.png", width = 8, height = 6, dpi = 500)
```
...
plot_summ_bc_draws = read_csv("temp-data/plot-summ-draws-roc-vis-no-vis.csv")

p_roc = dist_fit_data %>% 
  filter(
    fct_match(fit_type, "fit"), 
    fct_match(model, models_we_want) 
    ) %>% 
  mutate(
    model_name,
  ...
  ggplot(aes(roc_distance, group = interaction(fit_type, assigned_treatment))) +
  geom_line(aes(y = per_0.5, color = assigned_treatment)) +
  geom_ribbon(aes(ymin = per_0.25, ymax = per_0.75, fill = assigned_treatment), alpha = 0.25) +
  geom_ribbon(aes(ymin = per_0.05, ymax = per_0.95, fill = assigned_treatment), alpha = 0.25) +
  geom_rug(
    inherit.aes = FALSE,
    aes(dist),
    alpha = 0.75,
...
prior_sm_post_df = dist_fit_data %>% 
  filter(
    fct_match(fit_type, "prior-predict"), 
    fct_match(model, models_we_want) 
    ) %>% 
  ...
  filter(fct_match(assigned_treatment, c("control","bracelet"))) %>% 
  filter(
    fct_match(fit_type, "prior-predict") & assigned_treatment == "bracelet"
  ) %>%
  filter(roc_distance < 2.5)  
...
p_roc + 
  ...
  geom_ribbon(
    data = prior_sm_post_df,
    aes(ymin = per_0.25, ymax = per_0.75), fill = "grey", alpha = 0.25) +
  NULL
...
p_roc + 
  ...
  geom_ribbon(
    data = prior_sm_post_df, 
    aes(ymin = per_0.05, ymax = per_0.95), fill = "grey", alpha = 0.25) +
  NULL
...
#| fig.cap="Rate of Change Estimates",
#| fig.note = "This plot shows the rate of change estimated using the structural 
#|  model. The dashed line corresponds to a multiplier where there is no visibility.
#|  Line: median.
#|  Outer ribbon: 90\\% credible interval.
#|  Inner ribbon: 50\\% credible interval."
