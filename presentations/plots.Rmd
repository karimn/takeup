...
params:
  ...
  struct_model: STRUCTURAL_LINEAR_U_SHOCKS_PHAT_MU_REP
...
if (interactive()) {
  params = lst(
    ...
    output_path = "temp-data", 
    struct_model = "STRUCTURAL_LINEAR_U_SHOCKS_PHAT_MU_REP",
    rf_model = "REDUCED_FORM_NO_RESTRICT_DIST_CTS",
    component_checks = FALSE, 
    plot_caption = TRUE,
    cache = FALSE
  )
...
if (params$fit_version != params$rf_fit_version) {
  ...
  dist_fit_data = dist_fit_data %>%
    filter(model %in% params$struct_model)
  ...
  rf_fit_data = with_env(load_rf_function, rf_env)() %>%
    filter(model %in% params$rf_model)
...
dist_prob_plot <- dist_fit_data %>% 
  filter(
    ...
    fct_match(model, models_we_want),
    fct_match(model_type, "structural")
    ) %>% #, "STRUCTURAL_LINEAR_U_SHOCKS_NO_SUBMODELS"))) %>%
  transmute(
    model_name,
