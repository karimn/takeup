...
knitr::opts_chunk$set(
  ...
  warning = FALSE, 
  cache = TRUE, 
  cache.path = str_glue("takeup-{params$models}-figure-cache/"), 
  fig.path = str_glue("takeup-{params$models}-fig/"), 
  fig.align = "center"
  )
...
if (params$component_checks) {
  ...
  struct_df %>%
    filter(treatment_b %in% c("calendar", "control")) %>%
    select(mu_rep, dist, draw, treatment_b) %>%
    spread(treatment_b, mu_rep) %>%
    mutate(cal_minus_control = calendar - control) %>%
    summarise(mean(cal_minus_control))


  struct_df %>%
    select(mu_rep, dist, treatment_b) %>%
    group_by(
      dist, treatment_b
    ) %>%
    summarise(
  ...
  struct_df %>%
    filter(
      treatment_b %in% c(
        ...
        "control")) %>%
    select(mu_rep, dist, draw, treatment_b)  %>%
    group_by(treatment_b, dist) %>%
    summarise(
      mean_mu_rep = mean(mu_rep)
    )
...
if (params$load_phat) {
  phat_env = new.env()
  load_phat_function = function(){
      load(file.path("temp-data", str_interp("processed_dist_fit80_lite.RData")))
      return(dist_fit_data)
  }
  phat_fit_data = with_env(load_phat_function, phat_env)() %>%
    filter(model == "STRUCTURAL_LINEAR_U_SHOCKS_PHAT_MU_REP") 
  dist_fit_data = bind_rows(
    dist_fit_data, 
    phat_fit
  )
  dist_fit_data = dist_fit_data %>%
    mutate(
      model = if_else(
        fit_type == "prior-predict" & model == "STRUCTURAL_LINEAR_U_SHOCKS",
...
baseline_praise_stigma_plot_data %>%
  ggplot(aes(response, fill = fct_rev(response))) +
  geom_col(aes(y = n), alpha = 0.5, position = position_dodge(width = 0.8)) +
  labs(y = "Share of Respondents", x = "") +
  scale_y_continuous(breaks = seq(0.25, 1, 0.25)) +
  coord_flip() +
  facet_grid(topic ~ praise.stigma, labeller = label_wrap_gen(width = 20), space = "free") +
  theme_bw() +
  theme(legend.position = "bottom",
...
baseline_praise_stigma_plot_data %>%
  ggplot(aes(response, fill = fct_rev(response))) +
  geom_col(aes(y = n), alpha = 0.5, position = position_dodge(width = 0.6), width = 0.4) +
  labs(y = "Share of Respondents", x = "") +
  scale_y_continuous(breaks = seq(0.25, 1, 0.25)) +
  coord_flip() +
  facet_grid(topic ~ praise.stigma, labeller = label_wrap_gen(width = 20), space = "free") +
...
# baseline_praise_stigma_plot_data %>%
#   ggplot(aes(response, fill = fct_rev(response))) +
#   geom_col(aes(y = n), alpha = 0.5, width = 0.4, position = position_dodge()) +
#   labs(y = "Share of Respondents", x = "") +
#   scale_y_continuous(breaks = seq(0.25, 1, 0.25)) +
#   coord_flip() +
#   facet_grid(topic ~ praise.stigma, labeller = label_wrap_gen(width = 20), space = "free") +
...
#   guides(fill = "none") +
#   scale_fill_grey(start = 0.2, end = 0.5) +
#   scale_x_discrete(breaks = expansion(add = -0.5))
...
df %>%
  ...
  geom_vline(
    xintercept = c(0.8, 0.85)
  )  +
  geom_vline(
    xintercept = c(0.78, 0.68)
  ) 
...
## Social Multiplier  - OLD MODEL BELOW HERE
...
bracelet_sm_df = read_csv("optim/data/pred-social-multiplier-fit71-cutoff-b-bracelet-mu-bracelet-STRUCTURAL_LINEAR_U_SHOCKS.csv")
control_sm_df = read_csv("optim/data/pred-social-multiplier-fit71-cutoff-b-control-mu-control-STRUCTURAL_LINEAR_U_SHOCKS.csv")

sm_df = bind_rows(
  control_sm_df %>% mutate(treatment = "control"),
  bracelet_sm_df %>% mutate(treatment = "bracelet")
)
summ_sm_df = sm_df %>%
  group_by(
    treatment, dist
