...
\end{align*}
  <!-- \Delta'(v^*) &= \frac{\phi(v^*)\left\{\phi(v^*)[2\Phi(v^*) - 1] - v^*\phi(v^*)\Phi(v^*)[1 - \Phi(v^*)]\right\}}{[(1 - \Phi(v^*))\Phi(v^*)]^2}  \\ -->
...
```{r dist-results, fig.width=7, fig.height=5, dependson="load-processed-dist-fit"}
group_dist_param %>% 
...
```{r take-up-ate, fig.width=7.5,fig.height=6,cache=FALSE, dependson="load-processed-dist-fit"}
dist_fit_data %>% 
  filter(!fct_match(model, "STRUCTURAL_STACKED")) %>% 
...
```{r take-up-levels-dist-split, fig.width=7.5,fig.height=4.2, dependson="load-processed-dist-fit"}
dist_fit_data %>%
  filter(!fct_match(model, "STRUCTURAL_STACKED")) %>% 
  mutate(est_takeup_level = map(est_takeup_level, filter, is.na(mu_assigned_treatment) | mu_assigned_treatment == assigned_treatment)) %>%  
  select(est_takeup_level, model, model_name, fit_type) %>% 
...
```{r take-up-ate-dist-split, fig.width=7.5, fig.height=5.2, dependson="load-processed-dist-fit"}
dist_fit_data %>% 
  filter(!fct_match(model, "STRUCTURAL_STACKED")) %>% 
  mutate(est_takeup_te =
    map_if(est_takeup_te, 
...
```{r take-up-ate-dist-effect, fig.width=7.5,fig.height=5, dependson="load-processed-dist-fit"}
dist_fit_data %>% 
  filter(!fct_match(model, "STRUCTURAL_STACKED")) %>% 
  mutate(est_takeup_te =
    map_if(est_takeup_te, 
...
```{r take-up-ate-struct-dist-split, fig.width=7.5, fig.height=5.25, dependson="load-processed-dist-fit"}
dist_fit_data %>% 
  filter(fct_match(model_type, "structural")) %>% 
  select(model, model_name, est_takeup_te, fit_type) %>% 
  unnest(est_takeup_te) %>%
...
```{r, fig.width=7.5, fig.height=5.75, message=FALSE, warning=FALSE, dependson="load-processed-dist-fit"}
annot_delta_data <- delta_data %>% 
  filter(between(v, -3, 6)) %>% 
  rename(x = v, y = delta)
  
v_cluster_data <- dist_fit[c("STRUCTURAL_QUADRATIC", "STRUCTURAL_QUADRATIC_SALIENCE")] %>% 
...
v_cluster_data %>% 
  ggplot() +
  geom_histogram(aes(iter_value, y = stat(ncount) * 5, fill = model), color = "black", position = "dodge", alpha = 0.75) +
...
```{r cost-mitig-level, fig.width=7.5, fig.height=5.5, dependson="load-processed-dist-fit"}
...
takeup_partial_v_plot <- dist_fit_data %>% 
  ...
  labs(subtitle = "Rate-of-change of Take-up", y = TeX(r"{$\frac{\partial E\[Y\]}{\partial c}$}")) +  
  theme(legend.position = "right") +
  NULL
...
diff_takeup_partial_v_plot <- dist_fit_data %>% 
  ...
  geom_ribbon(aes(ymin = -per_0.1, ymax = -per_0.9), color = NA, alpha = 0.25, show.legend = FALSE) +
  coord_cartesian(xlim = v_lim) +
  labs(subtitle = "Rate-of-change of Take-up Compared to Control Level", y = TeX(r"{$\frac{\partial E\[Y\]}{\partial c}$ Difference}")) + 
  facet_wrap(vars(assigned_treatment_left), labeller = as_labeller(str_to_title)) +
  NULL
