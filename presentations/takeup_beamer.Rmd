...
cowplot::plot_grid(
  beliefs_results$prob_knows %>% 
    filter(ord == 1) %>% 
    mutate(assigned.treatment = str_to_title(assigned.treatment)) %>% 
  ...
  beliefs_results$ate_knows %>% 
    filter(ord == 1) %>% 
    mutate(assigned.treatment_right = str_to_title(assigned.treatment_right)) %>% 
    ggplot(aes(y = assigned.treatment_right)) +
    geom_vline(xintercept = 0, linetype = "dotted") +
