...
monitored.and.matched.data <- matched.tu.data.2 %>%
  filter(endline.type %in% c("endline", "endline.backup", "reconsent", "reconsent.backup"))  
```
...
The "Name Matched" proportion is a good indicator of much enumerators are failing to identify monitored individuals.
...
monitored.and.matched.data %>% 
  filter(!cluster.spillover, !treatment.spillover) %>% 
  ggplot() +
...
```{r, echo=FALSE, fig.width=8, fig.height=10}
monitored.and.matched.data %>% 
  filter(!is.na(assigned.treatment.home), !cluster.spillover, !treatment.spillover) %>% 
  ggplot() +
...
```{r}
monitored.and.matched.data %>% 
  filter(!treatment.spillover, !cluster.spillover, sms.treatment == "sms.control") %>% 
  left_join(census.data %>% select(KEY.individ, monitored, monitor.consent, starts_with("dewormed")),
            c("any.matched.KEY.individ" = "KEY.individ"),
            suffix = c(".tu", ".census")) %>% {
              filter(., !dewormed.any) %>% 
                count(wave, dewormed.any, monitor.consent, match.type) %>% print
              
              count(., wave, assigned.treatment.home) %>% print
              
              filter(., !dewormed.any, monitor.consent, match.type == "PoT Matched") %>% select(any.matched.KEY.individ, cluster.id.home) %>% print
...
```{r}
census.data %>% 
  filter(monitored, !is.na(endline.type), sms.treatment == "sms.control", dewormed.any) %>% {
    count(., wave, assigned.treatment) %>% print
  
    anti_join(., monitored.and.matched.data, c("KEY.individ" = "any.matched.KEY.individ")) %>% {
      count(., wave, dewormed, sms.treatment, assigned.treatment) %>% print
    }
  }
```
