...
Overall, social information treatment seems to have had a dramatic effect on deworming take-up, in particular when combined with incentives/signals. In columns 2-4 and 10-12, we observe an increase in the social information interaction effects, but only consistently statistically significant when comparing the bracelet and social information take-up with (i) the ink and social information take-up or (ii) the control social information take-up.
...
print.reg.table(sms.reg.table.data, 
                ...
                estimate.buffer = TRUE, landscape = TRUE)
```
...
Finally, regression analysis results for the impact of incentive/signal treatment, SMS reminders and social information treatment, and assigned distance to treatment location, are presented in Figure \ref{fig:sms-dist-plot} and Tables \ref{tab:reg-sms-dist-monitored} and \ref{tab:reg-sms-dist-endline}.
...
dist.sms.reg.table.data %>% 
  ...
  a_ply(1, . %>% 
          print.reg.table(caption = "Incentive/signal treatment and SMS treatment effects, split by assigned distance to treatment location.",
                          ...
                          estimate.buffer = FALSE, landscape = TRUE))
```
