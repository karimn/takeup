  ...
  map_df(function(reg.res) {
    ...
    tidy(reg.res) %>% 
      ...
      bind_rows(control = .,
                ...
                social.info = linear_tester(reg.res, c("(intercept) + social.info", "ink:social.info")) %>% 
                  mutate(term = c("social.info", "ink:social.info")),
                "calendar:social.info" = linear_tester(reg.res, c("(intercept) + social.info + calendar:social.info", "bracelet:social.info - calendar:social.info")) %>% 
                  mutate(term = c("calendar:social.info", "bracelet:social.info")),
                .id = "ref.type") %>% 
      mutate(term = fct_recode(factor(term), control = "(intercept)"),
             ref.pt = term == ref.type) %>% 
      list %>% 
...
The following two specifications considered, we consider the impact of SMS reminders and informational messages, combined with incentive/signal treatment. Hence, we limit our analysis to the population of phone owners.
...
```{r tab-test, dependson=c("reg-tab-data", "basic-reg-table"), results="asis", cache=FALSE, warning=FALSE}
print.reg.table(sms.reg.table.data, 
                caption = "Incentive/signal treatment and SMS treatment effects.", 
                font.size = "footnotesize", estimate.buffer = FALSE, landscape = TRUE)

dist.sms.reg.table.data %>% 
  a_ply(1, print.reg.table, 
        caption = "Incentive/signal treatment and SMS treatment effects, split by assigned distance to treatment location.",
