...
sim_int %>% 
  ...
  # facet_grid(rows = vars(delta_type), cols = vars(type)) +
  NULL
```
...
```{r}
load(file.path("temp-data", str_interp("processed_dist_fitdist_fit.RData")))

dist_fit_data %<>% 
  mutate(model_color = "black")
```
...
dist_fit_data %>% 
  ...
  filter(
    ...
    fct_match(mu_assigned_treatment_right, c("control", "calendar")),
  ) %>%
  ...
  scale_y_discrete("", labels = str_to_title) +
    labs(
      ...
      x = "") +
  ggforce::facet_col(vars(mu_assigned_treatment_right), 
             labeller = as_labeller(. %>% str_c("Compared to, $z' = ", ., "$") %>% TeX(), default = label_parsed), 
             space = "free",
             scales = "free_y") +
  theme(legend.position = "none") +
...
\end{align*}
where
<!-- \Delta'[w^*(b,c)] = &-\frac{\int_{-\infty}^\infty v f_u(w^*(b,c) - v) f_v(v)\,\mathrm{d}v}{F_w(w^*(b,c))\left[1 - F_w(w^*(b,c))\right]} \\ -->
<!--                     &-\frac{f_w(w^*(b,c))\left[1 - 2F_w(w^*(b,c))\right]\Delta[w^*(b,c)]}{F_w(w^*(b,c))\left[1 - F_w(w^*(b,c))\right]} -->
\begin{align*}
