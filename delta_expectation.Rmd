...
params:
  structural_takeup_version: 93
  reduced_form_takeup_version: 93
---
...
dist_fit_data <- str_split(params$structural_takeup_version, ",") %>% 
  ...
  # # temp solution to get rid of a model I don't want
  filter(!(model == "STRUCTURAL_LINEAR_U_SHOCKS_PHAT_MU_REP" & version == 93)) %>%
  filter(model != "STRUCTURAL_LINEAR_U_SHOCKS_PHAT_MU_REP_HIER") %>%
  filter(version == 93 | (model == "STRUCTURAL_LINEAR_U_SHOCKS_PHAT_MU_REP" & version == 91)) %>%
  ...
  left_join(
    bind_rows(
      filter(., fct_match(model_type, "structural"), fct_match(fit_type, "fit")) %>%
        ...
        mutate(
          model_color = paletteer_d("ggthemes::Tableau_20")[1:n()]
        ) %>%
      filter(., !fct_match(model_type, "structural") | !fct_match(fit_type, "fit")) %>%
        select(model, model_type, fit_type) %>% 
...
no_data_dist_fit_data = dist_fit_data %>%
  filter(str_detect(model, "_NO_"))

dist_fit_data = dist_fit_data %>%
  filter(!str_detect(model, "_NO_") | str_detect(model, "REDUCED"))
...
models_we_want = c(
  ...
  "STRUCTURAL_LINEAR_U_SHOCKS_PHAT_MU_REP_HEIR_FOB", 
  "STRUCTURAL_LINEAR_U_SHOCKS_PHAT_MU_REP_HEIR_SOB", 
