...
params:
  structural_takeup_version: test 
  reduced_form_takeup_version: 53
---
...
load(file.path("temp-data", str_glue("processed_dist_fit{params$structural_takeup_version}.RData")))
...
dist_fit_data %<>% 
  mutate(
    model = fct_expand(model, c("STRUCTURAL_LINEAR_U_SHOCKS_PRIOR", "REDUCED_FORM_NO_RESTRICT_PRIOR")),
    model = if_else(fct_match(fit_type, "prior-predict") & fct_match(model_type, "structural"), factor("STRUCTURAL_LINEAR_U_SHOCKS_PRIOR", levels = levels(model)), model),
  ...
  left_join(
    bind_rows(
      filter(., fct_match(model_type, "structural"), fct_match(fit_type, "fit")) %>%
        ...
        mutate(model_color = canva_pal(canva_palette_vibrant)(n())),
      filter(., !fct_match(model_type, "structural") | !fct_match(fit_type, "fit")) %>%
        select(model, model_type, fit_type) %>% 
...
```{r incentive-te, fig.width=8, fig.height=17}
dist_fit_data %>%
  filter(
    fct_match(model_type, "structural") | (fct_match(model_type, "reduced form") & fct_match(fit_type, "fit"))
  ) %>% 
...
```{r signaling-te, fig.width=8, fig.height=17}
dist_fit_data %>% 
  filter(fct_match(model_type, "structural")) %>% 
  select(model, model_name, est_takeup_te, fit_type, model_color) %>% 
  mutate(
...
```{r private-incentive-te, fig.width=8, fig.height=17}
dist_fit_data %>% 
  filter(fct_match(model_type, "structural")) %>% 
  select(model, model_name, est_takeup_te, fit_type, model_color) %>% 
  mutate(
...
dist_prob_plot <- dist_fit_data %>% 
  filter(fct_match(fit_type, "fit"), fct_match(model, c("STRUCTURAL_LINEAR_U_SHOCKS_NO_BELIEFS_DIST"))) %>% #, "STRUCTURAL_LINEAR_U_SHOCKS_NO_SUBMODELS"))) %>%
  transmute(
  ...
  geom_line(aes(y = per_0.5), data = . %>%  unnest(cluster_takeup_prop)) +
  geom_ribbon(aes(ymin = per_0.25, ymax = per_0.75), alpha = 0.3, data = . %>%  unnest(cluster_takeup_prop)) +
  geom_ribbon(aes(ymin = per_0.1, ymax = per_0.9), alpha = 0.3, data = . %>%  unnest(cluster_takeup_prop)) +
  geom_rug(
    aes(dist),
    alpha = 0.5,
...
dist_prob_plot +
  geom_linerange(aes(assigned_dist_obs / 1000, ymin = per_0.1, ymax = per_0.9), data = . %>% unnest(obs_cluster_takeup_level)) +
...
dist_fit_data %>% 
  filter(fct_match(fit_type, "fit"), fct_match(model, c("STRUCTURAL_LINEAR_U_SHOCKS_NO_BELIEFS_DIST"))) %>% #, "STRUCTURAL_LINEAR_U_SHOCKS_NO_SUBMODELS"))) %>%
  transmute(cluster_roc = map2(cluster_roc, stan_data, ~ { 
    mutate(.x,
...
dist_fit_data %>% 
  ...
  ggplot(aes(roc_distance)) +
  geom_line(aes(y = per_0.5, color = model)) +
  geom_ribbon(aes(ymin = per_0.25, ymax = per_0.75, fill = model), alpha = 0.25) +
  geom_ribbon(aes(ymin = per_0.1, ymax = per_0.9, fill = model), alpha = 0.25) +
  scale_color_manual("", values = "black", aesthetics = c("color", "fill")) +
  labs(
...
dist_fit_data %>% 
  ...
  transmute(cluster_w_cutoff = map(cluster_w_cutoff, mutate, roc_distance = roc_distance / 1000)) %>% 
...
dist_fit_data %>% 
  filter(fct_match(fit_type, "fit"), fct_match(model, c("REDUCED_FORM_NO_RESTRICT", "STRUCTURAL_LINEAR_U_SHOCKS"))) %>% 
