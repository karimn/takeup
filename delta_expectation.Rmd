...
dist_prob_plot <- dist_fit_data %>% 
  ...
  transmute(
    across(matches("cluster_takeup_prop_(control|bracelet|ink)"), map, ~ mutate(.x, roc_distance = roc_distance / 1000)),
    obs_cluster_takeup_level,
  ) %>% 
  pivot_longer(-obs_cluster_takeup_level, names_to = "treatment", names_pattern = "_(control|bracelet|ink)$") %>% 
  mutate(
