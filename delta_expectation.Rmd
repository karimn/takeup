...
takeup_version <- 46 
reduced_form_takeup_version <- 47 
```
...
dist_fit_data %>% 
  ...
  unnest(iter_data) %>% 
  ggplot() +
  geom_histogram(aes(iter_est, y = stat(density) * 0.1, fill = fit_type), 
                 binwidth = 0.1, boundary = 0, position = "identity", alpha = 0.5) +
...
#   get_imputed_dist(dist_fit_data$stan_data[[1]]) %>% 
  ...
  facet_wrap(vars(dist_treatment), ncol = 1, scales = "free")
```
# Reduced Form

```{r reduced-form-load-processed-results}
load(file.path("temp-data", str_glue("processed_dist_fit{reduced_form_takeup_version}.RData")))
