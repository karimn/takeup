...
rf_analysis_data %>%
  ...
  geom_col(aes(fill = assigned_treatment), position = "dodge", alpha = 0.5) +
  scale_fill_discrete("", labels = str_to_title) +
  labs(title = "Takeup by Age Group", x = "Age Group", y = "Size") +
  facet_wrap(vars(assigned_dist_group), labeller = labeller(.default = str_to_title))
```
...
dist_fit_data %>% 
  ...
  scale_fill_discrete("", labels = c("fit" = "Posterior", "prior-predict" = "Prior Predict"), aesthetics = c("fill", "color")) +
  labs(x = "Age Group", y = "") +
  facet_grid(vars(assigned_dist_group), vars(assigned_treatment)) +
  theme(legend.position = "top")
```
