...
fit_version <- "param_static_2330864" # Name matched and SMS, private value separated (bracelet and calendar)
...
```{r static_level_smsctrl, fig.height=3, fig.width=8}
plot_takeup(
  takeup_summ_data = est_deworming_takeup_all,
  data_preparer = . %>%
    filter(
...
```{r static_ate_signal_effect_dist_diff, fig.width=8, fig.height=3}
est_deworming_takeup_ate %>% 
  filter(incentive_treatment_left %in% c("ink", "bracelet"),
         incentive_treatment_right == "control",
         dist.pot.group_left == dist.pot.group_right,
