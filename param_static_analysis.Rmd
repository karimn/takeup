...
fit_version <- "param_test_cluster_corr" # Name matched and SMS, private value separated (bracelet and calendar)
...
est_deworming_takeup_phone_dist <- est_deworming_takeup %>%
  # prepare_est_deworming_takeup(subgroups = c("phone_owner", "dist.pot.group", "name_matched")) %>% 
  # filter(name_matched)
...
```{r param-takeup-level-all-sms, fig.width=12, fig.height=4}
plot_takeup(
  takeup_summ_data = est_deworming_takeup_all,
  data_preparer = . %>%
    filter(
...
```{r param-takeup-level-dist-sms, fig.width=12, fig.height=6}
plot_takeup(
  takeup_summ_data = est_deworming_takeup_dist,
  data_preparer = . %>%
    filter(
