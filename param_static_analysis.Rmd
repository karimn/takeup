...
```{r}
save(
  static_treatment_cell_info, 
  static_ate_pair_info,
  est_deworming_takeup,
  est_deworming_takeup_ate,
  est_deworming_takeup_phone,
  est_deworming_takeup_dist,
  est_deworming_takeup_phone_dist,
  est_deworming_takeup_all,
  ...
  est_deworming_takeup_ate_all_name_matched,
  file = file.path("stan_analysis_data", "static_processed.RData")
)
```
...
ggsave(filename = "static_level_smsctrl.png", path = plot_save_path,
...
ggsave(filename = "static_level_all.png", path = plot_save_path,
...
```{r param-ate-sms-ctrl, fig.width=8, fig.height=4.2, eval=FALSE}
est_deworming_takeup_ate_all %>% 
  plot_ate(
    ate_summ_data = .,
      data_preparer = . %>% 
...
ggsave(filename = "static_ate_smsctrl.png", path = plot_save_path,
       width = 7.5, height = 3)
```
...
est_deworming_takeup_ate_phone %>%
  ...
  labs(y = "", subtitle = "For phone owners only, compared to control incentive treatment, and conditional on SMS treatment.") #+ facet_wrap(~ name_matched_left)

ggsave(filename = "static_ate_all.png", path = plot_save_path,
       width = 7.5, height = 4.25)
```

```{r static_ate_all-2, fig.width=8, eval=FALSE}
# est_deworming_takeup_ate_phone_name_matched %>%
est_deworming_takeup_ate_phone %>%
  mutate(same_incentive = incentive_treatment_left == incentive_treatment_right,
         same_sms = sms.treatment.2_left == sms.treatment.2_right) %>% 
...
```{r static_ate_combined_sms, fig.width=8}
est_deworming_takeup_ate_phone %>%
  mutate(same_incentive = incentive_treatment_left == incentive_treatment_right,
         same_sms = sms.treatment.2_left == sms.treatment.2_right) %>% 
  filter(phone_owner_left, 
  ...
  labs(y = "Treatment Effect on Take-up Probability", 
       subtitle = "For phone owners only, comparing incentive treatment + SMS treatment to the control.") + #+ facet_wrap(~ name_matched_left)
  theme(legend.position = "top") +
  scale_color_brewer("SMS Treatment", type = "qual", palette = "Set2") +
  NULL

ggsave(filename = "static_ate_combined_sms.png", path = plot_save_path,
...
```{r param-ate-with-sms, fig.width=10, fig.height=8, eval=FALSE}
...
```{r param-ate-dist, fig.width=12, fig.height=8, eval=FALSE}
est_deworming_takeup_ate_dist %>% 
  plot_ate(
    ate_data = est_deworming_takeup_ate,
    combiner = . %>% 
...
```{r param-ate-dist-sms-ctrl, fig.width=8, fig.height=4.2, eval=FALSE}
est_deworming_takeup_ate_dist %>% 
  plot_ate(
    ate_summ_data = .,
    data_preparer = . %>% 
...
ggsave(filename = "static_ate_smsctrl_dist.png", path = plot_save_path,
...
```{r static_ate_combined_sms_dist, fig.width=8, fig.height=8}
est_deworming_takeup_ate_phone_dist %>%
  mutate(same_incentive = incentive_treatment_left == incentive_treatment_right,
         same_sms = sms.treatment.2_left == sms.treatment.2_right) %>% 
  filter(phone_owner_left, 
...
ggsave(filename = "static_ate_combined_sms_dist.png", path = plot_save_path,
