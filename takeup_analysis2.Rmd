...
reg.output.nonphone.covar <- analysis.data %>%  
  filter(monitored, sms.treatment == "sms.control", !hh.baseline.sample) %>% #, sms.ctrl.subpop == "non.phone.owner") %>% 
  anti_join(outlier.clusters, c("sms.treatment", "cluster.id")) %>% 
...
reg.output.sms.treat.lim <- analysis.data %>%  
  filter(monitored, sms.treated | sms.ctrl.subpop == "phone.owner", !hh.baseline.sample, !is.na(floor)) %>% 
  anti_join(outlier.clusters, "cluster.id") %>% 
  mutate(sms.treatment = factor(sms.treatment, levels = c("sms.control", "reminder.only", "social.info"))) %>% 
  run_strat_reg(dewormed.any ~ assigned.treatment * sms.treatment, .strat.by = c("county", "dist.pot.group"), .cluster = "cluster.id")
