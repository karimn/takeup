...
list(Baseline = baseline.data, Endline = endline.data) %>% 
  ...
  scale_x_continuous("Reported Rate", breaks = 1:10) +
  scale_y_continuous("Density") +
  scale_color_discrete("") +
  scale_linetype_manual("Hypothetical Signal Used in MDA", values = c("dashed", "solid")) +
  theme(legend.position = "bottom") +
...
```{r reported-gift-pref, fig.width=8}
analysis.data %>% 
  plot.pref.unfaceted() +
  facet_wrap(~ assigned.treatment, nrow = 1) 
```

```{r reported-gift-pref-deworm, fig.width=8}
analysis.data %>% 
...
```{r first-choice-wtp, fig.height=4}
wtp.data %>% 
...
wtp.data %>% 
  filter(!is.na(first_choice)) %>%
  mutate(first_choice = fct_relabel(first_choice, str_to_title)) %>% 
