...
reg.endline.beliefs <- endline.data %>% 
  ...
  do(filter(., !is.outlier(.$dworm_rate))) %>% 
  ungroup %>% 
...
kable(tidy(reg.output.sms.ctrl.covar, .include_covar = TRUE), digits = 4)
```
...
kable(tidy(reg.output.nonphone.covar, .include_covar = TRUE), digits = 4)
```
...
kable(tidy(reg.output.sms.ctrl.dist.covar, .include_covar = TRUE), digits = 4)
```
...
reg.output.sms.treat.covar %>% tidy(.include_covar = TRUE) %>% kable(digits = 4)
```
...
reg.output.sms.treat.dist.covar %>% tidy(.include_covar = TRUE) %>% kable(digits = 4)
```
