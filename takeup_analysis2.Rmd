...
```{r, fig.width=8}
analysis.data %>% 
  filter(!is.na(gift_choice), monitored, consent) %>% 
...
```{r, fig.width=8}
analysis.data %>% 
  filter(!is.na(gift_choice), monitored, consent, !is.na(hh_cal), assigned.treatment == "calendar") %>% 
  mutate(dewormed.any = if_else(dewormed.any, "Dewormed", "Not Dewormed"),
...
```{r, fig.width=8}
analysis.data %>% 
  filter(!is.na(gift_choice), monitored, consent, !is.na(hh_bracelet), assigned.treatment == "bracelet") %>% 
  mutate(dewormed.any = if_else(dewormed.any, "Dewormed", "Not Dewormed"),
...
```{r, fig.width=8}
analysis.data %>% 
  filter(monitored, consent, !is.na(cal_value), assigned.treatment == "calendar") %>% 
  mutate(dewormed.any = if_else(dewormed.any, "Dewormed", "Not Dewormed"),
...
```{r, fig.width=8}
analysis.data %>% 
  filter(monitored, consent, !is.na(cal_value), !is.na(gift_choice), assigned.treatment == "calendar") %>% 
  mutate(dewormed.any = if_else(dewormed.any, "Dewormed", "Not Dewormed"),
         cal_value = fct_recode(cal_value, "Would still want a calendar" = "yes", "Would not want another calendar" = "no")) %>% 
...
reg.output.sms.ctrl.lim <- analysis.data %>%  
  ...
  filter(monitored, monitor.consent, sms.treatment == "sms.control", !baseline.sample, !is.na(floor)) %>% 
  anti_join(outlier.clusters, c("sms.treatment", "cluster.id")) %>% 
  run_strat_reg(dewormed.any ~ assigned.treatment, .strat.by = c("county", "dist.pot.group"), .cluster = "cluster.id")
...
reg.output.sms.ctrl.cont.dist.lim <- analysis.data %>%  
  filter(monitored, monitor.consent, sms.treatment == "sms.control", !baseline.sample, floor!="NA") %>% 
  mutate(dist.to.pot = dist.to.pot / 1000, # Convert to km
         dist.to.pot.2 = dist.to.pot^2) %>% 
  anti_join(outlier.clusters, c("sms.treatment", "cluster.id")) %>% 
  run_strat_reg(dewormed.any ~ assigned.treatment * (dist.to.pot + dist.to.pot.2), .strat.by = "county", .cluster = "cluster.id")
...
reg.output.sms.treat.lim <- analysis.data %>%  
  filter(monitored, monitor.consent, sms.treatment != "sms.control" | sms.ctrl.subpop == "phone.owner", !baseline.sample, !is.na(floor)) %>% 
  anti_join(outlier.clusters, "cluster.id") %>% 
  mutate(sms.treatment = factor(sms.treatment, levels = c("sms.control", "reminder.only", "social.info"))) %>% 
  run_strat_reg(dewormed.any ~ assigned.treatment * sms.treatment, .strat.by = c("county", "dist.pot.group"), .cluster = "cluster.id")
