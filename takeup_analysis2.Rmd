...
```{r regression, eval=FALSE, include=TRUE}
  ...
  bind_cols(select(.data, KEY.individ, last.stratum.size, total.size, stratum), 
    ...
    mutate(interact.coef = interact.coef * if_else(interact.stratum == last.stratum, 1/stratum.size, 1),
           interact.term.key = paste(interact.assign, interact.stratum, sep = "_") %>% 
             str_replace_all(c("assigned.treatment" = "", .interact.with %>% paste0(".", .) %>% setNames(rep("_", length(.)), .)))) %>% 
...
reg.data <- census.data %>%  
  ...
  left_join(calc.strat.interaction.terms(., .interact.with = "dist.pot.group", last.stratum = "Siaya"), "KEY.individ") 

reg.data %>% {
    reg.formula <- as.formula(sprintf("dewormed.any ~ stratum + dist.pot.group - 1 + %s", 
    ...
    coeftest(vcov. = vcov.clx(., xx$cluster.id))
```
