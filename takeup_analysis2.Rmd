...
bind_rows(monitored = outlier.clusters, all = unmonitored.outlier.clusters, .id = "monitored.sample") %>% 
  select(cluster.id, sms.treatment, monitored.sample) %>% 
  arrange(monitored.sample, sms.treatment) %>% 
  kable
```
...
unmon.reg.output.sms.ctrl <- analysis.data %>%  
  ...
  anti_join(unmonitored.outlier.clusters, c("sms.treatment.2" = "sms.treatment", "cluster.id")) %>% 
  run_strat_reg(dewormed.any ~ assigned.treatment, .strat.by = c("county", "dist.pot.group"), .cluster = "cluster.id")
```
...
unmon.reg.output.sms.ctrl.dist <- analysis.data %>%  
  filter(sms.treatment.2 == "sms.control", !hh.baseline.sample) %>% 
  anti_join(outlier.clusters, c("sms.treatment.2" = "sms.treatment", "cluster.id")) %>% 
  run_strat_reg(dewormed.any ~ assigned.treatment * dist.pot.group, .strat.by = "county", .cluster = "cluster.id")
```
