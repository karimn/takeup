...
plot.takeup.dynamics <- function(.data, .aes = aes(dewormed.day.any, takeup.prop, color = assigned.treatment)) .data %>% 
  ggplot(.aes) +
  geom_line() + 
  ...
  coord_cartesian(y = c(0, max(.data$takeup.prop) + 0.0001)) +
  labs(caption = "Vertical bars mark the days after receiving SMS messages.\nThe red bar marks the first deworming, so subjects would have only received a reminder")
```
