...
options(contrasts=c("contr.Treatment", getOption("contrasts")[2]), dplyr.show_progress = FALSE) 
```
...
calc.strat.interaction.terms <- function(.data, .interact.with = NULL, last.stratum = "Siaya.far") {
  interact.formula <- sprintf("~ %s - 1", paste(c("assigned.treatment", .interact.with, "stratum"), collapse = ":")) %>% 
    as.formula
...
run.strat.reg <- function(.data, 
  ...
  reg.formula <- as.formula(sprintf("dewormed.any ~ stratum - 1 %s + %s", 
...
```{r}
neyman.results %>% unnest(treatment.data)
neyman.results.dist %>% unnest(treatment.data)
```
