...
analysis.data <- census.data %>% 
  filter(!cluster.id %in% outlier.clusters, # Remove outliers
         !is.na(wave)) %>%  # Remove clusters no longer in study
...
analysis.data %>% 
  ...
  group_by(assigned.treatment, stratum) %>% 
  ...
  scale_x_discrete("Strata") +
  scale_color_discrete("Treatment") +
  facet_wrap(~ assigned.treatment, nrow = 2) + 
  theme(legend.position = "bottom")
```

```{r}
outlier.clusters <- c(249, 141, 343)
```
...
Without controls (using cluster robust standard errors):
