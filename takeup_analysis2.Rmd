...
```{r}
endline.data %>% 
  ggplot(aes(gift_choice)) +
  geom_bar(aes(y = ..count../sum(..count..)), alpha = 0.5, color = "black") +
  labs(x = "Gift", y = "Preferred Proportion", title = "Endline Gift Preference", subtitle = "Pooling all arms") +
  coord_flip() 
```
...
analysis.data %>% 
  ...
  labs(x = "Gift", y = "Preferred Proportion", title = "Endline Gift Preference", subtitle = "Split by incentive arm") +
...
analysis.data %>% 
  ...
  group_by(assigned.treatment, dewormed.any) %>% 
  mutate(arm.size = n()) %>% 
  group_by(gift_choice, add = TRUE) %>%
  summarize(pref.prop = n() / first(arm.size)) %>% 
  ungroup %>% 
  ...
  labs(x = "Gift", y = "Preferred Proportion", title = "Endline Gift Preference", 
       subtitle = "Split by incentive arm and deworming take-up") +
  facet_grid(dewormed.any ~ assigned.treatment)
```

```{r, fig.width=8}
analysis.data %>% 
  filter(!is.na(gift_choice), monitored, monitor.consent, !hh.baseline.sample, !is.na(hh_cal), assigned.treatment == "calendar") %>% 
  mutate(dewormed.any = if_else(dewormed.any, "Dewormed", "Not Dewormed"),
         hh_cal = fct_recode(hh_cal, "Other in household got calendar" = "yes", "No one else got calendar" = "no")) %>%
  group_by(dewormed.any, hh_cal) %>% 
  mutate(arm.size = n()) %>% 
  group_by(gift_choice, add = TRUE) %>%
  summarize(pref.prop = n() / first(arm.size)) %>% 
  ungroup %>% 
  ...
  geom_col(alpha = 0.5, color = "black") +
  labs(x = "Gift", y = "Preferred Proportion", title = "Endline Gift Preference", 
       subtitle = "Split by whether household has other calendar(s) and deworming take-up", caption = "This is only for the calendar arm of the experiment") +
  facet_grid(dewormed.any ~ hh_cal)
```
...
```{r, fig.width=8}
analysis.data %>% 
  filter(!is.na(gift_choice), monitored, monitor.consent, !is.na(hh_bracelet), !hh.baseline.sample, assigned.treatment == "bracelet") %>% 
  mutate(dewormed.any = if_else(dewormed.any, "Dewormed", "Not Dewormed"),
         hh_bracelet = fct_recode(hh_bracelet, "Other in household got bracelet" = "yes", "No one else got bracelet" = "no")) %>%
  group_by(dewormed.any, hh_bracelet) %>% 
  mutate(arm.size = n()) %>% 
  group_by(gift_choice, add = TRUE) %>%
  summarize(pref.prop = n() / first(arm.size)) %>% 
  ungroup %>% 
  ...
  geom_col(alpha = 0.5, color = "black") +
  labs(x = "Gift", y = "Preferred Proportion", title = "Endline Gift Preference", 
       subtitle = "Split by whether household has other bracelet(s) and deworming take-up", caption = "This is only for the bracelet arm of the experiment") +
  facet_grid(dewormed.any ~ hh_bracelet)
```
...
wtp.data %>% 
  ...
  ggplot() +
  geom_bar(aes(first_choice, y = ..count../sum(..count..)), alpha = 0.5, color = "black") +
  labs(x = "First Choice", y = "Proportion", title = "First Choice When Offered Calendars or Bracelets", 
...
ggplot(wtp.cdf, aes(x=price,y=tot_pr)) + theme_bw() + geom_point() + geom_line()
```
