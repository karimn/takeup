...
lm(dewormed.any ~ county*dist.pot.group + assigned.treatment*dist.pot.group, data = xx) %>% 
...
calc.strata.stats <- function(.data, 
  ...
  strata.data %>% {
    left_join(distinct_(., .dots = c(.strat.by, "treatment.group", .interact.with), .keep_all = TRUE) %>% 
                ...
                nest(-treatment.group, .key = strata.data),
              group_by_(., .dots = c("treatment.group", "rhs.treatment.group", .interact.with)) %>%
                ...
                ungroup %>% 
                nest(-treatment.group, .key = treatment.data),
              "treatment.group")
  }
}
    ...
    select(county, dist.pot.group, cluster.id, assigned.treatment, sms.treatment, dewormed.any) %>% {
     ...
     if (.reps > 0) {
       ...
       bs.analysis <- block.bootstrap(., .reps, "cluster.id") %>%
         ...
         ungroup %>% 
         select(replicate, treatment.group, treatment.data) %>% 
         unnest %>% 
         group_by_(.dots = c("treatment.group", "rhs.treatment.group", .interact.with)) %>%
       ...
       original.stats %<>% 
         ...
         nest(treatment.group, .key = treatment.data) %>% 
         left_join(original.stats, ., c("treatment.group", .interact.with))
       
      left_join(original.stats, ., c("treatment.group", "rhs.treatment.group", .interact.with)) 
     } else {
