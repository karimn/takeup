...
nonparam_dyn_analysis_data <- analysis.data %>%
  mutate(private_value = fct_collapse(assigned.treatment, 
         ...
         dyn_dist_pot = dist.pot.group,
         reminder_info_stock = sms.treatment.2) %>% 
  filter(!name_matched, assigned.treatment == "control", sms.treatment.2 == "control") #, !hh.baseline.sample)
...
nonparam_dyn_static_treatment_map <- nonparam_dyn_analysis_data %>% 
  ...
  data_grid(private_value, dyn_dist_pot) %>% #, social_value, incentive_shift, signal_observed, dist.pot.group, phone_owner) %>%
...
nonparam_dyn_stan_data <- prepare_bayesian_analysis_data(
  ...
  treatment_formula = ~ 1, # private_value, 
  subgroup_col = NULL,
  # subgroup_col = "phone_owner",
  drop_intercept_from_dm = FALSE,
  ...
  hyper_dyn_rho_shape = 4,
  hyper_dyn_rho_scale = 4,
  
  estimate_ate = 0,
  use_logit = 1,
  first_dynamics_day = 2
)
...
model_3_gp_nonparam <- stan_model(file = file.path("stan_models", "takeup_model_3_gp_gumbel.stan"), model_name = "model_3_gp_gumbel")
```

```{r model-3-gp-nonparam, eval=FALSE}
nonparam_dyn_fit_version <- "testing_2"
...
model_3_fit_debug <- nonparam_dyn_stan_data %>% 
  sampling(model_3_gp_nonparam, data = ., chains = 2, iter = 20, # , control = lst(max_treedepth = 15))
           init = gen_nonparam_initializer(.)) 
```
