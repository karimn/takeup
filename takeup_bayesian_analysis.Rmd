...
sms_ctrl_est_deworming_takeup_ate <- est_deworming_takeup_ate %>% 
  ...
  summarize(mean_ate = mean(sim_outcome),
...
dyn_analysis_data <- analysis.data %>%
  mutate(private_value = fct_collapse(assigned.treatment, control = c("control", "ink")),
         ...
         day_after_reminder_info = reminder_info_stock,
         days_observed = 11,
         days_sms = 11) %>% 
  filter(!name_matched | sms.treatment.2 == "sms.control") # There are a couple of SMS observations that are name-checked. Drop them. 

dyn_dyn_treatment_maps <- 
  lst(formula = ~ (poly(days_observed, 2, raw = TRUE)*signal_observed + poly(days_sms, 2, raw = TRUE)*reminder_info_stock + days_after_sms*day_after_reminder_info)*phone_owner, 
      trends = tibble(days_observed = 0:11,
                      days_sms = rep(0:5, each = 2),
                      days_after_sms = c(0, rep(0:1, 5), 0))) %>% 
  prepare_dynamic_treatment_maps(dyn_analysis_data)

dyn_static_treatment_map <- dyn_analysis_data %>% 
  data_grid(private_value, social_value, signal_observed, reminder_info_stock, day_after_reminder_info, dist.pot.group, sms.treatment.2, 
            days_observed = 0:12, days_sms = 0:12,
  ...
  filter(phone_owner | sms.treatment.2 == "sms.control",
        (private_value == "control" & social_value == "control") | sms.treatment.2 != "reminder.only",
         ...
         reminder_info_stock == fct_recode(sms.treatment.2, control = "sms.control") | reminder_info_stock == "control",
         day_after_reminder_info == reminder_info_stock) %>% 
...
dyn_sms_control_ate <- tribble(
  ...
  #     "control",            "bracelet",          "bracelet",              12,                  "control",             "control",            "control",               0,
  ...
  mutate(sms.treatment.2_left = "sms.control", sms.treatment.2_right = "sms.control",
         ...
         day_after_reminder_info_left = reminder_info_stock_left,
         day_after_reminder_info_right = reminder_info_stock_right) %>% 
...
dyn_phone_owners_ate <- tribble(
  ...
  bind_rows(
    tribble(
      ...
      "control",            "bracelet",         "control",               "social.info",          "control",                  "control",             "bracelet",            "control",    "sms.control",           "control",
      
      
      "control",            "bracelet",         "control",               "social.info",          "control",                  "control",             "control",             "control",    "social.info",       "control")) %>% 
  ...
  bind_rows(
    tribble(
      ...
      "control",            "bracelet",         "bracelet",              "social.info",          "social.info",              12,               "control",                    "bracelet",         "bracelet",               "social.info",           "social.info",               0,
      "control",            "bracelet",         "bracelet",              "social.info",          "social.info",              0,                "control",                    "bracelet",         "bracelet",               "sms.control",           "control",                   0
  )) %>% 
  bind_rows("close" = ., "far" = ., .id = "dist.pot.group") %>%
  mutate(phone_owner = TRUE,
         hh.baseline.sample = FALSE, 
...
summ_dyn_takeup <- est_deworming_days %>% 
  ...
  group_by(day, phone_owner, dist.pot.group, private_value, social_value, sms.treatment.2, incentive_treatment) %>% 
  summarize(mean_takeup_prop = mean(sim_outcome),
            ub_90 = quantile(sim_outcome, 0.95),
            lb_90 = quantile(sim_outcome, 0.05)) %>% 
  ungroup() %>% {
...
bind_rows(Static = sms_ctrl_est_deworming_takeup_ate %>% 
          ...
          Dynamic = dyn_ate %>% 
            ...
            select(incentive_treatment_left, phone_owner, dist.pot.group_left, mean_ate, matches("[lu]b_9[05]"), name_matched_left),
          .id = "model") %>%
  mutate(model = if_else(model == "Static", model, str_c(model, name_matched_left, sep = "-"))) %>% 
  mutate_at(vars(matches("incentive_treatment")), 
            funs(fct_relevel(factor(.), "control-ink", "calendar-control", "control-bracelet", "bracelet-bracelet"))) %>% 
  ggplot(aes(incentive_treatment_left, mean_ate, group = model)) +
  geom_linerange(aes(ymin = lb_90, ymax = ub_90, color = model), position = position_dodge(width = 0.2), size = 2) +
  geom_pointrange(aes(ymin = lb_95, ymax = ub_95), position = position_dodge(width = 0.2), size = 0.25) +
  geom_hline(yintercept = 0, linetype = "dotted") +
  # geom_text_repel(aes(label = sprintf("%.2f", mean_ate), color = NULL)) +
  geom_text(aes(label = sprintf("%.2f", mean_ate), color = NULL), nudge_x = -0.25, data = . %>% filter(model == "Dynamic")) +
  geom_text(aes(label = sprintf("%.2f", mean_ate), color = NULL), nudge_x = 0.25, data = . %>% filter(model == "Static")) +
  scale_x_discrete("Comparator Incentive Treatment") +
  scale_y_continuous("Average Difference in Deworming Proportion") +
