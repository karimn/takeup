...
stan_data <- analysis.data %>% 
  ...
  mutate_at(vars(county_dist_stratum, county_dist_mon_stratum, county, stratum, # county_phone_owner_stratum,
            ...
            funs(id = as.integer)) %>% 
  prep_data_arranger() %>% 
  prepare_bayesian_analysis_data(treatment_formula = ~ assigned.treatment * sms.treatment.2 * phone_owner - assigned.treatment * sms.treatment.2 + assigned.treatment * phone_owner * (name_matched * hh.baseline.sample),
...
basic_model_0_fit <- sampling(basic_model_0, data = stan_data, chains = 8, iter = 500, sample_file = file.path("stanfit", "basic_model_0.csv"), control = lst(adapt_delta = 0.99, max_treedepth = 15))
