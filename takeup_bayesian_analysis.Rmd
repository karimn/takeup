...
basic_model_1_fit <- sampling(basic_model_1, data = param_stan_data, chains = 16, iter = 400, sample_file = file.path("stanfit", "basic_model_1_4.csv"), control = lst(max_treedepth = 15)) # adapt_delta = 0.99, 
```

```{r, eval=FALSE}
basic_model_1_fit <- dir("stanfit", pattern = "basic_model_1_3_..csv") %>% 
  file.path("stanfit", .) %>% 
  read_stan_csv()
```
...
```
```{r}
print(basic_model_1_fit, pars = "hyper_ksh_util_gamma_raw")
```
