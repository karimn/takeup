...
param_all_ate <- param_sms_control_ate %>% # bind_rows(sms_control_ate, phone_owners_ate) %>%
  mutate(hh.baseline.sample = FALSE) 
```
...
param_stan_data <- analysis.data %>%
  ...
  filter(!name_matched, sms.treatment.2 == "sms.control", assigned.treatment != "ink") %>% # | sms.treatment.2 == "sms.control") %>% 
  prepare_bayesian_analysis_data(
    wtp.data, 
