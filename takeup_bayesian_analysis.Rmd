...
```{r static-model-ate-plot, fig.height=6}
sms_ctrl_est_deworming_takeup_ate %>% 
  ...
  geom_text(aes(label = sprintf("%.2f", mean_ate)), nudge_x = 0.2, size = 3) +
  # geom_text_repel(aes(label = sprintf("%.3f", mean_ate), nudge_x = 0.2, size = 3, segment.color = NA) +
  scale_x_discrete("Signal/Incentive Treatment") +
  scale_y_continuous("Posterior Treatment Effect on Deworming") +
  labs(title = "Signal/Incentive Treatment Effect", subtitle = "Compared with the control arm and with no SMS treatment", 
  ...
  facet_grid(phone_owner ~ dist.pot.group_left, 
             labeller = labeller(.cols = as_labeller(. %>% str_to_title() %>% str_c("Distance: ", .)),
                                 .rows = as_labeller(function(row_val) str_c(case_when(row_val == "TRUE" ~ "", TRUE ~ "Non-"), "Phone Owners")))) +
  theme(axis.text.x = element_text(angle = 25, hjust = 1))
```

```{r static-model-takeup, fig.width=8}
sms_ctrl_est_deworming_takeup %>%
  ...
  geom_pointrange(aes(ymin = lb_95, ymax = ub_95), position = position_dodge(width = 0.5)) +
  geom_text_repel(aes(label = sprintf("%.3f", mean_takeup_prop), color = NULL), nudge_x = -0.5, size = 3, segment.color = NA) +
  scale_x_discrete("Signal/Incentive Treatment") +
  scale_y_continuous("Posterior Probability of Deworming") + #, breaks = seq(0, 1, 0.1)) +
  labs(title = "Deworming Take-up by Signal/Incentive Treatment Arm", 
  ...
  coord_flip() +
  # facet_grid(phone_owner ~ dist.pot.group, labeller = labeller(.cols = . %>% str_to_title() %>% str_c("Distance: ", .))) +
...
fast_sim %<>% 
  ...
  bind_rows(group_by_at(., vars(-c(deworming_day, prob_deworm, log_alpha, log_kappa_dyn_treat, iter_obs_id))) %>% 
...
fast_sim_mean <- fast_sim %>%  
  ...
  do(bind_cols(summarize_at(., vars(prob_deworm, log_alpha, log_kappa_dyn_treat), mean), 
...
```{r fast-sim-daily-obs-plot, fig.width=12, fig.height=8}
fast_sim_mean %>%
  ...
  mutate(obs_type = ifelse(incentive_treatment == "control", 0, dyn_treat_days) %>% 
         ...
         phone_owner = if_else(phone_owner, "Phone Owners", "Non-Phone Owners")) %>% 
           # fct_explicit_na(na_level = "control")) %>% 
...
```{r fast-sim-daily-dyn-kappa, fig.width=12, fig.height=8}
fast_sim_mean %>%
  filter(deworming_day <= 12, 
  ...
  geom_line(aes(y = exp(log_kappa_dyn_treat), 
...
```{r fast-sim-daily-ate-obs-plot, fig.width=12, fig.height=8}
dyn_fast_sim_ate %>%
  filter(deworming_day <= 12,
...
fast_sim_mean %>%
  ...
  facet_grid(dist.pot.group ~ incentive_treatment, scales = "free_y",
             labeller = labeller(.rows = as_labeller(. %>% str_to_title() %>% str_c("Distance: ", .)),
                                 .cols = as_labeller(. %>% str_to_title()))) +
...
fast_sim_mean %>%
  ...
  labs(title = expression(paste("Daily Change in Dynamic Component of ", kappa, " Function")), subtitle = "In response to signal observation and SMS treatment.",
       y = expression(paste("Posterior Mean of ", kappa^dyn))) +
  theme(legend.position = "bottom")
```
...
dyn_fast_sim_ate %>%
  filter(deworming_day == 1,
         ...
         sms.treatment.2_left != "sms.control",
         sms.treatment.2_right == "sms.control") %>% 
  mutate(sms.treatment.2_left = fct_recode(sms.treatment.2_left, "Reminders Only" = "reminder.only", "Social Information" = "social.info"),
         incentive_treatment_left = fct_relabel(incentive_treatment_left, str_to_title)) %>% 
  ggplot(aes(x = incentive_treatment_left, y = prob_deworm_ate)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = alpha("black", 0.5)) +
  geom_pointrange(aes(ymin = prob_deworm_diff_quant_5, ymax = prob_deworm_diff_quant_95, color = sms.treatment.2_left), position = position_dodge(width = 0.4)) +
  ...
  coord_flip() +
  facet_wrap(~ dist.pot.group, scales = "free_y", labeller = as_labeller(. %>% str_to_title() %>% str_c("Distance: ", .)), ncol = 1, strip.position = "right") +
  labs(title = "Treatment Effect of SMS Treatment on Day One",
       subtitle = "with the same arm (no SMS) as reference", 
       x = "Signal/Incentive Treatment", y = "Posterior Difference in Probability of Deworming") +
  theme(legend.position = "bottom") 
```

##### Overall Take-up

```{r sim-fast-take-up, fig.width=12, fig.height=8}
fast_sim_mean %>% 
...
# no_obs_ate <- dyn_fast_sim_ate %>% 
#   filter(deworming_day == 13, dyn_treat_days_left == 0)

fast_sim_ate %>%
  # bind_rows(observe = ., no_observe = no_obs_ate, .id = "obs_type") %>% 
...
bind_rows(observe = fast_sim_ate %>% filter(incentive_treatment_right == "control", sms.treatment.2_right == "sms.control"),
