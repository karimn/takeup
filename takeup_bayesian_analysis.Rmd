...
paropts <- lst(.packages = c("magrittr", "stringr", "tidyverse"),
               .export = c("estimate_deworm_prob", "identify_treatment_id", "estimate_treatment_deworm_prob", "estimate_treatment_deworm_prob_dm",
...
fast_treatments <- tribble(
  ...
  expand(nesting_(names(.)), 
         dist.pot.group = c("close", "far"), phone_owner = c(TRUE, FALSE), name_matched = c(TRUE, FALSE), sms.treatment.2 = c("sms.control", "reminder.only", "social.info")) %>% 
...
```{r sim-inference, eval=FALSE}
dyn_day_sim <- obs_meta_data %>% 
  distinct(stratum_id, cluster_id) %>% 
  left_join(sim_stratum_census_covar_coef, c("stratum_id")) %>%
  left_join(sim_stratum_treatment_coef, c("stratum_id", "iter_id")) %>%
  left_join(sim_stratum_dyn_treatment_coef, c("stratum_id", "iter_id")) %>%
  right_join(sim_cluster_hazards, c("cluster_id", "iter_id")) %>% 
  filter(iter_id %in% 1:1) %>%
  ddply(.(iter_id), estimate_deworm_prob, .parallel = FALSE, .progress = "text", 
        ...
        treatment_dm = dyn_stan_data$treatment_map %>%
          filter(private_value == "control", social_value == "control", sms.treatment.2 == "sms.control",
                 signal_observed == "control", reminder_info_stock == "control", !hh.baseline.sample) %>%
          select(all_treatment_id, phone_owner, name_matched) %>%
          bind_cols(dyn_stan_data$treatment_map_design_matrix[.$all_treatment_id, ]),
        full_dyn_treatment_dm = dyn_stan_data$dynamic_treatment_map)
```

```{r sim-ate-inference, warning=FALSE, eval=FALSE}
dyn_day_ate_sim <- obs_meta_data %>% 
  distinct(stratum_id, cluster_id) %>% 
  left_join(sim_stratum_census_covar_coef, c("stratum_id")) %>%
  left_join(sim_stratum_treatment_coef, c("stratum_id", "iter_id")) %>%
  left_join(sim_stratum_dyn_treatment_coef, c("stratum_id", "iter_id")) %>%
  right_join(sim_cluster_hazards, c("cluster_id", "iter_id")) %>% 
  # filter(iter_id %in% 1:1) %>%
  ddply(.(iter_id), estimate_deworm_prob_ate, .parallel = TRUE, .progress = "text", 
        .paropts = lst(.packages = c("magrittr", "stringr", "tidyverse"),
                       .export = c("identify_treatment_id", "estimate_treatment_deworm_prob", "estimate_treatment_deworm_prob_dm")),
        obs_meta_data = obs_meta_data,
        sim_stratum_hazards = sim_stratum_hazards,
        census_covar_dm = dyn_stan_data %$% census_covar_map_dm[census_covar_id, ],
        ate_info = dyn_all_ate[1, ],
...
xx <- iter_cluster_parameters %>% 
  ...
  ddply.(iter_id), fast_est_fun, .parallel = FALSE, .progress = "text", .paropts = paropts,
                  treatments_info = fast_treatments %>% filter(social_value == "bracelet" & sms.treatment.2 == "sms.control"), days_treated = 4)
```
...
```{r}
treatment_col <- c("phone_owner", "dist.pot.group", "incentive_treatment", "sms.treatment.2", "dyn_treat_days")

load(file.path("data", "fast_sim.RData"))

fast_sim %<>%
  filter(days_sms == 12) %>% 
  group_by_at(vars(-c(deworming_day, prob_deworm, alpha, days_observed, days_sms))) %>% 
  distinct(deworming_day, .keep_all = TRUE) %>% 
  ungroup() %>% 
  mutate(incentive_treatment = if_else(private_value != "control", private_value, social_value) %>% 
           fct_relevel("control", "ink", "calendar", "bracelet"),
...
fast_sim %<>% 
  ...
  bind_rows(group_by_at(., vars(-c(deworming_day, prob_deworm, alpha, days_observed, days_sms, iter_obs_id))) %>% 
...
fast_sim_mean <- fast_sim %>%  
  ...
  do(bind_cols(summarize_at(., vars(prob_deworm, alpha), mean), 
...
```{r}
fast_sim %>% 
  distinct_(.dots = c("fast_treatment_id", treatment_col)) %>% 
...
dyn_fast_sim_ate_pairs <- tribble(
  ...
  # expand(nesting_(names(.)), dyn_treat_days = 0:11)

fast_sim_ate <- fast_sim_ate_pairs %>% 
  rowwise() %>% 
  do({
    inner_join(filter(fast_sim_day13_only,
                      dyn_treat_days == 11,
               ...
               c("iter_id", "phone_owner", "dist.pot.group", "dyn_treat_days"),
               suffix = c("_left", "_right")) %>%
      mutate(prob_deworm_diff = prob_deworm_left - prob_deworm_right) %>% 
      group_by_at(vars(phone_owner, dist.pot.group, 
                       starts_with("incentive_treatment"), starts_with("sms.treatment.2"), starts_with("dyn_treat_days"))) %>% 
      do(bind_cols(summarize(., prob_deworm_ate = mean(prob_deworm_diff)), 
                   tidy(t(quantile(.$prob_deworm_diff, probs = quants / 100, names = FALSE))) %>% set_colnames(str_c("prob_deworm_diff_quant_", quants)))) %>%  
      ungroup()
  })
...
dyn_fast_sim_ate <- dyn_fast_sim_ate_pairs %>% 
  rowwise() %>% 
  do({
    inner_join(filter(fast_sim,
                      incentive_treatment == .$incentive_treatment_left,
                      sms.treatment.2 == .$sms.treatment.2_left),
               filter(fast_sim, 
                      incentive_treatment == .$incentive_treatment_right,
                      sms.treatment.2 == .$sms.treatment.2_right), 
               c("iter_id", "phone_owner", "dist.pot.group", "deworming_day"), # "dyn_treat_days"),
               suffix = c("_left", "_right")) %>% 
      mutate(prob_deworm_diff = prob_deworm_left - prob_deworm_right) %>% 
      group_by_at(vars(phone_owner, dist.pot.group, deworming_day,
                       starts_with("incentive_treatment"), starts_with("sms.treatment.2"), starts_with("dyn_treat_days"))) %>% 
      do(bind_cols(summarize(., prob_deworm_ate = mean(prob_deworm_diff)), 
                   tidy(t(quantile(.$prob_deworm_diff, probs = quants / 100, names = FALSE))) %>% set_colnames(str_c("prob_deworm_diff_quant_", quants)))) %>%  
      ungroup()
...
fast_sim_mean %>% 
  ...
  coord_flip() +
  facet_wrap(~ dist.pot.group + phone_owner, scales = "free_y", labeller = label_both, ncol = 1, strip.position = "right") +
  labs(title = "Deworming Take-up", x = "Incentive/Signal Treatment", y = "Posterior Probability of Deworming") +
  theme(legend.position = "bottom")
```
...
fast_sim_ate %>% 
  ...
  coord_flip() +
  facet_wrap(~ dist.pot.group + phone_owner, scales = "free_y", labeller = label_both, ncol = 1, strip.position = "right") +
  labs(title = "Treatment Effect on Deworming Take-up", x = "Incentive/Signal Treatment", y = "Posterior Difference in Probability of Deworming") +
  theme(legend.position = "bottom")
```
...
fast_sim_mean %>%
  ...
  ggplot(aes(deworming_day, prob_deworm, group = str_c(dyn_treat_days, incentive_treatment))) +
  geom_line(aes(color = dyn_treat_days, linetype = fct_relabel(incentive_treatment, str_to_title))) +
  scale_x_continuous("Deworming Day", breaks = 1:12, minor_breaks = FALSE) +
  scale_color_distiller("Days Observing Signal", breaks = seq(0, 11, 2), palette = "Spectral") +
  scale_linetype_discrete("Incentive/Signal Treatment") +
  facet_grid(phone_owner ~ dist.pot.group, labeller = label_both, scales = "free_y") +
...
dyn_fast_sim_ate %>%
  filter(deworming_day <= 12,
         incentive_treatment_left %in% c("control", "ink", "bracelet"), 
         sms.treatment.2_left == "sms.control",
         dyn_treat_days_left %in% c(0, 11)) %>% 
  # ggplot(aes(deworming_day, prob_deworm_ate, color = dyn_treat_days_left)) +
  ggplot(aes(deworming_day, prob_deworm_ate, group = str_c(dyn_treat_days_left, incentive_treatment_left))) +
  # geom_point() +
  # geom_ribbon(aes(ymin = prob_deworm_diff_quant_5, ymax = prob_deworm_diff_quant_95), alpha = 0.15) +
  geom_hline(yintercept = 0, linetype = "dashed", color = alpha("black", 0.5)) +
  geom_line(aes(color = dyn_treat_days_left, linetype = fct_relabel(incentive_treatment_left, str_to_title))) +
  scale_x_continuous("Deworming Day", breaks = 1:12, minor_breaks = FALSE) +
  # scale_color_continuous("Days Observing Signal", breaks = seq(0, 11, 2)) +
  scale_color_distiller("Days Observing Signal", breaks = seq(0, 11, 2), palette = "Spectral") +
  scale_linetype_discrete("Incentive/Signal Treatment") +
  # facet_grid(incentive_treatment_left ~ dist.pot.group + phone_owner, labeller = label_both, scales = "free_y") +
  facet_grid(phone_owner ~ dist.pot.group, labeller = label_both, scales = "free_y") +
...
fast_sim_mean %>%
  filter(deworming_day <= 12, 
         phone_owner,
         dyn_treat_days %in% c(0, 11)) %>% 
  ggplot(aes(deworming_day, prob_deworm, group = str_c(dyn_treat_days, sms.treatment.2))) +
  geom_line(aes(color = dyn_treat_days, linetype = sms.treatment.2)) +
  scale_x_continuous("Deworming Day", breaks = 1:12, minor_breaks = FALSE) +
  scale_color_distiller("Days Observing Signal", breaks = seq(0, 11, 2), palette = "Spectral") +
  scale_linetype_discrete("SMS Treatment", labels = c("None", "Reminders Only", "Social Information")) +
  facet_grid(dist.pot.group ~ incentive_treatment, scales = "free_y") +
...
dyn_fast_sim_ate %>%
  filter(deworming_day <= 12,
         ...
         dyn_treat_days_left %in% c(0, 11)) %>% 
  # ggplot(aes(deworming_day, prob_deworm_ate, group = str_c(dyn_treat_days_left, incentive_treatment_left))) +
  ggplot(aes(deworming_day, prob_deworm_ate, group = str_c(dyn_treat_days_left, sms.treatment.2_left))) +
  geom_hline(yintercept = 0, linetype = "dashed", color = alpha("black", 0.5)) +
  geom_line(aes(color = dyn_treat_days_left, linetype = sms.treatment.2_left)) +
  scale_x_continuous("Deworming Day", breaks = 1:12, minor_breaks = FALSE) +
  scale_color_distiller("Days Observing Signal", breaks = seq(0, 11, 2), palette = "Spectral") +
  scale_linetype_discrete("SMS Treatment", labels = c("Reminders Only", "Social Information")) +
  facet_grid(dist.pot.group ~ incentive_treatment_left, scales = "free_y") +
  labs(title = "Daily Treatment Effect with Varying Days of SMS Treatment", 
       subtitle = "with the control (no SMS) arm as reference", y = "Posterior Difference in Probability of Deworming") +
  theme(legend.position = "bottom") 
```