...
all_ate <- bind_rows(sms_control_ate, phone_owners_ate) %>%
  mutate(hh.baseline.sample = FALSE) #, name_matched = FALSE) 
```
...
stan_data <- analysis.data %>%
  ...
  prepare_bayesian_analysis_data(
    wtp.data,
    # treatment_formula = ~ assigned.treatment * dist.pot.group * sms.treatment.2 * phone_owner * hh.baseline.sample - assigned.treatment * dist.pot.group * hh.baseline.sample * sms.treatment.2 + assigned.treatment * dist.pot.group * phone_owner * hh.baseline.sample * name_matched,
    treatment_formula = ~ assigned.treatment * dist.pot.group * sms.treatment.2 * hh.baseline.sample * phone_owner - phone_owner,
    # treatment_formula = ~ assigned.treatment * dist.pot.group * sms.treatment.2 * hh.baseline.sample * name_matched * phone_owner - phone_owner,
    all_ate = all_ate,
...
basic_model_0_fit <- sampling(basic_model_0, data = stan_data, chains = 4, iter = 100, sample_file = file.path("stanfit", "basic_model_0.csv"), control = lst(max_treedepth = 15)) # adapt_delta = 0.99, 
```
