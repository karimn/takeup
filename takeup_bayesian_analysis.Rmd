...
# sim_stratum_hazards <- basic_model_2_simple_fit %>%
#   as.data.frame(pars = c("hyper_baseline_hazard", "stratum_hazard_effect")) %>% 
#   mutate(iter_id = seq_len(n())) %>% 
#   gather(stratum_id, stratum_hazard_effect, starts_with("stratum_hazard_effect")) %>% 
  # gather(deworming_day, baseline_hazard, starts_with("hyper_baseline_hazard")) %>% 
  ...
  # mutate(stratum_hazard = baseline_hazard * stratum_hazard_effect) 
...
obs_meta_data <- dyn_stan_data$prepared_analysis_data %>% 
    transmute(stratum_id, cluster_id = new_cluster_id, obs_index, phone_owner, name_matched)

iter_cluster_parameters <- obs_meta_data %>% 
  group_by_at(vars(-obs_index)) %>% 
...
```{r, eval=FALSE}
est_fun <- partial(estimate_deworm_prob, 
                   obs_meta_data = obs_meta_data,
                   sim_stratum_hazards = sim_stratum_hazards,
                   census_covar_dm = dyn_stan_data %$% census_covar_map_dm[census_covar_id, ],
                   treatment_map = dyn_stan_data$treatment_map,
...
dyn_day_prob_sim <- obs_meta_data %>% 
  distinct(stratum_id, cluster_id) %>% 
  left_join(sim_stratum_census_covar_coef, c("stratum_id")) %>%
  left_join(sim_stratum_treatment_coef, c("stratum_id", "iter_id")) %>%
  left_join(sim_stratum_dyn_treatment_coef, c("stratum_id", "iter_id")) %>%
  right_join(sim_cluster_hazards, c("cluster_id", "iter_id")) %>% 
  filter(iter_id %in% 1:100) %>%
  ddply(.(iter_id), est_fun, 
        ...
        treatments_info = sms_control_treatments %>% filter(social_value == "control"),
        days_treated = 11)
```

```{r}
est_fun <- partial(fast_estimate_deworm_prob, 
                   sim_stratum_hazards = sim_stratum_hazards,
                   treatment_map = dyn_stan_data$treatment_map,
                   treatment_map_dm = dyn_stan_data$treatment_map_design_matrix,
                   all_treat_dyn_id = dyn_stan_data$all_treatment_dyn_id,
...
paropts <- lst(.packages = c("magrittr", "stringr", "tidyverse"),
               .export = c("fast_estimate_deworm_prob", "identify_treatment_id", "estimate_treatment_deworm_prob", "estimate_treatment_deworm_prob_dm",
                                   "obs_meta_data", "dyn_stan_data", "sim_stratum_hazards"))
```

```{r, warning=FALSE}
xx <- ddply(iter_cluster_parameters, .(iter_id), est_fun, .parallel = FALSE, .progress = "text", .paropts = paropts,
                  treatments_info = fast_treatments %>% filter(social_value == "bracelet" & sms.treatment.2 == "sms.control"), days_treated = 4)
```


```{r, eval=FALSE, warning=FALSE}
fast_sim <- ddply(iter_cluster_parameters, .(iter_id), est_fun, .parallel = FALSE, .progress = "text", .paropts = paropts,
                  treatments_info = fast_treatments %>% filter(social_value == "control" & sms.treatment.2 == "sms.control"), days_treated = 11)

fast_sim <- ddply(iter_cluster_parameters, .(iter_id), est_fun, .parallel = TRUE, .progress = "text", .paropts = paropts,
                  treatments_info = fast_treatments %>% filter(social_value != "control" | sms.treatment.2 != "sms.control"), days_treated = 0:11) %>% 
  bind_rows(fast_sim)

fast_sim <- ddply(iter_cluster_parameters, .(iter_id), est_fun, .parallel = TRUE, .progress = "text", .paropts = paropts,
                  treatments_info = fast_treatments %>% filter(sms.treatment.2 != "sms.control"), days_treated = 0:11) %>% 
  bind_rows(fast_sim)
...
fast_sim %<>% 
  ...
  bind_rows(group_by_at(., vars(iter_id, fast_treatment_id, treatment_col)) %>% 
              # slice(1) %>% 
              # mutate(deworming_day = 13, prob_deworm = sum(prob_deworm)) %>% 
              summarize(deworming_day = 13, prob_deworm = sum(prob_deworm)) %>%
              ungroup()) 
...
fast_sim_mean %>% 
  filter(deworming_day == 13, 
         dyn_treat_days == 11) %>% 
  ggplot(aes(incentive_treatment, prob_deworm, color = sms.treatment.2)) +
  geom_pointrange(aes(ymin = prob_deworm_quant_5, ymax = prob_deworm_quant_95), position = position_dodge(width = 0.5)) +
  geom_text_repel(aes(label = sprintf("%.2f", prob_deworm), color = NULL), nudge_x = -0.5, size = 3, segment.color = NA) +
  coord_flip() +
...
fast_sim_ate %>% 
  ...
  ggplot(aes(incentive_treatment_left, prob_deworm_ate, color = sms.treatment.2_left, shape = ref)) +
  geom_hline(yintercept = 0, linetype = "dotted") +
  geom_pointrange(aes(ymin = prob_deworm_diff_quant_5, ymax = prob_deworm_diff_quant_95), position = position_dodge(width = 0.4)) +
  geom_text_repel(aes(label = sprintf("%.2f", prob_deworm_ate), color = NULL), nudge_x = -0.5, size = 3, segment.color = NA) +
  coord_flip() +
  facet_wrap(~ dist.pot.group + phone_owner, scales = "free_y", labeller = label_both, ncol = 1, strip.position = "right") +
  theme(legend.position = "bottom")
```
...
fast_sim_mean %>%
  filter(deworming_day <= 12, incentive_treatment %in% c("ink", "bracelet"), sms.treatment.2 == "sms.control") %>% 
  ggplot(aes(deworming_day, prob_deworm, color = dyn_treat_days)) +
  geom_point() +
  geom_line(aes(group = dyn_treat_days)) +
  scale_x_continuous(breaks = 1:12, minor_breaks = FALSE) +
  scale_color_continuous(breaks = seq(0, 11, 2)) +
  facet_grid(incentive_treatment ~ dist.pot.group + phone_owner, labeller = label_both, scales = "free_y") +
  theme(legend.position = "bottom")
```
...
dyn_fast_sim_ate %>%
  ...
  geom_point() +
  geom_line(aes(group = dyn_treat_days_left)) +
  scale_x_continuous(breaks = 1:12, minor_breaks = FALSE) +
