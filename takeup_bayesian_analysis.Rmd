...
dyn_dyn_treatment_maps <- lst(formula = ~ (signal_observed + reminder_info_stock * day_after_reminder_info) * phone_owner, 
                             trends = tibble(signal_observed = 0:11,
                                             reminder_info_stock = rep(0:5, each = 2),
                                             day_after_reminder_info = c(0, rep(0:1, 5), 0))) %>% 
...
dyn_stan_data <- prepare_bayesian_analysis_data(
  ...
  # dynamic_treatment_map = lst(formula = ~ (signal_observed + reminder_info_stock * day_after_reminder_info) * phone_owner, 
  #                             trends = tibble(signal_observed = 0:11,
  #                                             reminder_info_stock = rep(0:5, each = 2),
  #                                             day_after_reminder_info = c(0, rep(0:1, 5), 0))),
  ...
  all_ate = dyn_all_ate %>% filter(!name_matched),
