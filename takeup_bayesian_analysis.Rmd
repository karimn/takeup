...
sim_stratum_hazards <- basic_model_2_simple_fit %>% 
  ...
  mutate_at(vars(stratum_id, deworming_day), funs(str_extract(., "\\d+") %>% as.integer())) %>% 
...
dyn_day_sim <- obs_meta_data %>% 
  ...
  filter(iter_id %in% 1:10) %>%
  ddply(.(iter_id), function(iter_parameters, obs_meta_data, 
                             census_covar_dm, 
                             treatment_dm, 
                             full_dyn_treatment_dm, 
                             dyn_treatment_dm = matrix(0, nrow = nrow(treatment_dm), ncol = ncol(full_dyn_treatment_dm))) {
    dyn_treatment_dm %>% 
      set_colnames(str_c("dyn_treatment_", 1:ncol(.))) %>% 
      as_tibble() %>% 
      bind_cols(treatment_dm, .) %>% 
      ddply(.(all_treatment_id), function(dm_datarow, iter_est, full_dyn_treatment_dm) {
        ...
        dyn_dm_row <- select(dm_datarow, starts_with("dyn_treatment_")) %>% 
          ...
          multiply_by(full_dyn_treatment_dm)
        ...
        applicable_obs %>% 
          ...
          mutate(log_kappa_static_treatment = 
                   (as.matrix(beta_params) %*% unlist(select(static_dm_row, -matches("private_valuebracelet")))) + 
                   (as.matrix(select(beta_params, matches("private_valuecalendar"))) %*% unlist(select(static_dm_row, matches("private_valuebracelet")))),
                 log_kappa_static_treatment = c(log_kappa_static_treatment)) %>% 
          left_join(sim_stratum_hazards, c("iter_id", "stratum_id")) %>% 
          mutate(cluster_hazard = stratum_hazard * cluster_hazard_effect,
                 log_kappa_dyn_treatment = rowSums(dyn_dm_row[deworming_day, ] * select(., matches("^stratum_dynamic_treatment_coef_\\d+"))),
          ...
          group_by(obs_index) %>% 
          mutate(day_prob_deworm = m1_alpha * accumulate(lag(alpha, default = 1), multiply_by)) %>% 
          group_by(deworming_day) %>% 
          summarize_at(vars(m1_alpha, day_prob_deworm), mean) %>% 
          ungroup() %>% 
          add_row(deworming_day = 13, day_prob_deworm = 1 - sum(.$day_prob_deworm))
      },
