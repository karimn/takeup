  ...
  takeup_summ_data %>%
    inner_data_preparer() %>% { 
      ...
      if (!is_null(takeup_data)) {
        plot_obj <- plot_obj + geom_violin(aes(y = wtd_iter_est), draw_quantiles = c(0.25, 0.5, 0.75), stroke = 0.35, color = "white", fill = "darkgrey", data = takeup_data) 
      }
...
plot_dyn_ate <- function(ate_summ_data, ate_data = NULL, combiner = NULL, data_preparer = function(data) data) {
  ...
  ate_summ_data %>% 
    inner_data_preparer() %>% { 
      ...
      if (!is_null(ate_data)) {
        plot_obj <- plot_obj + geom_violin(aes(y = wtd_iter_est), draw_quantiles = c(0.25, 0.5, 0.75), stroke = 0.35, color = "white", fill = "darkgrey", data = ate_data)
      }
