...
cl <- parallel::makeCluster(max(1, parallel::detectCores()))
...
sms_ctrl_est_deworming_takeup %>%
  ...
  mutate(incentive_treatment = fct_relabel(incentive_treatment, . %>% str_replace( "-?control-?", "") %>% str_to_title())) %>% 
...
xx <- iter_cluster_parameters %>% 
  filter(iter_id == 1) %>% 
  ddply(.(iter_id), fast_est_fun, .parallel = TRUE, .progress = "text", .paropts = paropts,
                  treatments_info = fast_treatments[1, ], days_treated = c(0, 11))
```
...
fast_sim <- ddply(iter_cluster_parameters, .(iter_id), fast_est_fun, .parallel = TRUE, .progress = "text", .paropts = paropts,
                  treatments_info = fast_treatments %>% filter(social_value != "control" | sms.treatment.2 != "sms.control"), days_treated = c(0, 11)) %>% 
  bind_rows(fast_sim)
...
fast_sim_mean %>%
  ...
  geom_line(aes(y = prob_deworm, group = str_c(dyn_treat_days, incentive_treatment), color = dyn_treat_days, linetype = fct_relabel(incentive_treatment, str_to_title))) +
  geom_ribbon(aes(ymin = `0`, ymax = `11`, group = incentive_treatment), alpha = 0.2,
              data = . %>%
                filter(incentive_treatment != "control") %>%
                select(deworming_day, dist.pot.group, phone_owner, incentive_treatment, dyn_treat_days, prob_deworm) %>%
                spread(dyn_treat_days, prob_deworm)) +
...
dyn_fast_sim_ate %>%
  ...
  geom_line(aes(y = prob_deworm_ate, color = dyn_treat_days_left, group = str_c(dyn_treat_days_left, incentive_treatment_left), linetype = fct_relabel(incentive_treatment_left, str_to_title))) +
  geom_ribbon(aes(ymin = `0`, ymax = `11`, group = incentive_treatment_left), alpha = 0.2,
              data = . %>%
                select(deworming_day, dist.pot.group, phone_owner, incentive_treatment_left, dyn_treat_days_left, prob_deworm_ate) %>%
                spread(dyn_treat_days_left, prob_deworm_ate)) +
  scale_x_continuous("Deworming Day", breaks = 1:12, minor_breaks = FALSE) +
  scale_color_distiller("Days Observing Signal", breaks = seq(0, 11, 2), palette = "Spectral") +
  scale_linetype_discrete("Incentive/Signal Treatment") +
  facet_grid(phone_owner ~ dist.pot.group, labeller = label_both, scales = "free_y") +
...
fast_sim_mean %>%
  filter(deworming_day <= 12, 
         ...
         dyn_treat_days %in% c(0, 11)) %>% 
  ggplot(aes(x = deworming_day)) +
  geom_line(aes(y = prob_deworm, group = str_c(dyn_treat_days, sms.treatment.2), color = dyn_treat_days, linetype = sms.treatment.2)) +
  geom_ribbon(aes(ymin = `0`, ymax = `11`, group = sms.treatment.2), alpha = 0.2,
              data = . %>%
                filter(incentive_treatment != "control") %>%
                select(deworming_day, dist.pot.group, phone_owner, incentive_treatment, sms.treatment.2, dyn_treat_days, prob_deworm) %>%
                spread(dyn_treat_days, prob_deworm)) +
  scale_x_continuous("Deworming Day", breaks = 1:12, minor_breaks = FALSE) +
  scale_color_distiller("Days Observing Signal", breaks = seq(0, 11, 2), palette = "Spectral") +
...
dyn_fast_sim_ate %>%
  ...
  ggplot(aes(x = deworming_day)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = alpha("black", 0.5)) +
  geom_line(aes(y = prob_deworm_ate, color = dyn_treat_days_left, group = str_c(dyn_treat_days_left, sms.treatment.2_left), linetype = sms.treatment.2_left)) +
  geom_ribbon(aes(ymin = `0`, ymax = `11`, group = sms.treatment.2_left), alpha = 0.2,
              data = . %>%
                select(deworming_day, dist.pot.group, phone_owner, incentive_treatment_left, sms.treatment.2_left, dyn_treat_days_left, prob_deworm_ate) %>%
                spread(dyn_treat_days_left, prob_deworm_ate)) +
  scale_x_continuous("Deworming Day", breaks = 1:12, minor_breaks = FALSE) +
  scale_color_distiller("Days Observing Signal", breaks = seq(0, 11, 2), palette = "Spectral") +
