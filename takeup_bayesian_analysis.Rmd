...
dyn_stan_data <- analysis.data %>%
  ...
  prepare_bayesian_analysis_data(
    ...
    dynamic_treatment_map = tibble(signal_observed = 0:11),
    ...
    scale_df = 7,
    scale_sigma = 0.1,
    scale_sigma_dynamic = 0.1,
   
    coef_df = 7,
    coef_sigma = 0.1,
    coef_sigma_dynamic = 0.1,
...
basic_model_2_simple_fit <- dyn_stan_data %>% 
  sampling(basic_model_2_simple, data = ., #init = .$dynamic_initializer,
           chains = 1, iter = 10, control = lst(max_treedepth = 15), # adapt_delta = 0.99, 
