...
dyn_dyn_treatment_maps <- lst(formula = ~ (poly(days_observed, 2, raw = TRUE) * (signal_observed + incentive_shift) + 
                                             poly(days_sms, 2, raw = TRUE) * reminder_info_stock) * dyn_dist_pot * phone_owner,
                              trends = tibble(days_observed = 0:11,
                                              days_sms = rep(0:5, each = 2)),
                              scale = TRUE) %>% 
