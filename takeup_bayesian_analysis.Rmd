...
nonparam_dyn_treatment_maps <- lst(formula = ~ days_observed * incentive_shift * dyn_dist_pot,
                                   trends = tibble(days_observed = (0:11)/11),
                                   scale = FALSE) %>% 
  prepare_dynamic_treatment_maps(nonparam_dyn_analysis_data)
...
nonparam_dyn_static_treatment_map <- nonparam_dyn_analysis_data %>% 
  ...
  data_grid(private_value, social_value, incentive_shift, dist.pot.group, dyn_dist_pot, phone_owner) %>%
...
nonparam_dyn_stan_data <- prepare_bayesian_analysis_data(
  ...
  # treatment_formula = ~ private_value + dist.pot.group, subgroup_col = NULL,
  subgroup_col = "phone_owner",
  drop_intercept_from_dm = FALSE,
  ...
  all_ate = dyn_sms_control_ate %>% 
    select(starts_with("private_value"), starts_with("social_value"), starts_with("phone_owner"), starts_with("incentive_shift"), contains("dist.pot.group"), contains("dyn_dist")) %>% slice(2),
  # %>% 
...
model_3_fit <- nonparam_dyn_stan_data %>% 
  sampling(model_3_gp_nonparam, data = ., 
           chains = 7, iter = 400, control = lst(max_treedepth = 15), #adapt_delta = 0.99), 
