...
data {
  ...
  int<lower = 1> num_incentive_effects;
  int<lower = 1> num_dist_effects;
  int<lower = 1> num_incentive_dist_effects;
  ...
  //int<lower = 1> missing_treatment_sizes[num_treatments + 1];
 
  //matrix[num_obs, num_treatments] treatment_design_matrix;
  //matrix[num_missing, num_treatments] missing_treatment_design_matrix;
  
  //matrix[num_obs, num_dist_effects] dist_design_matrix;
  //matrix[num_missing, num_dist_effects] missing_dist_design_matrix;
  //matrix[num_obs, num_treatment_dist_effects] treatment_dist_design_matrix;
  //matrix[num_missing, num_treatment_dist_effects] missing_treatment_dist_design_matrix;
...
transformed data {
  ...
  // Column start positions hardcoded (danger!)
  matrix[num_obs, num_incentive_effects] incentive_design_matrix = block(all_treatment_design_matrix, 1, 1, num_obs, num_incentive_effects);
  matrix[num_obs, num_dist_effects] dist_design_matrix = block(all_treatment_design_matrix, 1, 4, num_obs, num_dist_effects);
  matrix[num_obs, num_incentive_dist_effects] incentive_dist_design_matrix = block(all_treatment_design_matrix, 1, 5, num_obs, num_incentive_dist_effects);
  ...
  vector[num_incentive_effects] mu_beta = rep_vector(0, num_incentive_effects);
  vector[num_dist_effects] mu_dist = rep_vector(0, num_dist_effects);
  vector[num_incentive_dist_effects] mu_incentive_dist = rep_vector(0, num_incentive_dist_effects);
}
...
parameters {
  ...
  vector[num_incentive_effects] beta[num_strata];
  vector[num_dist_effects] dist_effects[num_strata];
  vector[num_incentive_dist_effects] incentive_dist_effects[num_strata];
  vector[num_clusters] cluster_effects;
  
  // Scale hyperparameters for betas 
  
  // corr_matrix[num_treatments] Omega_beta;
  vector<lower = 0>[num_incentive_effects] tau_beta;
  vector<lower = 0>[num_dist_effects] tau_dist;
  vector<lower = 0>[num_incentive_dist_effects] tau_incentive_dist;
  real<lower = 0> tau_cluster_effect;
}

transformed parameters {
  cov_matrix[num_incentive_effects] Sigma_beta = diag_matrix(tau_beta);
  cov_matrix[num_dist_effects] Sigma_dist = diag_matrix(tau_dist);
  cov_matrix[num_incentive_dist_effects] Sigma_incentive_dist = diag_matrix(tau_incentive_dist);
}
...
model {
  ...
  tau_dist ~ cauchy(0, 2.5); 
  tau_incentive_dist ~ cauchy(0, 2.5); 
  ...
  dist_effects ~ multi_normal(mu_dist, Sigma_dist); // For now assuming no correlation between effects
  incentive_dist_effects ~ multi_normal(mu_incentive_dist, Sigma_incentive_dist); // For now assuming no correlation between effects
 
  {
    ...
    for (strata_index in 1:num_strata) {
      ...
      link_model[strata_pos:stratum_end] = 
        incentive_design_matrix[strata_pos:stratum_end] * beta[strata_index] +
        dist_design_matrix[strata_pos:stratum_end] * dist_effects[strata_index] +  
        incentive_dist_design_matrix[strata_pos:stratum_end] * incentive_dist_effects[strata_index];  
...
/*
  ...
  real<lower = 0> takeup_proportion[num_treatments + 1];
  
  {
    ...
    for (treatment_index in 1:(num_treatments + 1)) {
      ...
      int treatment_end = treatment_pos + curr_treatment_size - 1;
      int missing_treatment_end = missing_treatment_pos + curr_missing_treatment_size - 1;
      
      takeup_proportion[treatment_index] = 
        sum(dewormed_any[treatment_id[treatment_pos:treatment_end]]) + 
        sum(missing_dewormed_any[missing_treatment_id[missing_treatment_pos:missing_treatment_end]]);
        
      takeup_proportion[treatment_index] = takeup_proportion[treatment_index] / (curr_treatment_size + curr_missing_treatment_size);
      
      treatment_pos = treatment_pos + curr_treatment_size;
      missing_treatment_pos = missing_treatment_pos + curr_missing_treatment_size;
    }
  }
  */
}
...
stan_data <- analysis.data %>% 
  ...
  (function(prepared_analysis_data) {
    treatment_map <- data_grid(prepared_analysis_data, assigned.treatment, dist.pot.group) %>% 
      mutate(
    ...
    treatment_map_design_matrix <- model_matrix(treatment_map, ~ assigned.treatment * dist.pot.group) %>% 
      magrittr::extract(, -1)
    
    # treatment_grid <- prepared_analysis_data %>% {
    #   left_join(data_grid(., KEY.individ, assigned.treatment, dist.pot.group), 
    #             select(., KEY.individ, new_cluster_id, county_id, county_dist_stratum_id), 
    #             "KEY.individ") 
    # }
    # 
    # missing_treatment <- anti_join(treatment_grid, prepared_analysis_data, c("KEY.individ", "assigned.treatment", "dist.pot.group")) %>% 
    #   left_join(treatment_map, c("assigned.treatment", "dist.pot.group"))
    ...
    lst(
      ...
      #   magrittr::extract(, -1),
      num_incentive_effects = n_distinct(prepared_analysis_data$assigned.treatment) - 1, #ncol(treatment_design_matrix),
      
      # dist_design_matrix = model_matrix(prepared_analysis_data, ~ dist.pot.group) %>% 
      #   magrittr::extract(, -1),
      num_dist_effects = n_distinct(prepared_analysis_data$dist.pot.group) - 1, # ncol(dist_design_matrix),
      
      # treatment_dist_design_matrix = model_matrix(prepared_analysis_data, ~ assigned.treatment:dist.pot.group * dist.pot.group) %>% 
      #   magrittr::extract(, -seq_len(num_treatments + num_dist_effects + 1)),
      num_incentive_dist_effects = num_incentive_effects * num_dist_effects, # ncol(treatment_dist_design_matrix),
      
      num_all_treatment_coef = num_incentive_effects + num_dist_effects + num_incentive_dist_effects,
    
      # missing_treatment_design_matrix = model_matrix(missing_treatment, ~ assigned.treatment) %>% 
      #   magrittr::extract(, -1),
      # missing_dist_design_matrix = model_matrix(missing_treatment, ~ dist.pot.group) %>% 
      #   magrittr::extract(, -1),
      # missing_treatment_dist_design_matrix = model_matrix(missing_treatment, ~ assigned.treatment:dist.pot.group * dist.pot.group) %>% 
      #   magrittr::extract(, -seq_len(num_treatments + num_dist_effects + 1)),
      
      treatment_id = prepared_analysis_data %>% 
        mutate(index = seq_len(n())) %>% 
        arrange(all_treatment_id, index) %$% 
        index,
      ...
      num_obs = length(obs_treatment), # nrow(treatment_design_matrix),
      # num_missing = num_obs * (num_all_treatments - 1), #nrow(missing_treatment_design_matrix),
      
      treatment_sizes = count(prepared_analysis_data, all_treatment_id) %>% arrange(all_treatment_id) %$% n,
      # missing_treatment_sizes = count(missing_treatment, treatment_id) %>% arrange(treatment_id) %$% n,
      ...
      # missing_strata_sizes = count(missing_treatment, county_id) %>% arrange(county_id) %$% n 
    )
  }) 
```
...
print(basic_model_0_fit, pars = c("cluster_effects", "missing_dewormed_any"), include = FALSE)
```
