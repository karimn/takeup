...
model_3_fit <- dir("stanfit", pattern = str_interp("model_3_${dyn_fit_version}_[1-4].csv"), full.names = TRUE) %>% 
  read_stan_csv()
...
param_dyn_stan_data <- dyn_stan_data
```
...
```{r summarize-est}
prepare_est_deworming <- function(est_data, outcome_var, daily = FALSE, subgroups = NULL) { 
  ...
  est_data %>%
    ...
    summarize_at(vars("wtd_iter_est"), funs(mean_est = mean,
                                         ub_90 = quantile(., 0.95),
                                         ub_95 = quantile(., 0.975),
                                         lb_90 = quantile(., 0.05),
                                         lb_95 = quantile(., 0.025))) %>% 
...
prepare_est_deworming_takeup_dyn <- function(est_data, ...) { 
  prepare_est_deworming(est_data, "iter_takeup", ...) %>% 
...
est_deworming_takeup_dyn_all %>%
  ...
  mutate_at(vars(starts_with("incentive_treatment_static")), str_to_title) %>% 
