...
```{r model-data}
treatment_map <- data_grid(param_analysis_data, private_value, social_value, dist.pot.group, sms.treatment.2, hh.baseline.sample, phone_owner) %>% 
...
```{r param-model-data}
param_analysis_data <- analysis.data %>% 
...
treatment_map <- data_grid(param_analysis_data, private_value, social_value, dist.pot.group, sms.treatment.2, hh.baseline.sample, phone_owner) %>% 
  filter(phone_owner | sms.treatment.2 == "sms.control",
         (private_value == "control" & social_value == "control") | sms.treatment.2 != "reminder.only",
         # private_value != "control" | social_value %in% c("control", "ink"),
         private_value != "calendar" | social_value == "control",
         private_value != "bracelet" | social_value == "bracelet")

param_stan_data <- param_analysis_data %>%
  filter(!name_matched | sms.treatment.2 == "sms.control") %>% 
  prepare_bayesian_analysis_data(
    wtp.data, 
