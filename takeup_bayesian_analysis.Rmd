...
scale_covar <- function (covar_data) {
  ...
  is_factor_col <- map(covar_data, ~ if(is.factor(.)) rep(TRUE, nlevels(.) - 1) else FALSE) %>% 
    unlist()
...
basic_model_0_fit <- sampling(basic_model_0, data = stan_data, sample_file = file.path("stanfit", "basic_model_0.csv")) #, control = lst(adapt_delta = 0.99, max_treedepth = 15))
