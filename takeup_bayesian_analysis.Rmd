...
fast_sim_mean %>%
  filter(deworming_day <= 12, 
         # incentive_treatment %in% c("control", "ink", "bracelet social", "bracelet"),
...
$$
\Pr[Y_{im}(t) = 1|\theta] = 1 - \exp\left[-\kappa^\text{static effect}(t)\cdot\kappa^\text{dynamic effect}(t)\cdot\lambda_m\right]
$$
so we can "shutdown" learning/observation by forcing $\kappa^\text{dynamic effect} = 1$. 
...
fast_sim_mean %>%
  ...
  geom_line(aes(y = exp(log_kappa_dyn_treat), 
                ...
                color = fct_relabel(incentive_treatment, str_to_title))) +
  scale_x_continuous("Deworming Day", breaks = 1:12, minor_breaks = FALSE) +
  scale_color_discrete("Incentive/Signal Treatment") +
  facet_grid(phone_owner ~ dist.pot.group, labeller = label_both, scales = "free_y") +
...
bind_rows(observe = fast_sim_ate %>% filter(incentive_treatment_right == "control", sms.treatment.2_right == "sms.control"),
          no_observe = dyn_fast_sim_ate %>% 
            filter(deworming_day == 13, 
                   ...
                   incentive_treatment_left %in% c("ink", "bracelet") | sms.treatment.2_left != "sms.control"),
          .id = "obs_type") %>% 
