...
data {
  ...
  int<lower = 1> treatment_sizes[num_treatments + 1];
  ...
  
  {
    ...
    for (treatment_index in 1:(num_treatments + 1)) {
      ...
      takeup_proportion[treatment_index] = 
        sum(dewormed_any[treatment_id[treatment_pos:treatment_end]]) + sum(missing_dewormed_any[missing_treatment_id[missing_treatment_pos:missing_treatment_end]]);
...
stan_data <- analysis.data %>% 
  ...
  (function(prepared_analysis_data) {
    ...
    missing_treatment <- anti_join(treatment_grid, prepared_analysis_data, c("KEY.individ", "assigned.treatment")) %>% 
      left_join(treatment_map, c("assigned.treatment"))
    ...
    lst(
      ...
      treatment_id = prepared_analysis_data %>% 
        ...
        arrange(treatment_id, index) %$% 
        index,
      missing_treatment_id = missing_treatment %>% 
        mutate(index = seq_len(n())) %>% 
        arrange(treatment_id, index) %$% 
        index,
      treatment_sizes = count(prepared_analysis_data, treatment_id) %>% arrange(treatment_id) %$% n,
