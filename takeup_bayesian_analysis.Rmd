...
stan_data <- analysis.data %>% 
  ...
  mutate_at(vars(county_dist_stratum, county_dist_mon_stratum, county, stratum, # county_phone_owner_stratum,
            ...
            funs(id = as.integer)) %>% 
  prep_data_arranger() %>% 
  prepare_bayesian_analysis_data(treatment_formula = ~ assigned.treatment * sms.treatment.2 * phone_owner - assigned.treatment * sms.treatment.2 + assigned.treatment * name_matched * phone_owner,
