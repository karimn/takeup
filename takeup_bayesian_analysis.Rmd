...
dyn_fast_sim_ate %>%
  filter(deworming_day <= 12,
         phone_owner,
         sms.treatment.2_left != "sms.control" | incentive_treatment_left %in% c("ink", "bracelet"),
         dyn_treat_days_left %in% c(0, 11)) %>% 
  mutate(obs_type = ifelse(incentive_treatment_left %in% c("control", "calendar") & sms.treatment.2_left == "sms.control", 0, dyn_treat_days_left) %>% 
           factor(levels = c(0, 11), labels = c("no_obs", "full_obs"))) %>% 
  ggplot(aes(x = deworming_day)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = alpha("black", 0.5)) +
