  ...
  takeup_summ_data %>%
    inner_data_preparer() %>% { 
      ...
      if (!is_null(takeup_data)) {
        plot_obj <- plot_obj + geom_violin(aes(y = wtd_iter_est), draw_quantiles = c(0.25, 0.5, 0.75), stroke = 0.35, color = "white", fill = "darkgrey", data = inner_data_preparer(takeup_data)) 
      }
...
plot_dyn_ate <- function(ate_summ_data, ate_data = NULL, data_preparer = function(data) data) {
  inner_data_preparer <- . %>% 
    data_preparer() %>% 
    mutate_at(vars(starts_with("incentive_treatment_static")), funs(fct_relabel(., str_to_title))) 
  ...
  ate_summ_data %>% 
    inner_data_preparer() %>% { 
      ...
      if (!is_null(ate_data)) {
        plot_obj <- plot_obj + geom_violin(aes(y = wtd_iter_est), draw_quantiles = c(0.25, 0.5, 0.75), stroke = 0.35, color = "white", fill = "darkgrey", data = inner_data_preparer(ate_data)) 
      }
