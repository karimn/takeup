...
est_deworming_takeup_ate %<>% 
  filter(sms.treatment.2_right == "sms.control",
...
phone_est_deworming_takeup_ate <- est_deworming_takeup_ate %>% 
  group_by_at(vars(matches("((private|social)_value|sms.treatment.2)_(left|right)"), phone_owner_left, iter_id)) %>% 
  summarize(wtd_sim_outcome = weighted.mean(sim_outcome, treatment_size_left)) %>% 
  ungroup() %>% 
  group_by_at(vars(matches("((private|social)_value|sms.treatment.2)_(left|right)"), phone_owner_left)) %>% 
...
est_deworming_takeup_ate %<>% 
  group_by_at(vars(matches("((private|social)_value|sms.treatment.2)_(left|right)"), iter_id)) %>% 
  summarize(wtd_sim_outcome = weighted.mean(sim_outcome, treatment_size_left)) %>% 
  ungroup() %>% 
  group_by_at(vars(matches("((private|social)_value|sms.treatment.2)_(left|right)"))) %>% 
  summarize(mean_ate = mean(wtd_sim_outcome),
            ub_90 = quantile(wtd_sim_outcome, 0.95),
            ub_95 = quantile(wtd_sim_outcome, 0.975),
            lb_90 = quantile(wtd_sim_outcome, 0.05),
...
est_deworming_takeup %<>% 
  group_by_at(vars(matches("((private|social)_value|sms.treatment.2)"), iter_id)) %>% 
  summarize(wtd_sim_outcome = weighted.mean(sim_outcome, treatment_size)) %>% 
  ungroup() %>% 
  group_by_at(vars(matches("((private|social)_value|sms.treatment.2)"))) %>% 
...
```{r static-model-ate-plot, fig.height=5, fig.width=10}
sms_ctrl_est_deworming_takeup_ate %>%
  ...
  ggplot(aes(incentive_treatment_left, mean_ate, color = sms.treatment.2_left)) +
  geom_pointrange(aes(ymin = lb_95, ymax = ub_95), size = 0.75, position = position_dodge(width = 0.3)) +
  geom_linerange(aes(ymin = lb_90, ymax = ub_90), size = 1.75, position = position_dodge(width = 0.3)) +
...
```{r static-model-takeup, fig.height=4, fig.width=8}
sms_ctrl_est_deworming_takeup %>%
  ...
  geom_pointrange(aes(ymin = lb_95, ymax = ub_95), size = 0.75, position = position_dodge(width = 0.5)) +
  geom_linerange(aes(ymin = lb_90, ymax = ub_90), size = 1.75, position = position_dodge(width = 0.5)) +
