...
```{r param-model-data-no-sms}
no_sms_param_stan_data <- analysis.data %>%
  mutate(private_value = fct_collapse(assigned.treatment, control = c("control", "ink")),
         social_value = fct_collapse(assigned.treatment, control = c("control", "calendar"))) %>% 
  # filter(!name_matched, sms.treatment.2 == "sms.control") %>%
  filter(sms.treatment.2 == "sms.control") %>%
...
```{r, eval=FALSE}
basic_model_1_fit <- dir("stanfit", pattern = "basic_model_1_10_[1-8].csv") %>% 
  file.path("stanfit", .) %>% 
  read_stan_csv()
```
...
sms_ctrl_est_deworming_takeup %>%
  ...
  ggplot(aes(incentive_treatment, mean_takeup_prop)) + 
  geom_pointrange(aes(ymin = lb_95, ymax = ub_95), size = 0.75) +
  geom_linerange(aes(ymin = lb_90, ymax = ub_90), size = 1.75) +
  scale_y_continuous(breaks = seq(0, 1, 0.1)) +
  facet_grid(phone_owner ~ dist.pot.group + monitored, labeller = label_both)
```
...
```{r, eval=FALSE}
basic_model_2_simple_fit <- read_stan_csv(dir("stanfit", pattern = "basic_model_2_simple_6_[1-8].csv", full.names = TRUE))
```
