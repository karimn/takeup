...
basic_model_1_fit <- sampling(basic_model_1, data = param_stan_data, chains = 8, iter = 200, sample_file = file.path("stanfit", "basic_model_1_1.csv"), control = lst(max_treedepth = 15)) # adapt_delta = 0.99, 
```

```{r, eval=FALSE}
basic_model_1_fit <- dir("stanfit", pattern = "basic_model_1_2_..csv") %>% 
  file.path("stanfit", .) %>% 
  read_stan_csv()
```
