...
stan_data <- analysis.data %>% 
  ...
  (function(prepared_analysis_data) {
    treatment_map <- data_grid(prepared_analysis_data, assigned.treatment, dist.pot.group, name_matched) %>% #, hh.baseline.sample) %>% 
      mutate(
    ...
    treatment_map_design_matrix <- model_matrix(treatment_map, ~ assigned.treatment * dist.pot.group + name_matched) %>% # + hh.baseline.sample) %>% 
      magrittr::extract(, -1)
    ...
    prepared_analysis_data %<>% 
      left_join(treatment_map, c("assigned.treatment", "dist.pot.group", "name_matched"))
