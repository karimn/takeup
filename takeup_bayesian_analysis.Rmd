...
dyn_fit_version <- "param_13"

nonparam_dyn_fit_version <- "hazard_1"
static_fit_ver <- 13
```
...
wtp_model_0_fit <- sampling(wtp_model_0, data = wtp_stan_data, chains = 8, iter = 800, sample_file = file.path("stanfit", "wtp_model_2.csv")) 
```
...
* $Z_w^p(z^p) = \mathbbm{1}\{z^p = w\}$
* $Z_w^r(z^p) = \mathbbm{1}\{z^r = w\}$
* $S_w(s) = \mathbbm{1}\{s = w\}$
...
static_analysis_data <- analysis.data %>%
  mutate(private_value = fct_collapse(assigned.treatment, control = c("control", "ink"), calendar = c("calendar", "bracelet")),
         social_value = fct_collapse(assigned.treatment, control = c("control", "calendar"))) %>% 
  filter(!name_matched | sms.treatment.2 == "sms.control") # There are a couple of SMS observations that are name-checked. Drop them. 

static_treatment_map <- static_analysis_data %>% 
  # data_grid(private_value, social_value, dist.pot.group, sms.treatment.2, hh.baseline.sample, phone_owner, name_matched) %>% 
  data_grid(private_value, social_value, dist.pot.group, sms.treatment.2, phone_owner, name_matched) %>% 
...
param_stan_data <- static_analysis_data %>%  
  prepare_bayesian_analysis_data(
    ...
    treatment_formula = ~ (private_value + social_value * sms.treatment.2 * dist.pot.group) * phone_owner * name_matched,
...
gen_lkj_initializer <- function(stan_data_list) {
  function() {
    lst(
      stratum_beta_corr_mat = with(stan_data_list, rethinking::rlkjcorr(1, num_all_treatment_coef, lkj_df)),
      cluster_beta_corr_mat = with(stan_data_list, rethinking::rlkjcorr(1, num_all_treatment_coef, lkj_df)),
      stratum_census_covar_corr_mat = with(stan_data_list, rethinking::rlkjcorr(1, num_census_covar_coef, lkj_df)),
      cluster_census_covar_corr_mat = with(stan_data_list, rethinking::rlkjcorr(1, num_census_covar_coef, lkj_df)),
      
      L_stratum_beta_corr_mat = t(chol(stratum_beta_corr_mat)), # chol() returns the upper triangle while Stan uses the lower
      L_cluster_beta_corr_mat = t(chol(cluster_beta_corr_mat)),
      L_stratum_census_covar_corr_mat = t(chol(stratum_census_covar_corr_mat)),
      L_cluster_census_covar_corr_mat = t(chol(cluster_census_covar_corr_mat))
    )
  }
}

# basic_model_1 <- stan_model(file = "takeup_model_1.stan", model_name = "model_1")
basic_model_1 <- stan_model(file = "takeup_model_1_deep_multilevel.stan", model_name = "model_1_deep_multilevel")
```

```{r basic-model-1, eval=FALSE}
basic_model_1_fit <- param_stan_data %>% 
  sampling(basic_model_1, 
           data = ., 
           chains = 8, iter = 800, control = lst(max_treedepth = 15),
           init = gen_lkj_initializer(.), 
           sample_file = file.path("stanfit", str_interp("basic_model_1_${static_fit_ver}.csv"))) 

save(param_stan_data, file = file.path("stan_analysis_data", str_interp("model_1_${static_fit_ver}.RData")))
```

```{r basic-model-1-debug, eval=FALSE}
basic_model_1_fit_debug <- param_stan_data %>% 
  sampling(basic_model_1, 
           data = ., 
           chains = 2, iter = 10, control = lst(max_treedepth = 15), 
           init = gen_lkj_initializer(.)) 
```
...
est_deworming_takeup_ate %<>% 
  ...
  mutate(monitored_all_treatment_id_right = select(., ends_with("_right"), -ends_with("_id_right"), -ends_with("size_right")) %>% 
           set_colnames(str_replace(names(.), "_right", "")) %>% 
           mutate(name_matched = FALSE) %>% 
           left_join(param_stan_data$treatment_map) %>% 
           pull(all_treatment_id))

est_deworming_takeup <- basic_model_1_fit %>% 
  as.data.frame(pars = "treatment_cell_takeup") %>% 
  mutate(iter_id = seq_len(n())) %>% 
  gather(treatment_rank, takeup, -iter_id) %>% 
  ...
  mutate(all_treatment_id = param_stan_data$ate_treatments[treatment_rank],
         ...
         observed_treatment_size = param_stan_data$observed_treatment_sizes[treatment_rank], 
         observed_takeup_prop = param_stan_data$observed_takeup_total[treatment_rank] / observed_treatment_size) 
...
est_deworming_takeup_ate %<>% 
  left_join(select(est_deworming_takeup, all_treatment_id, iter_id, takeup), c("monitored_all_treatment_id_right" = "all_treatment_id", "iter_id"))

prepare_est_ate <- function(est_ate_data, subgroups = NULL) { 
  est_ate_data %>% 
    group_by_at(c(vars(matches("((private|social)_value|sms.treatment.2)_(left|right)"), iter_id), subgroups)) %>% 
    summarize(ate = weighted.mean(sim_outcome, treatment_size_left), 
              ate_percent = weighted.mean(sim_outcome / takeup, treatment_size_left)) %>% 
    ungroup() %>% 
    group_by_at(c(vars(matches("((private|social)_value|sms.treatment.2)_(left|right)")), subgroups)) %>% 
    summarize_at(vars(ate, ate_percent), funs(mean, 
                                              ub_90 = quantile(., 0.95),
    ...
    mutate_at(vars(starts_with("incentive_treatment")), funs(str_replace_all(.,  
                                                                             c("control-bracelet" = "bracelet social", 
                                                                               "(calendar|bracelet)-bracelet" = "bracelet",
...
prepare_est_takeup <- function(est_data, subgroups = NULL) { 
  est_data %>%
    ...
    summarize(wtd_takeup = weighted.mean(takeup, treatment_size),
              observed_takeup_prop = weighted.mean(observed_takeup_prop, observed_treatment_size)) %>% 
    ungroup() %>% 
    group_by_at(c(vars(matches("((private|social)_value|sms.treatment.2)")), subgroups, "observed_takeup_prop")) %>% 
    summarize(mean_takeup_prop = mean(wtd_takeup),
              ub_90 = quantile(wtd_takeup, 0.95),
              ub_95 = quantile(wtd_takeup, 0.975),
              lb_90 = quantile(wtd_takeup, 0.05),
              lb_95 = quantile(wtd_takeup, 0.025)) %>% 
    ungroup() %>% 
...
plot_static_est_ate <- function(est_ate_data, outcome_name = "ate") { 
  est_ate_data %>% 
    mutate(incentive_treatment_left = fct_relabel(incentive_treatment_left, . %>% str_to_title()),
           ref = if_else(incentive_treatment_right == "control" & sms.treatment.2_right == "sms.control", 
                         "control-sms.control", 
      ...
      ggplot(., aes_string("incentive_treatment_left", str_interp("${outcome_name}_mean"), color = "sms.treatment.2_left", shape = "ref")) +
        geom_pointrange(aes_string(ymin = str_interp("${outcome_name}_lb_95"), ymax = str_interp("${outcome_name}_ub_95")), size = 0.5, fatten = 6, position = position_dodge(width = 0.4)) +
        geom_linerange(aes_string(ymin = str_interp("${outcome_name}_lb_90"), ymax = str_interp("${outcome_name}_ub_90")), size = 1.25, position = position_dodge(width = 0.4)) +
        geom_hline(yintercept = 0, linetype = "dotted") +
        geom_text_repel(aes_string(label = str_interp("sprintf('%.2f', ${outcome_name}_mean)"), color = NULL), nudge_x = 0.3, size = 3, segment.colour = NA) +
...
plot_static_est_ate_percent <- . %>% {
  plot_static_est_ate(., "ate_percent") + 
    labs(title = "Percentage Change in Deworming Take-up", 
         x = "Incentive/Signal Treatment", 
         y = "Posterior Percentage Change in Probability of Deworming", 
         caption = "Points on plot are the mean point estimates.\nThe thick and thin vertical lines show the 90% and 95% posterior probability ranges, respectively.") 
}
...
dist_pot_est_deworming_takeup_ate %>%
  plot_static_est_ate() +
  labs(subtitle = "Split by distance from treatment location") +
...
phone_dist_pot_est_deworming_takeup_ate %>%
  ...
  plot_static_est_ate() +
  labs(subtitle = "Phone owners only, split by distance from treatment location") +
  facet_wrap(~ dist.pot.group_left, ncol = 1,
             labeller = labeller(.cols = as_labeller(. %>% str_to_title() %>% str_c("Distance: ", .)))) 
```

```{r static-model-non-phone-dist-ate-plot, fig.height=15, fig.width=10}
phone_dist_pot_est_deworming_takeup_ate %>%
  filter(!phone_owner_left) %>% 
  plot_static_est_ate() +
  labs(subtitle = "Non phone owners only, split by distance from treatment location") +
  facet_wrap(~ dist.pot.group_left, ncol = 1,
             labeller = labeller(.cols = as_labeller(. %>% str_to_title() %>% str_c("Distance: ", .)))) 
```

```{r static-est-takeup-plotter}
plot_static_est_takeup <- . %>% 
  mutate(incentive_treatment = fct_relabel(incentive_treatment, . %>% str_to_title())) %>% {
  ggplot(., aes(incentive_treatment, mean_takeup_prop, color = sms.treatment.2)) + 
    geom_point(aes(y = observed_takeup_prop), shape = 9, size = 5, position = position_dodge(width = 0.6)) +
    geom_pointrange(aes(ymin = lb_95, ymax = ub_95), size = 0.5, fatten = 5, position = position_dodge(width = 0.6)) +
    geom_linerange(aes(ymin = lb_90, ymax = ub_90), size = 1.5, position = position_dodge(width = 0.6)) +
    geom_text_repel(aes(label = sprintf("%.2f", mean_takeup_prop), color = NULL), nudge_x = -0.4, size = 3, segment.color = NA) +
    scale_color_discrete("SMS Treatment", labels = c("None", "Reminders Only", "Social Information")) +
...
phone_dist_pot_est_deworming_takeup %>%
  ...
  facet_wrap(~ dist.pot.group + phone_owner, ncol = 1, scales = "free_y",
             labeller = labeller(dist.pot.group = as_labeller(. %>% str_to_title() %>% str_c("Distance: ", .)))) 
```

```{r static-model-ate-percent-plot, fig.height=6, fig.width=12}
plot_static_est_ate_percent(est_deworming_takeup_ate)
```

```{r static-model-phone-ate-percent-plot, fig.height=10, fig.width=10}
phone_est_deworming_takeup_ate %>%
  mutate(phone_owner_left = if_else(phone_owner_left, "Phone Owners", "Non-Phone Owners")) %>% 
  plot_static_est_ate_percent() +
  facet_wrap(~ phone_owner_left, ncol = 1, scales = "free_y", drop = TRUE)
...
```{r static-model-dist-ate-percent-plot, fig.height=10, fig.width=10}
dist_pot_est_deworming_takeup_ate %>%
  plot_static_est_ate_percent() +
  labs(subtitle = "Split by distance from treatment location") +
  facet_wrap(~ dist.pot.group_left, ncol = 1,
...
```{r static-model-phone-dist-ate-percent-plot, fig.height=15, fig.width=10}
phone_dist_pot_est_deworming_takeup_ate %>%
  filter(phone_owner_left) %>% 
  plot_static_est_ate_percent() +
  labs(subtitle = "Phone owners only, split by distance from treatment location") +
...
```{r static-model-non-phone-dist-ate-percent-plot, fig.height=6, fig.width=10}
phone_dist_pot_est_deworming_takeup_ate %>%
  filter(!phone_owner_left) %>% 
  plot_static_est_ate_percent() +
  labs(subtitle = "Non phone owners only, split by distance from treatment location") +
...
M(t, m, h; \gamma) &= M(z^{dyn}, s^{dyn}, d, s, b, m, h; \gamma) \\
                   &= \sum_{w\in\widetilde{Z}^r} \sum_{q = 1}^2 (m - 1)^q\cdot Z^r_w(z^{dyn})\left\{\gamma^r_{qw1} + h\times\gamma^r_{qw2}\right\} \\
                   {} &+ \sum_{w\in\widetilde{S}} \sum_{q = 1}^2\lfloor (m - 1)/2 \rfloor^q\cdot S_w(s^{dyn})\left\{\gamma^s_{qw1} + h\times\gamma^s_{qw2} + S^{nd}(m)\times\gamma^s_{qw3} + S^{nd}(m)\times h\times\gamma^s_{qw4}\right\} \\
\end{align}
$$

where

* $Z_w^r(z^{dyn}) = \mathbbm{1}\{z^{dyn} = w\}$
* $S_w(s^{dyn}) = \mathbbm{1}\{s^{dyn} = w\}$
* $S^{nd}(m)$ is a binary variable indicating whether $m$ is a day right after receiving an SMS message.
...
$$
\mathcal{L}_i = \left[\prod_{m = 1}^{M_i - 1} \alpha_m(t, m, h;\beta_{j[i]}, \gamma_{j[i]}, \lambda_{j[i],m})\right] \cdot (1 - \alpha_{M_i}(t, M_i, h;\beta_{j[i]}, \gamma_{j[i]}, \lambda_{j[i],m})^{Y_i}
$$
...
dyn_analysis_data <- analysis.data %>%
  mutate(private_value = fct_collapse(assigned.treatment, 
         ...
         incentive_shift = private_value,
         dyn_dist_pot = dist.pot.group,
         reminder_info_stock = sms.treatment.2) %>% 
  # filter(!name_matched, !hh.baseline.sample, sms.treatment.2 == "control", !phone_owner)
  filter(!name_matched)
  # filter(!name_matched | sms.treatment.2 == "control") # There are a couple of SMS observations that are name-checked. Drop them. 

dyn_dyn_treatment_maps <- lst(
  formula = ~ (poly(days_observed, 2, raw = TRUE) * (signal_observed + incentive_shift) - poly(days_observed, 2, raw = TRUE) +
                 poly(days_sms, 2, raw = TRUE) * reminder_info_stock - poly(days_sms, 2, raw = TRUE)) * dyn_dist_pot * phone_owner - dyn_dist_pot * phone_owner,
  trends = tibble(days_observed = drop(scale(0:11)),
                  days_sms = rep(drop(scale(0:5)), each = 2)),
                  scale = FALSE) %>%
  prepare_dynamic_treatment_maps(dyn_analysis_data)

# dyn_dyn_treatment_maps <- lst(
#   formula = ~ (poly(days_observed, 1, raw = TRUE) * (signal_observed + incentive_shift) - poly(days_observed, 1, raw = TRUE) +
#                  poly(days_sms, 1, raw = TRUE) * reminder_info_stock - poly(days_sms, 1, raw = TRUE)) * dyn_dist_pot * phone_owner - dyn_dist_pot * phone_owner,
#   trends = tibble(days_observed = drop(scale(0:11)),
#                   days_sms = rep(drop(scale(0:5)), each = 2)),
#                   scale = FALSE) %>%
#   prepare_dynamic_treatment_maps(dyn_analysis_data)
...
# dyn_dyn_treatment_maps <- lst(
#   formula = ~ (poly(days_observed, 2, raw = TRUE) * (signal_observed + incentive_shift) - poly(days_observed, 2, raw = TRUE)) * dyn_dist_pot - dyn_dist_pot,
#   trends = tibble(days_observed = drop(scale(0:11)),
#                   scale = FALSE)) %>%
#   prepare_dynamic_treatment_maps(dyn_analysis_data)

dyn_static_treatment_map <- dyn_analysis_data %>% 
  data_grid(private_value, social_value, signal_observed, incentive_shift, dyn_dist_pot, reminder_info_stock, dist.pot.group, sms.treatment.2,
            phone_owner) %>% #, name_matched) %>%
  # data_grid(private_value, social_value, signal_observed, incentive_shift, dyn_dist_pot, dist.pot.group) %>% 
  prepare_treatment_map(dyn_dyn_treatment_maps)
```
...
dyn_stan_data <- prepare_bayesian_analysis_data(
  ...
  dynamic_treatment_map = dyn_dyn_treatment_maps,
    
  # treatment_formula = ~ (private_value + social_value * sms.treatment.2 * (dist.pot.group + hh.baseline.sample)) * phone_owner * name_matched,
  treatment_formula = ~ (private_value + social_value * sms.treatment.2 * dist.pot.group) * phone_owner,
  # treatment_formula = ~ (private_value + social_value * dist.pot.group),
  subgroup_col = "phone_owner",
    
  all_ate = get_dyn_ate(),
  # all_ate = select(dyn_sms_control_ate, -contains("sms.treatment.2"), -contains("reminder_info_stock"), -contains("phone_owner")),
...
dyn_param_fit_ver <- "testing"

basic_model_2_simple_fit <- sampling(
  basic_model_2_simple, data = dyn_stan_data, 
  chains = 8, iter = 800, 
  sample_file = file.path("stanfit", str_interp("basic_model_2_simple_centered_${dyn_param_fit_ver}.csv"))
) 

save(dyn_stan_data, file = file.path("stan_analysis_data", str_interp("model_2_${dyn_param_fit_ver}.RData")))
```

```{r basic-model-2-simple-debug, eval=FALSE}
basic_model_2_simple <- stan_model(file = "takeup_model_2_simple_centered.stan", model_name = "model_2_simple")
basic_model_2_simple_fit_debug <- sampling(basic_model_2_simple, data = dyn_stan_data, chains = 1, iter = 1, control = lst(max_treedepth = 15)) 
```


### Posterior Simulation

```{r old-load-dyn-model-fit}
basic_model_2_simple_fit <- dir("stanfit", pattern = str_interp("basic_model_2_simple_centered_${dyn_param_fit_ver}_[1-8].csv"), full.names = TRUE) %>% 
  read_stan_csv()
...
load(file.path("stan_analysis_data", str_interp("model_2_${dyn_param_fit_ver}.RData")))
```

```{r}
print(basic_model_2_simple_fit, par = c("hyper_baseline_cond_takeup"))
print(basic_model_2_simple_fit, par = c("stratum_baseline_cond_takeup"))
print(basic_model_2_simple_fit, par = c("stratum_hazard_effect"))
print(basic_model_2_simple_fit, par = c("stratum_hazard_frailty_var"))
print(basic_model_2_simple_fit, par = c("est_takeup"))
```
...
fast_sim_mean_monitored <- fast_sim %>% 
  filter(!name_matched) %>% 
  combine_treatment_group(c("phone_owner", "dist.pot.group"))

fast_sim_mean_dist_monitored <- fast_sim %>% 
  filter(!name_matched) %>% 
...
save(fast_sim, fast_sim_day13_only, fast_sim_mean, fast_sim_mean_dist_monitored, fast_sim_mean_monitored, fast_sim_mean_phone_owner, fast_sim_mean_phone_owner_monitored, 
...
fast_sim_ate <- fast_sim_ate_pairs %>% 
  ...
  do(ate_phone_owner_dist = group_by_at(.$diff_data, 
       ...
       ungroup(),
     ate_phone = group_by_at(.$diff_data, vars(phone_owner,
                                           starts_with("incentive_treatment"), starts_with("sms.treatment.2"), starts_with("dyn_treat_days"))) %>% 
       do(summarize_ate_diff(.)) %>%  
       ungroup(),
     ate_dist = group_by_at(.$diff_data, vars(dist.pot.group, 
                                           starts_with("incentive_treatment"), starts_with("sms.treatment.2"), starts_with("dyn_treat_days"))) %>% 
       do(summarize_ate_diff(.)) %>%  
       ungroup(),
     ate_all = group_by_at(.$diff_data, vars(starts_with("incentive_treatment"), starts_with("sms.treatment.2"), starts_with("dyn_treat_days"))) %>% 
       do(summarize_ate_diff(.)) %>%  
       ungroup()) %>% 
  ungroup()
...
dyn_fast_sim_ate <- dyn_fast_sim_ate_pairs %>% 
  ...
  do(ate_phone_owner_dist = group_by_at(.$diff_data, 
       ...
       do(summarize_ate_diff(.)) %>%  
       ungroup(),
    ate_phone_owner = group_by_at(.$diff_data, 
                                   vars(phone_owner, deworming_day,
                                        starts_with("incentive_treatment"), starts_with("sms.treatment.2"), starts_with("dyn_treat_days"))) %>% 
       do(summarize_ate_diff(.)) %>%  
       ungroup(),
     ate_dist = group_by_at(.$diff_data, vars(dist.pot.group, deworming_day, 
                                           starts_with("incentive_treatment"), starts_with("sms.treatment.2"), starts_with("dyn_treat_days"))) %>% 
       do(summarize_ate_diff(.)) %>%  
       ungroup(),
     ate_all = group_by_at(.$diff_data, vars(deworming_day, 
                                           starts_with("incentive_treatment"), starts_with("sms.treatment.2"), starts_with("dyn_treat_days"))) %>% 
       do(summarize_ate_diff(.)) %>%  
       ungroup()) %>% 
  ungroup()
...
fast_sim_mean_monitored %>%
  ...
  geom_ribbon(aes(ymin = no_obs, ymax = full_obs, group = incentive_treatment), 
              ...
              data = . %>%
                ...
                select(deworming_day, incentive_treatment, obs_type, prob_deworm) %>%
...
dyn_fast_sim_ate %>% 
  ...
  geom_ribbon(aes(ymin = no_obs, ymax = full_obs, group = incentive_treatment_left), alpha = 0.2,
              data = . %>%
                ...
                select(deworming_day, incentive_treatment_left, prob_deworm_ate, obs_type) %>%
...
dyn_fast_sim_ate %>% 
  ...
  geom_ribbon(aes(ymin = no_obs, ymax = full_obs, group = incentive_treatment_left), alpha = 0.2,
              data = . %>%
                ...
                select(deworming_day, phone_owner, incentive_treatment_left, prob_deworm_ate, obs_type) %>%
                spread(obs_type, prob_deworm_ate)) +
  scale_x_continuous("Deworming Day", breaks = 1:12, minor_breaks = FALSE) +
  scale_linetype_discrete("Days Observing Signal", labels = c("No Observation", "Full Observation")) +
  scale_color_discrete("Incentive/Signal Treatment") +
  facet_wrap(~ phone_owner) +
  labs(title = "Daily Treatment Effect with Varying Days of Signal Observation", 
...
dyn_fast_sim_ate %>%
  ...
  geom_ribbon(aes(ymin = no_obs, ymax = full_obs, group = sms.treatment.2_left), alpha = 0.2,
              data = . %>%
                ...
                select(deworming_day, phone_owner, incentive_treatment_left, sms.treatment.2_left, obs_type, prob_deworm_ate) %>%
                spread(obs_type, prob_deworm_ate)) +
  scale_x_continuous("Deworming Day", breaks = seq(2, 12, 2), minor_breaks = TRUE) +
  scale_linetype_discrete("Days Observing Signal", labels = c("No Observation", "Full Observation")) +
  scale_color_discrete("SMS Treatment", labels = c("None", "Reminders Only", "Social Information")) +
  facet_wrap(~ incentive_treatment_left,
             labeller = labeller(incentive_treatment_left = as_labeller(. %>% str_to_title()))) +
...
fast_sim_mean_monitored %>% 
  ...
  geom_text_repel(aes(label = sprintf("%.3f", prob_deworm), color = NULL), nudge_x = -0.25, size = 3, segment.color = NA) +
  scale_color_discrete("SMS Treatment", labels = c("None", "Reminders Only", "Social Information")) +
  coord_flip() +
  # coord_flip(ylim = c(0, 1)) +
  # facet_wrap(~ dist.pot.group,  
...
```{r sim-fast-take-up-dist, fig.width=12, fig.height=8}
fast_sim_mean_dist_monitored %>% 
  filter(deworming_day == 13, 
         dyn_treat_days == 11) %>%
  ggplot(aes(fct_relabel(incentive_treatment, str_to_title), prob_deworm, color = sms.treatment.2)) +
  geom_pointrange(aes(ymin = prob_deworm_quant_2.5, ymax = prob_deworm_quant_97.5), size = 0.5, position = position_dodge(width = 0.5)) +
  ...
  facet_wrap(~ dist.pot.group, scales = "free_y", 
             ...
             ncol = 1) +
  labs(title = "Deworming Take-up", 
       subtitle = "Split by distance to treatment location", 
       x = "Incentive/Signal Treatment", 
       y = "Posterior Probability of Deworming", 
       caption = "Points on plot are the mean point estimates.\nThe thick and thin vertical lines show the 90% and 95% posterior probability ranges, respectively.") + 
  theme(legend.position = "right")
...
fast_sim_ate %>%
  ...
  geom_hline(yintercept = 0, linetype = "dotted") +
  geom_pointrange(aes(ymin = prob_deworm_diff_quant_2.5, ymax = prob_deworm_diff_quant_97.5), fatten = 6, position = position_dodge(width = 0.4)) +
  geom_linerange(aes(ymin = prob_deworm_diff_quant_5, ymax = prob_deworm_diff_quant_95), size = 1.5, position = position_dodge(width = 0.4)) +
...
```{r fast-sim-ate-phone-plot, fig.width=12, fig.height=8}
fast_sim_ate %>%
  unnest(ate_phone) %>% 
  mutate(ref = if_else(incentive_treatment_right == "control" & sms.treatment.2_right == "sms.control", 
                       "control-sms.control", 
  ...
  labs(title = "Treatment Effect on Deworming Take-up", 
       subtitle = "Split by phone ownership",
       x = "Incentive/Signal Treatment", 
       y = "Posterior Difference in Probability of Deworming", 
       caption = "Points on plot are the mean point estimates.\nThe thick and thin vertical lines show the 90% and 95% posterior probability ranges, respectively.") + 
  theme(legend.position = "right", legend.direction = "vertical")
...
```{r fast-sim-ate-dist-plot, fig.width=12, fig.height=8}
fast_sim_ate %>%
  unnest(ate_dist) %>% 
  mutate(ref = if_else(incentive_treatment_right == "control" & sms.treatment.2_right == "sms.control", 
                       "control-sms.control", 
...
```{r fast-sim-ate-plot-phone-owner-dist, fig.width=12, fig.height=8}
fast_sim_ate %>%
  unnest(ate_phone) %>% 
  mutate(ref = if_else(incentive_treatment_right == "control" & sms.treatment.2_right == "sms.control", 
                       "control-sms.control", 
  ...
  coord_flip() +
  facet_wrap(~  phone_owner, scales = "free_y", ncol = 1) +
...
## New Parameteric Model

```{r param-dyn-analysis-data}
dyn_analysis_data <- analysis.data %>%
  mutate(private_value = fct_collapse(assigned.treatment, 
                                      control = c("control", "ink"), 
                                      "calendar" = c("calendar", "bracelet")), 
...
```{r param-dyn-model-data, warning=FALSE}
param_dyn_stan_data <- prepare_bayesian_analysis_data(
  ...
  #   select(-starts_with("sms.treatment.2"), -starts_with("reminder")) %>% 
  #   distinct(),
...
model_3_fit <- dir("stanfit", pattern = str_interp("model_3_${dyn_fit_version}_[1-8].csv"), full.names = TRUE) %>% 
  read_stan_csv()
...
load(file.path("stan_analysis_data", str_interp("model_3_${dyn_fit_version}.RData")))
```
...
model_3_fit %>% print(pars = c("hyper_beta_day1", "hyper_baseline_dyn_effect"))
model_3_fit %>% print(pars = c("QR_hyper_treat_beta_dyn_effect"))
model_3_fit %>% print(pars = c("strata_beta_day1_tau"))
model_3_fit %>% print(pars = c("strata_beta_day1_raw"))
model_3_fit %>% print(pars = c("cluster_beta_day1"))

model_3_fit %>% print(pars = c("est_takeup"))
model_3_fit %>% print(pars = c("est_deworming_days"))
...
mcmc_pairs(model_3_posterior_array, 
           regex_pars = c(
             "strata_baseline_dyn_effect_raw\\[[1-3],1",
             "strata_baseline_dyn_effect_tau\\[[1-3]\\]"
           ),
           transform = lst(
             "strata_baseline_dyn_effect_tau[1]" = "log", 
             "strata_baseline_dyn_effect_tau[2]" = "log", 
             "strata_baseline_dyn_effect_tau[3]" = "log", 
           ), 
           np = model_3_np)
```
...
```{r new-param-dyn-model-prep-data}
...
dyn_treatment_cell_info <- tibble(treatment_size = with(param_dyn_stan_data, missing_treatment_sizes + observed_treatment_sizes)) %>% 
  bind_cols(param_dyn_stan_data$ate_treatments) %>% 
  left_join(param_dyn_stan_data$treatment_map, c("static_all_treatment_id" = "all_treatment_id")) %>% 
  left_join(param_dyn_stan_data$treatment_map, c("dynamic_all_treatment_id" = "all_treatment_id"), suffix = c("_static", "_dynamic")) %>% 
  mutate(treatment_rank = seq_len(n()),
         observed_treatment_size = param_dyn_stan_data$observed_treatment_sizes, 
         observed_takeup_prop = param_dyn_stan_data$observed_takeup_total / observed_treatment_size) 

dyn_ate_pair_info <- param_dyn_stan_data$ate_pairs %>% 
...
est_deworming_takeup_ate <- model_3_fit %>% 
...
```{r summarize-est}
subgroup_combiner <- function(iter_data, outcome_var, subgroups) {
  iter_data %>% 
    group_by_at(c(vars(matches("(incentive_treatment|(private|social)_value|sms.treatment.2)"), iter_id), subgroups)) %>% 
...
prepare_est_deworming_takeup_dyn <- function(est_data, ...) { 
...
prepare_est_deworming_ate_dyn <- function(est_data, ...) { 
  prepare_est_deworming(est_data, "iter_ate", ...) %>% 
    unite(incentive_treatment_static_left, str_c(c("private", "social"), "_value_static_left"), sep = "-", remove = FALSE) %>% 
    unite(incentive_treatment_dynamic_left, str_c(c("private", "social"), "_value_dynamic_left"), sep = "-", remove = FALSE) %>% 
    unite(incentive_treatment_static_right, str_c(c("private", "social"), "_value_static_right"), sep = "-", remove = FALSE) %>% 
    unite(incentive_treatment_dynamic_right, str_c(c("private", "social"), "_value_dynamic_right"), sep = "-", remove = FALSE) %>% 
    create_incentive_treatment_col()
}
...
est_deworming_takeup_dyn_phone <- est_deworming_takeup_dyn %>% 
  ...
  prepare_est_deworming_takeup_dyn(subgroups = "phone_owner_static")

est_deworming_day_phone <- est_deworming_days %>% 
  filter(dist.pot.group_static == dist.pot.group_dynamic) %>%
  prepare_est_deworming_takeup_dyn(subgroups = "phone_owner_static", daily = TRUE)

est_deworming_takeup_dyn_dist <- est_deworming_takeup_dyn %>% 
  prepare_est_deworming_takeup_dyn(subgroups = str_c("dist.pot.group_", c("static", "dynamic")))

est_deworming_day_dist <- est_deworming_days %>% 
  prepare_est_deworming_takeup_dyn(subgroups = str_c("dist.pot.group_", c("static", "dynamic")), daily = TRUE)

est_deworming_takeup_dyn_phone_dist <- est_deworming_takeup_dyn %>%
  prepare_est_deworming_takeup_dyn(subgroups = c("phone_owner_static", str_c("dist.pot.group_", c("static", "dynamic"))))

est_deworming_day_phone_dist <- est_deworming_days %>%
  prepare_est_deworming_takeup_dyn(subgroups = c("phone_owner_static", str_c("dist.pot.group_", c("static", "dynamic"))), daily = TRUE)

est_deworming_takeup_dyn_all <- est_deworming_takeup_dyn %>% 
  filter(dist.pot.group_static == dist.pot.group_dynamic) %>% 
  prepare_est_deworming_takeup_dyn() 

est_deworming_day_all <- est_deworming_days %>% 
  filter(dist.pot.group_static == dist.pot.group_dynamic) %>% 
  prepare_est_deworming_takeup_dyn(daily = TRUE) 
...
est_deworming_takeup_ate_phone <- est_deworming_takeup_ate %>% 
  ...
  prepare_est_deworming_ate_dyn(subgroups = "phone_owner_static_left")

est_deworming_takeup_ate_dist <- est_deworming_takeup_ate %>% 
  prepare_est_deworming_ate_dyn(subgroups = str_c("dist.pot.group_", c("static", "dynamic"), rep(c("_left", "_right"), each = 2)))
...
est_deworming_takeup_ate_phone_dist <- est_deworming_takeup_ate %>%
  ...
  prepare_est_deworming_ate_dyn(subgroups = c("phone_owner_static_left", str_c("dist.pot.group_", c("static", "dynamic"), rep(c("_left", "_right"), each = 2))))

est_deworming_takeup_ate_all <- est_deworming_takeup_ate %>% 
  filter(dist.pot.group_static_left == dist.pot.group_dynamic_left,
         dist.pot.group_static_right == dist.pot.group_dynamic_right) %>%
  prepare_est_deworming_ate_dyn() 
```
...
```{r new-plotters}
plot_dyn_takeup <- function(takeup_summ_data, takeup_data, data_preparer = function(data) data) {
  inner_data_preparer <- . %>% 
    data_preparer() %>% 
  ...
  takeup_summ_data %>%
    inner_data_preparer() %>% { 
      ggplot(., aes(incentive_treatment_static, mean_est)) +
        # ggplot(aes(incentive_treatment_static, mean_est, color = sms.treatment.2_static)) +
        geom_violin(aes(y = wtd_iter_est), stroke = 0.25, color = "grey", fill = "lightgrey", data = inner_data_preparer(takeup_data)) +
...
plot_dyn_ate <- . %>% 
  ...
  #        ref = fct_relevel(ref, "control-sms.control", "own sms.control", "control-social.info")) %>%
  mutate_at(vars(starts_with("incentive_treatment_static")), funs(fct_relabel(., str_to_title))) %>% {
    ggplot(., aes(incentive_treatment_static_left, mean_est)) +
      # ggplot(aes(incentive_treatment_static, mean_est, color = sms.treatment.2_static)) +
      geom_hline(yintercept = 0, linetype = "dotted") +
      geom_pointrange(aes(ymin = lb_95, ymax = ub_95), size = 0.5, position = position_dodge(width = 0.5)) +
      geom_linerange(aes(ymin = lb_90, ymax = ub_90), size = 1.5, position = position_dodge(width = 0.5)) +
...
```{r, fig.width=10, fig.height=12}
est_deworming_takeup_dyn_phone_dist %>% 
  filter(incentive_treatment_static == incentive_treatment_dynamic,
         dist.pot.group_static == dist.pot.group_dynamic) %>% 
         # sms.treatment.2_static == sms.treatment.2_dynamic) %>% 
  mutate_at(vars(starts_with("dist.pot.group_static")), funs(fct_relabel(., str_to_title))) %>% 
...
```{r nonparam-dyn-analysis-data}
nonparam_dyn_analysis_data <- analysis.data %>%
  mutate(private_value = fct_collapse(assigned.treatment, 
                                      control = c("control", "ink"), 
                                      "calendar" = c("calendar", "bracelet")), 
         social_value = fct_collapse(assigned.treatment, control = c("control", "calendar")),
         sms.treatment.2 = fct_recode(sms.treatment.2, control = "sms.control")) %>% 
  filter(!name_matched) #, !hh.baseline.sample)

nonparam_dyn_static_treatment_map <- nonparam_dyn_analysis_data %>% 
  data_grid(private_value, social_value, dist.pot.group, sms.treatment.2, phone_owner) %>% 
  filter(sms.treatment.2 == "control" | phone_owner) %>% 
  prepare_treatment_map()
```
...
```{r old-ate-cleanup, eval=FALSE}
nonparam_ate <- dyn_all_ate %>% 
  select(-starts_with("sms.treatment.2"), -starts_with("reminder_info")) %>%
  distinct() %>% { 
    lst(left = select(., -ends_with("_right")),
        right = select(., -ends_with("_left")))
  } %>% 
  map(~ set_colnames(.x, str_replace(names(.x), "_(left|right)$", ""))) %>% 
  map(~ lst(static = select(.x, -one_of(c("incentive_shift", "signal_observed", "dyn_dist_pot"))), #, "reminder_info_stock"))),
            dyn = select(.x, -one_of(c("private_value", "social_value", "dist.pot.group"))) %>% #, "sms.treatment.2"))) %>% 
              rename_(.dots = setNames(c("incentive_shift", "signal_observed", "dyn_dist_pot"), 
                                       c("private_value", "social_value", "dist.pot.group")))) %>% 
        map(left_join, nonparam_dyn_static_treatment_map)) 
              # rename_(.dots = setNames(c("incentive_shift", "signal_observed", "dyn_dist_pot", "reminder_info_stock"), 
              #                          c("private_value", "social_value", "dist.pot.group", "sms.treatment.2")))))

nonparam_ate_pairs <- map(nonparam_ate, ~ map(.x, pull, all_treatment_id)) %>% 
  flatten() %>% 
  set_names(str_c(c("left", "left", "right", "right"), names(.), sep = "_")) %>% 
  as_tibble() %>% 
  mutate(pair_index = seq_len(n()))

nonparam_ate_treatments <- nonparam_ate_pairs %>%
  gather(left_right, val, -pair_index) %>% 
  separate(left_right, c("left_right", "dyn_static")) %>% 
  spread(dyn_static, val) %>% 
  select(pair_index, dyn, static)  
```

```{r nonparam-dyn-model-data, warning=FALSE}
nonparam_dyn_stan_data <- prepare_bayesian_analysis_data(
  nonparam_dyn_analysis_data,
  wtp.data, 
   
  prepared_treatment_maps = TRUE, 
  treatment_map = nonparam_dyn_static_treatment_map,
...
gen_nonparam_initializer <- function(stan_data_list) {
  function() {
    num_dynamic_treatment_days <-  with(stan_data_list, (num_deworming_days - first_dynamics_day + 1) * num_dynamic_treatments);
    lst(
      # hyper_beta = with(stan_data_list, rep(0, num_all_treatment_coef - 1)),
      # stratum_beta_z = with(stan_data_list, matrix(0, num_all_treatment_coef, num_strata)),
      # stratum_census_covar_z = with(stan_data_list, matrix(0, num_census_covar_coef, num_strata)),
      # 
      # stratum_beta_corr_mat = with(stan_data_list, rethinking::rlkjcorr(1, num_all_treatment_coef, lkj_df)),
      # # cluster_beta_corr_mat = with(stan_data_list, rethinking::rlkjcorr(1, num_all_treatment_coef, lkj_df)),
      # stratum_census_covar_corr_mat = with(stan_data_list, rethinking::rlkjcorr(1, num_census_covar_coef, lkj_df)),
      ...
      # hyper_dyn_latent_var = rep(0, num_dynamic_treatment_days)
      
      
      strata_beta_day1_corr_mat = with(stan_data_list, rethinking::rlkjcorr(1, num_all_treatment_coef, lkj_df)),
      strata_beta_day1_corr_mat = foreach(seq_len(stan_data_list$num_all_treatment_coef)) %do% { 
        with(stan_data_list, rethinking::rlkjcorr(1, num_deworming_days - 1, lkj_df)) 
      },
      strata_beta = foreach(seq_len(stan_data_list$num_strata)) %do% { matrix(0, stan_data_list$num_deworming_days, stan_data_list$num_all_treatment_coef) }
    )
  }
}
```

```{r model-3-gp-nonparam, eval=FALSE}
nonparam_dyn_fit_version <- "param_testing_1"

model_3_nonparam <- stan_model(file = file.path("stan_models", "takeup_model_3_nonparam_fe_centered.stan"), model_name = "model_3_param")
# model_3_nonparam <- stan_model(file = file.path("stan_models", "takeup_model_3_nonparam_fe.stan"), model_name = "model_3_nonparam_fe")

model_3_fit <- nonparam_dyn_stan_data %>% 
  sampling(model_3_nonparam, data = ., 
           chains = 2, iter = 300,
           control = lst(max_treedepth = 15), #adapt_delta = 0.99), 
           # init = gen_nonparam_initializer(.),
           sample_file = file.path("stanfit", str_interp("model_3_nonparam_${nonparam_dyn_fit_version}.csv")))

# save(nonparam_dyn_stan_data, file = file.path("stan_analysis_data", str_interp("model_3_gp_nonparam_${nonparam_dyn_fit_version}.RData")))
```

```{r model-3-gp-nonparam-debug, eval=FALSE}
model_3_fit_debug <- nonparam_dyn_stan_data %>% 
  sampling(model_3_gp_nonparam, data = ., 
           chains = 2, control = lst(max_treedepth = 15), 
           iter = 400) 
           # init = gen_nonparam_initializer(.)) 
```

```{r load-nonparam-dyn-model-fit}
model_3_fit <- dir("stanfit/azure", pattern = str_interp("model_3_gp_nonparam_${nonparam_dyn_fit_version}_[1-8].csv"), full.names = TRUE) %>% 
  read_stan_csv()
...
load(file.path("stan_analysis_data", str_interp("model_3_gp_nonparam_${nonparam_dyn_fit_version}.RData")))
```

### Finite Sample Treatment Effects 

```{r}
model_3_fit@model_pars
# model_3_fit %>% print(pars = c("hyper_intercept_raw", "strata_beta_day1_tau_raw"))
model_3_fit %>% print(pars = c("hyper_beta_day1", "strata_beta_day1_tau", "hyper_baseline_dyn_effect"))

model_3_fit %>% print(pars = c("hyper_dyn_latent_var"))
model_3_fit %>% print(pars = c("hyper_intercept"))
model_3_fit %>% print(pars = c("hyper_baseline_beta"))
# model_3_fit %>% print(pars = c("hyper_baseline_cond_takeup"))
# model_3_fit %>% print(pars = c("stratum_baseline_cond_takeup"))
model_3_fit %>% print(pars = c("est_takeup"))
model_3_fit %>% print(pars = c("est_deworming_days"))
```

```{r, fig.width=15}
posterior_data <- as.data.frame(model_3_fit)
model_3_posterior_array <- as.array(model_3_fit)
model_3_np <- nuts_params(model_3_fit)
model_3_lp <- log_posterior(model_3_fit)

mcmc_trace(model_3_posterior_array, regex_pars = c("hyper_baseline_dyn_effect"), np = model_3_np)

mcmc_nuts_divergence(model_3_np, model_3_lp)

color_scheme_set("darkgray")
mcmc_parcoord(model_3_posterior_array, np = model_3_np, 
              regex_pars = c("hyper_intercept", "strata_beta_day1_tau_raw",
                             "strata_beta_day1"))
```

```{r}
mcmc_scatter(posterior_data, pars = c("strata_beta[1,1,1]", "strata_beta_day1_tau_raw[1]"), transform = lst("strata_beta_day1_tau_raw[1]" = "log"), np = model_3_np)

mcmc_scatter(posterior_data, pars = c("strata_beta[1,1,2]", "strata_beta_day1_tau_raw[2]"), transform = lst("strata_beta_day1_tau_raw[2]" = "log"), np = model_3_np)
```
...
mcmc_pairs(model_3_posterior_array, 
           regex_pars = c(
             "hyper_beta_day1\\[[1-2]", 
             "hyper_beta_baseline_dyn_effect\\[[1-2]\\]", 
             "strata_beta_day1_raw\\[[1-3]\\]",
             
             "strata_beta_day1_tau\\[[1-3]\\]" 
              # "strata_beta_dyn_tau_raw\\[[1-2],1\\]"
             ),
           transform = lst(
             "strata_beta_day1_tau[1]" = "log", 
             "strata_beta_day1_tau[2]" = "log",
             "strata_beta_day1_tau[3]" = "log",
             # "strata_beta_day1_tau_raw[4]" = "log",
             # "strata_beta_day1_tau_raw[5]" = "log", 
             # "strata_beta_day1_tau_raw[6]" = "log"
             # "strata_beta_dyn_tau_raw[1,1]" = "log", 
             # "strata_beta_dyn_tau_raw[2,1]" = "log"
             ), 
           np = model_3_np)
```

```{r}
baseline_dyn_latent_var <- model_3_fit %>% 
  tidy(pars = "hyper_dyn_latent_var") %>% 
  separate(term, c("term", "dyn_treat_index", "day"), sep = "[\\[,\\]]") %>% 
  mutate_at(vars(dyn_treat_index, day), as.integer) %>% 
  rename(est_dyn = estimate) %>% 
  filter(dyn_treat_index == 1)

model_3_fit %>% 
  tidy(pars = "stratum_beta_mat") %>% 
  separate(term, c("term", "param_index", "stratum_id"), sep = "[\\[,\\]]") %>% 
  mutate_at(vars(stratum_id, param_index), as.integer) %>% 
  rename(est_intercept = estimate) %>% 
  filter(param_index == 1) %>% 
  group_by(stratum_id) %>% 
  do({
    intercept <- .$est_intercept
    
    baseline_dyn_latent_var %>% 
      mutate(est = est_dyn + intercept)
  }) %>% 
  ungroup() %>% 
  select(-c(term, dyn_treat_index)) %>% 
  mutate(prob = VGAM::pgumbel(- est, lower.tail = FALSE)) %>% kable()

model_3_fit@model_pars

model_3_fit %>% 
  tidy(par = "est_deworming_days") %>% 
  separate(term, c("term", "treatment_index", "day"), sep = "[\\[,\\]]") %>% 
  filter(treatment_index == 1)  
  
```


```{r dyn-model-prep-data}
dyn_treatment_cell_size <- tibble(treatment_size = with(dyn_stan_data, missing_treatment_sizes + observed_treatment_sizes)) %>% 
  mutate(treatment_rank = seq_len(n()))

est_deworming_takeup_dyn <- basic_model_2_simple_fit %>% 
  as.data.frame( pars = "est_takeup") %>%
  mutate(iter_id = seq_len(n())) %>% 
  gather(treatment_rank, iter_takeup, -iter_id) %>% 
  mutate(treatment_rank = str_extract(treatment_rank, "\\d+") %>% as.integer()) %>% 
  mutate(all_treatment_id = dyn_stan_data$ate_treatments[treatment_rank]) %>%
  inner_join(dyn_treatment_cell_size, "treatment_rank")

num_raw_deworming_takeup_dyn <- nrow(est_deworming_takeup_dyn)

est_deworming_takeup_dyn %<>% 
...
num_raw_deworming_days <- nrow(est_deworming_days)

est_deworming_days %<>% 
  inner_join(dyn_stan_data$treatment_map, "all_treatment_id") 

prepare_est_deworming_takeup_dyn <- function(est_data, daily = FALSE, subgroups = NULL) { 
  subgroups <- if (daily) c(subgroups, "day") else subgroups
  
  est_data %>%
    group_by_at(c(vars(matches("((private|social)_value|sms.treatment.2)"), iter_id), subgroups)) %>% 
    summarize(wtd_takeup = weighted.mean(iter_takeup, treatment_size)) %>% 
    ungroup() %>% 
...
est_deworming_takeup_dyn %<>% prepare_est_deworming_takeup_dyn() 
```
...
summ_dyn_takeup <- est_deworming_days %>% 
  ...
  summarize(mean_takeup_prop = mean(iter_takeup),
            ub_90 = quantile(iter_takeup, 0.95),
            lb_90 = quantile(iter_takeup, 0.05)) %>% 
  ungroup() %>% {
...
```{r dyn-take-up, fig.width=12, fig.height=7}
est_deworming_takeup_dyn %>% 
  ggplot(aes(fct_relabel(incentive_treatment, str_to_title), mean_takeup_prop, color = sms.treatment.2)) +
  geom_pointrange(aes(ymin = lb_90, ymax = ub_90), size = 0.5, position = position_dodge(width = 0.5)) +
  geom_linerange(aes(ymin = lb_95, ymax = ub_95), size = 1.5, position = position_dodge(width = 0.5)) +
  geom_text_repel(aes(label = sprintf("%.3f", mean_takeup_prop), color = NULL), nudge_x = -1, size = 3, segment.color = NA) +
  # scale_color_discrete("SMS Treatment", labels = c("None", "Reminders Only", "Social Information")) +
  # coord_flip(ylim = c(0, 1)) +
  # facet_wrap(~ dist.pot.group,  
  #            labeller = as_labeller(. %>% str_to_title() %>% str_c("Distance: ", .)),
...
```{r daily-obs-plot, fig.width=12, fig.height=6}
est_deworming_days %>%
  filter(day <= 12, 
         incentive_treatment %in% c("control", "ink", "bracelet", "bracelet social"),
         sms.treatment.2 == "control") %>% 
  ggplot(aes(x = day)) +
  geom_line(aes(y = mean_takeup_prop, 
                group = incentive_treatment, color = fct_relabel(incentive_treatment, str_to_title))) +
  geom_ribbon(aes(ymin = no_obs, ymax = full_obs, group = incentive_treatment), 
              alpha = 0.2,
              data = . %>%
                filter(incentive_treatment != "control") %>%
