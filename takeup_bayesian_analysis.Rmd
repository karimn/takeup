...
nonparam_dyn_treatment_maps <- lst(formula = ~ (days_observed : (signal_observed + incentive_shift + reminder_info_stock)) : dyn_dist_pot,
                                   trends = tibble(days_observed = 1:12),
                                   scale = FALSE) %>% 
...
model_3_fit <- nonparam_dyn_stan_data %>% 
  sampling(model_3_gp_nonparam, data = ., 
           ...
           sample_file = file.path("stanfit", "model_3_gp_nonparam.csv")) 

save(nonparam_dyn_stan_data, file = file.path("stan_analysis_data", "model_3_1.RData"))
```
...
model_3_fit <- dir("stanfit", pattern = str_interp("model_3_gp_nonparam_${dyn_model_version}_[1-8].csv"), full.names = TRUE) %>% 
  read_stan_csv()
...
load(file.path("stan_analysis_data", str_interp("model_3_${dyn_model_version}.RData")))
```
