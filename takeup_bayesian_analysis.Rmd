...
nonparam_dyn_analysis_data <- analysis.data %>%
  ...
  # filter(!name_matched, sms.treatment.2 == "control", phone_owner, assigned.treatment %in% c("control", "calendar")) #, !hh.baseline.sample)
  # filter(!name_matched | sms.treatment.2 == "control") # There are a couple of SMS observations that are name-checked. Drop them. 

# nonparam_dyn_treatment_maps <- lst(formula = ~ (days_observed : (signal_observed + incentive_shift + reminder_info_stock)) : dyn_dist_pot,
# nonparam_dyn_treatment_maps <- lst(formula = ~ days_observed * (signal_observed + incentive_shift) * dyn_dist_pot * phone_owner,
# nonparam_dyn_treatment_maps <- lst(formula = ~ days_observed * (incentive_shift + signal_observed) * dyn_dist_pot,
nonparam_dyn_treatment_maps <- lst(formula = ~ days_observed, # * dyn_dist_pot,
                                   trends = tibble(days_observed = (0:11)/11),
                                   scale = FALSE) %>% 
  prepare_dynamic_treatment_maps(nonparam_dyn_analysis_data, day_interactions_only = FALSE)

nonparam_dyn_treatment_maps$map %<>% select(-contains(":"))

nonparam_dyn_static_treatment_map <- nonparam_dyn_analysis_data %>% 
  # data_grid(private_value, social_value, signal_observed, incentive_shift, dyn_dist_pot, reminder_info_stock, dist.pot.group, sms.treatment.2, 
            # phone_owner, name_matched) %>% 
  # data_grid(private_value, social_value, signal_observed, incentive_shift, dist.pot.group, dyn_dist_pot, phone_owner) %>%
  data_grid(private_value, social_value, dist.pot.group, dyn_dist_pot) %>% #, social_value, incentive_shift, signal_observed, dist.pot.group, phone_owner) %>%
  # data_grid(private_value, incentive_shift, dist.pot.group, dyn_dist_pot) %>% 
  prepare_treatment_map(nonparam_dyn_treatment_maps)
```
...
nonparam_dyn_stan_data <- prepare_bayesian_analysis_data(
  ...
  dynamic_treatment_map = nonparam_dyn_treatment_maps,
  ...
  # all_ate = dyn_all_ate, 
  # all_ate = dyn_sms_control_ate %>% select(-starts_with("reminder_info_stock"), -starts_with("sms.treatment")) %>% distinct(),
  all_ate = dyn_sms_control_ate %>% 
    select(starts_with("private_value"), 
           starts_with("social_value"),
           # starts_with("phone_owner"), 
           # starts_with("incentive_shift"), 
           # starts_with("signal_observed"),
           contains("dist.pot.group"),
           contains("dyn_dist")) %>% slice(2),
  # %>% 
  #   select(starts_with("private_value"), starts_with("social_value"), starts_with("signal_observed"), starts_with("incentive_shift"), contains("dist.pot.group"), contains("dyn_dist")) %>% slice(c(2, 9, 14, 21)),
...
model_3_gp_nonparam <- stan_model(file = file.path("stan_models", "takeup_model_3_gp_gumbel_fe.stan"), model_name = "model_3_gp_gumbel_fe")
```

```{r model-3-gp-nonparam, eval=FALSE}
nonparam_dyn_fit_version <- "testing_5"

model_3_fit <- nonparam_dyn_stan_data %>% 
  sampling(model_3_gp_nonparam, data = ., 
           chains = 8, iter = 300, control = lst(max_treedepth = 15, adapt_delta = 0.99), 
           # init = gen_nonparam_initializer(.),
           sample_file = file.path("stanfit", str_interp("model_3_gp_nonparam_${nonparam_dyn_fit_version}.csv")))
...
model_3_fit_debug <- nonparam_dyn_stan_data %>% 
  sampling(model_3_gp_nonparam, data = ., chains = 2, iter = 10) # , control = lst(max_treedepth = 15))
           # init = gen_nonparam_initializer(.)) 
```
...
model_3_fit %>% print(pars = c("hyper_beta"))
...
model_3_fit %>% print(pars = c("day_constant_latent_var"))
...
# mcmc_trace(posterior_data, pars = c("strata_beta_day1_tau_raw[1]"), np = model_3_np)

mcmc_nuts_divergence(model_3_np, model_3_lp)

color_scheme_set("darkgray")
mcmc_parcoord(posterior_data, np = model_3_np, 
              regex_pars = c("hyper_intercept_raw", "strata_beta_day1_tau_raw",
                             "strata_beta\\[1,"))
```
...
mcmc_pairs(model_3_posterior_array, 
           regex_pars = c(
             #"hyper_intercept_raw", #"hyper_treat_beta_day1\\[1", "hyper_beta_dyn\\[1,1",
              "strata_beta\\[1,1,[1-6]",
              "strata_beta_day1_tau_raw\\[[1-6]" 
              # "strata_beta_dyn_tau_raw\\[[1-2],1\\]"
             ),
           transform = lst(
             "strata_beta_day1_tau_raw[1]" = "log", 
             "strata_beta_day1_tau_raw[2]" = "log",
             "strata_beta_day1_tau_raw[3]" = "log", 
