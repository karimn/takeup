...
```{r prior-constants}
name_match_false_pos_alpha <- 1.24
name_match_false_pos_beta <- 10
```
...
stan_data <- analysis.data %>% 
  ...
  mutate_at(vars(county_dist_stratum, county_dist_mon_stratum, county_phone_owner_stratum, county, stratum,
            ...
            funs(id = as.integer)) %>% 
  prep_data_arranger() %>% 
  prepare_bayesian_analysis_data(name_match_false_pos_alpha, name_match_false_pos_beta)
```

```{r basic-model-0}
basic_model_0 <- stan_model(file = "takeup_model_0.stan", model_name = "model_0")
basic_model_0_fit <- sampling(basic_model_0, data = stan_data, chains = 6, iter = 1000, sample_file = file.path("stanfit", "basic_model_0.csv")) #, control = lst(adapt_delta = 0.99, max_treedepth = 15))
