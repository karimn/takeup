...
stan_data <- analysis.data %>% 
  ...
  mutate(county_phone_owner_stratum = factor(county_phone_owner_stratum),
         stratum = county_phone_owner_stratum) %>% 
  mutate_at(vars(county_dist_stratum, county_dist_mon_stratum, county_phone_owner_stratum, county, stratum), funs(id = as.integer)) %>% 
  arrange(stratum_id) %>% 
