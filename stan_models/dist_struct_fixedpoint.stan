...
parameters {
  ...
  matrix[use_county_effects ? num_counties : 0, (use_cost_model == COST_MODEL_TYPE_DISCRETE ? num_treatments * num_discrete_dist : num_treatments)] structural_beta_county_raw;
  row_vector<lower = 0>[use_county_effects ? (use_cost_model == COST_MODEL_TYPE_DISCRETE ? num_treatments * num_discrete_dist : num_treatments) : 0] structural_beta_county_sd;
...
transformed parameters {
  ...
  for (dist_index in 1:num_discrete_dist) {
    if (use_cost_model == COST_MODEL_TYPE_DISCRETE) {
      ...
      beta[(num_treatments * (dist_index - 1) + 1):(num_treatments * dist_index)] = 
        [ beta_control[1], beta_ink_effect[1], beta_calendar_effect[1], beta_bracelet_effect[1] ]';
    }
  }
...
model {
  ...
  for (cluster_index in 1:num_clusters) {
    ...
    // target += log_mix(group_dist_mix[curr_assigned_dist_group], group_dist_mix_lp);
    target += log_sum_exp(group_dist_mix_lp);
    
    // cluster_standard_dist[cluster_index] ~ normal(group_dist_mean[curr_assigned_dist_group], group_dist_sd[curr_assigned_dist_group]) T[0,];
    ...
    for (dist_group_index in 1:num_discrete_dist) {
      if (dist_group_index != curr_assigned_dist_group) {
        ...
        // target += log_mix(group_dist_mix[dist_group_index], missing_group_dist_mix_lp);
  ...
  
  {
    ...
    for (treatment_index in 1:num_dist_group_treatments) {
      ...
      if (use_cost_model == COST_MODEL_TYPE_DISCRETE) {
        ...
        treatment_dist_cost = param_dist_cost(cf_cluster_standard_dist[, cluster_treatment_map[treatment_index, 2]], // cluster_standard_dist, 
                                              cluster_linear_dist_cost[, curr_assigned_treatment],
                                              rep_vector(quadratic_dist_cost[curr_assigned_treatment], num_clusters),
    ...
    for (cluster_index in 1:num_clusters) {
      ...
      if (use_cost_model != COST_MODEL_TYPE_DISCRETE) {
        ...
        if (use_cost_model == COST_MODEL_TYPE_PARAM_KAPPA) { 
         ...
         real rep_linear_dist_cost = linear_dist_cost[curr_assigned_treatment];
          ...
          if (in_array(use_cost_model, { COST_MODEL_TYPE_PARAM_LINEAR_SALIENCE, COST_MODEL_TYPE_PARAM_QUADRATIC_SALIENCE, COST_MODEL_TYPE_SEMIPARAM_SALIENCE })) {
            ...
            if (use_single_cost_model) {
              ...
              rep_linear_dist_cost += treatment_map_design_matrix[curr_assigned_treatment] * rep_dist_beta;
            }
          } 
            
          rep_dist_cost = param_dist_cost([ cluster_standard_dist[cluster_index] ]', 
                                          [ rep_linear_dist_cost ]',
                                          [ quadratic_dist_cost[curr_assigned_treatment] ]',
                                          to_matrix(u_splines_v[curr_assigned_treatment]),
                                          to_matrix(Z_splines_v[curr_assigned_treatment]))[1];
        }
        
        cluster_rep_benefit_cost[cluster_index] = structural_treatment_effect[curr_assigned_treatment] 
          + treatment_map_design_matrix[curr_assigned_treatment] * (rep_beta_cluster + rep_beta_county) - rep_dist_cost;
      } else {
