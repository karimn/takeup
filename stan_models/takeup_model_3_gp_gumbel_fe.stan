...
transformed data {
  ...
  
  {
    ...
    for (stratum_index in 1:num_strata) {
      ...
      //   
      //   relevant_daily_cluster_sizes[curr_cluster_id] = sum(to_array_1d(relevant_latent_var_map[dewormed_day_any[cluster_obs_ids[cluster_obs_pos:cluster_obs_end]]]))
      //   
...
model {
  ...
  
  {
    ...
    for (stratum_index in 1:num_strata) {
      ...
      // int curr_num_clusters = strata_num_clusters[sratum_index];
      // int cluster_end = cluster_pos + curr_num_clusters - 1;
...
generated quantities {
  ...
  if (estimate_ate) {
    ...
    matrix[num_strata, num_ate_treatments] latent_var_map_day1 = (rep_matrix(hyper_beta_day1, num_strata) 
      + strata_beta_day1_effect) * (treatment_map_design_matrix[ate_treatments[, 1]]');
