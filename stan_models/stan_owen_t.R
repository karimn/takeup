...
if (TRUE) {
  ...
  stan_file = cmdstanr::write_stan_file(code = stan_owen_t_code)

  exposed_stan_func = expose_cmdstanr_functions(model_path = stan_file)

  saveRDS(
    exposed_stan_func,
    file = "stan_models/stan_owen_t.rds" 
  )
} else {
  exposed_stan_func = read_rds("stan_models/stan_owen_t.rds")
}