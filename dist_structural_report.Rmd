...
```{r load-data}
load(file.path("stan_analysis_data", "dist_fit15.RData"))
load(file.path("stan_analysis_data", "dist_kfold_15.RData"))
```
...
```{r, fig.width=8, fig.cap="Fit and observed distance to deworming treatment, conditional on assigned distance group."}
group_dist_param %>% 
  group_by(assigned_dist_group, mix_index) %>%
  # summarize(
  #   median_mean = median(group_dist_mean), 
  #   median_sd = median(group_dist_sd)) %>%
  # ungroup() %>%
  # mutate(distrib_samples = map2(median_mean, median_sd,
  ...
  # mutate(distrib_samples = map2(group_dist_mean, group_dist_sd, ~ tibble(x = seq(-3, 6, 0.5), density = dnorm(x, mean = .x, sd = .y)))) %>%
...
```{r cf, eval=FALSE}
cluster_cf_benefit_cost <- map(dist_fit, extract_obs_cf, par = "cluster_cf_benefit_cost", stan_data = stan_data, iter_level = "cluster", quant_probs = quant_probs, thin = 1)  
    ...
    function(.data) {
      .data %>%
        select(cluster_id, 
               ...
               iter_data) %>% 
        # mutate(iter_data = pbmclapply(mc.cores = 12, iter_data, select, iter_id, prob, iter_num_takeup)) %>% 
        ...
        summarize(
          ...
          # iter_prop_takeup = unlist(pbmclapply(mc.cores = 12, cluster_data, as_mapper(~ sum(.$iter_num_takeup) / sum(.$cluster_size)))),
          iter_prop_takeup = sum(iter_num_takeup) / sum(cluster_size),
        ) %>% 
        ungroup() %>% 
...
save(cluster_cf_benefit_cost, 
     file = file.path("temp-data", "cluster_cf_benefit_cost.RData"))
...
est_takeup_level %>%
  map(filter, is.na(mu_assigned_treatment) | mu_assigned_treatment == assigned_treatment) %>% 
  magrittr::extract(names(model_names)) %>% 
  bind_rows(.id = "model") %>% 
  mutate(model = factor(model, levels = names(model_names))) %>% 
  ggplot(aes(x = per_0.5, y = assigned_treatment, group = model), position = plot_pos) +
...
est_takeup_te %>%
  map_if(~ any(str_detect(names(.), "mu_assigned_treatment")), filter, mu_assigned_treatment_left == assigned_treatment_left, mu_assigned_treatment_right == assigned_treatment_right) %>% 
  map(filter, assigned_dist_group_left == assigned_dist_group_right, assigned_treatment_right == "control") %>% 
  magrittr::extract(names(model_names)) %>% 
  bind_rows(.id = "model") %>% 
  mutate(model = factor(model, levels = names(model_names))) %>% 
  ...
  scale_color_manual("Model", values = model_colors, labels = model_names) +
    labs(
      ...
      subtitle = TeX("E\\[Y_{z} | G = g \\] - E\\[Y_{control} | G = g\\]"),
      x = "Average Treatment Effect",
      caption = "Line range: 90% credible interval. Outer box: 80% credible interval. Inner box: 50% credible interval. 
                 Thick vertical line: median. Point: mean.") +
...
est_takeup_te %>% 
  ...
  scale_y_discrete("Assigned Signaling Treatment (z')", labels = str_to_title) +
    labs(
      ...
      subtitle = TeX("E\\[Y_{z=control,\\bar{B}_{z'}} | G = g\\] - E\\[Y_{z=control} | G = g\\]"),
      x = "Average Treatment Effect",
      caption = "Line range: 90% credible interval. Outer box: 80% credible interval. Inner box: 50% credible interval. 
                 Thick vertical line: median. Point: mean.") +
  facet_wrap(vars(assigned_dist_group_left), ncol = 1, labeller = as_labeller(. %>% str_c("g = ", .) %>% TeX(), default = label_parsed)) +
...
# rep_benefit_cost["STRUCTURAL_QUADRATIC"] %>% 
  ...
  geom_density_ridges(aes(prob, fill = model), quantile_lines = TRUE, quantiles = c(0.05, 0.95), alpha = 0.5,
                      data = . %>% unnest(iter_data)) +
  labs(x = "Takeup Prop", y = "") +
