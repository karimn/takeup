...
data {
  ...
  int<lower = 1, upper = num_obs> strata_bracelet_sizes[num_strata];
...
transformed parameters {
  ...
  
  {
    ...
    vector[num_obs] full_bracelet_val_diff = rep_vector(0, num_obs);
    ...
    for (strata_index in 1:num_strata) {
      ...
      latent_utility[stratum_pos:stratum_end] =
          ...
          full_bracelet_val_diff[stratum_pos:stratum_end] * bracelet_val_coef;
...
model {
  ...
  
  {
    ...
    for (strata_index in 1:num_strata) {
      ...
      latent_bracelet_val_diff[bracelet_val_stratum_pos:bracelet_val_stratum_end] ~ student_t(wtp_utility_df, mu_wtp_diff[strata_index], sigma_wtp_diff);
